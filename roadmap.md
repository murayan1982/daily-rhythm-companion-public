...
```text
C-1a  CURRENT / NOT_COMPLETED  Inventory current Backend/Flutter behavior and freeze the contract
C-1b  PLANNED                  Add Backend lifecycle outcomes, bounded turns, and mock-safe tests
C-1c  PLANNED                  Add Flutter lifecycle/recovery states and accept parent C-1
```

C-1a is docs/check only. It records that the accepted service has TTL/capacity/LRU bounds but no turn bound; missing sessions share one 404 detail; and Flutter retains stale session state after a terminal message error. It does not change runtime or existing tests.
