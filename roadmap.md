...
T-1a is completed and accepted at implementation commit `0b06378`: it inventories the accepted opaque MP3 handoff, 24-hour/100-artifact retention, generic audio-route 404, external URL launch flow, and the missing in-app playback/expired-state model. T-1b is completed and accepted at implementation commit `161e624`: its adapter-neutral engine interface and controller model idle/loading/playing/stopped/completed/failed/expired states with ten fake-engine lifecycle tests. Acceptance passed with 110 Backend tests, 74 Flutter tests, diff review, operator approval, and no Backend/HomeScreen/dependency changes or real TTS execution. T-1c is current and owns HomeScreen, concrete player wiring, audible playback, and artifact HTTP expiry recovery.
T-1c implementation is now present but not accepted: `audioplayers` is isolated behind the accepted engine interface, HomeScreen owns in-app play/stop/replay/regenerate controls, and HTTP 404/410 becomes expired without exposing the opaque URL. Mock-safe engine/widget tests are added; audible PC/smartphone Web evidence and aggregate acceptance remain pending.
