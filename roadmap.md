...
T-1a is completed and accepted at implementation commit `0b06378`: it inventories the accepted opaque MP3 handoff, 24-hour/100-artifact retention, generic audio-route 404, external URL launch flow, and the missing in-app playback/expired-state model. Acceptance passed with 110 Backend tests, 64 Flutter tests, diff review, operator approval, and no runtime/dependency/test changes or real TTS execution. T-1b is current; T-1c remains planned. T-1b implementation is now present but not accepted: an adapter-neutral engine interface and controller model idle/loading/playing/stopped/completed/failed/expired states with fake-engine lifecycle tests. It intentionally leaves HomeScreen, Backend, pubspec, platform plugin wiring, audible playback, and artifact HTTP expiry recovery to T-1c.
