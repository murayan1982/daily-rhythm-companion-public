...
Updated: 2026-09-03
## 30-second current view

Current official release: v3.0.0.
In preparation: v4.0.0.
Current state: implementing and reviewing the Stage 4 verifier output-encoding corrective after the initial same-artifact verifier failed before verdict.
Completed: release-candidate verification, Backend/Flutter verification, Web/Windows/APK debug builds, and Stage 2 acceptance-sync commit/push.
Not yet run: Stage 4 retry, tag, GitHub Release, and publication.
Next human decision: review the output-encoding corrective; retry remains not authorized.

Next workflow:
Control C: release-candidate verification / completed.
Control D Stage 1: fixed ZIP tooling / completed.
Control D Stage 2: Backend, Flutter, and build preflight / completed / PASS.
Control D Stage 2 acceptance-sync: verification result recording / committed and pushed.
Control D Stage 3 authorization-sync: fixed ZIP build authorization preparation / completed / accepted.
Control D Stage 3 build: create exactly one fixed ZIP / completed / pass / accepted.
Control D Stage 4 authorization-sync: committed, pushed, reviewed, accepted, and closed.
Control D Stage 4 invocation 1: executed exactly once / execution failed / verdict not reached.
Control D Stage 4 corrective R1: verifier output-encoding corrective / current.
Control E: tag, GitHub Release, and publication / not run.
...
Same-artifact verification without rebuilding: completed
Post-publication SHA-256 re-verification: completed
```
...
Status: DRC-V4-6 Control B COMPLETED / VERIFIED / ACCEPTED / COMMITTED / PUSHED / CLOSED
...
DRC-V4-6 Control A added a default-off configured provider-free FW v6 Flutter
session runtime/factory. It returns a controller factory only when explicitly
enabled and configured with a valid HTTP/HTTPS Backend base URL. Runtime
construction and factory lookup perform no HTTP client creation and no Backend
request; each returned factory invocation creates a fresh owned HTTP client,
V4-4 session client, and V4-4 controller in the idle state.

Control A does not wire HomeScreen or main.dart, open a session automatically,
execute providers, use microphone/STT/LLM/TTS/playback/VTube Studio/motion, or
replace `/realtime/text` or any accepted v3 realtime path.
...
The completed checklist is `docs/DRC_v20x_maintenance_checklist.md`. M-1 through M-9 are completed and accepted, and v2.0.1 is released.
...
- At M-5 acceptance, M-6 through M-9 remained PLANNED.
```
...
```text
- mock remains the credential-free default.
...
- fitbit remains a legacy migration/reference provider.
- Legacy token-like data and OAuth URL preparation are not reported as verified real-use success.
...
M-9 accepted contract:

```text
- The accepted M-1 through M-8 scope was frozen for v2.0.1.
- The clean official Public main gate passed at source HEAD 3e4c9f6186ef7195045a445307e14f412924bc26.
- The fixed ZIP was built once from detached committed HEAD.
- The same ZIP was verified without rebuilding.
...
- Explicit final approval was received before annotated tag and GitHub Release publication.
- The downloaded published asset matched the recorded SHA-256.
```

The completed M-9 boundary is `docs/v20x_patch_release.md`; the final record is `docs/v201_patch_release_record.md`.
...
Status: COMPLETED / ACCEPTED — W-1 through W-5, C-1, T-1, V-1, and R-1 completed; v2.1.0 released
...
C-1a is completed and accepted at implementation commit `a4263ca`. C-1b is completed and accepted at implementation commit `3055995`. It preserves TTL/capacity/LRU, adds an 8-turn default, structured lifecycle and provider-neutral outcome models, bounded expired/evicted terminal reasons, structured 404 reasons, and a restartable HTTP 409 turn-limit result. Acceptance passed with 17 focused Backend tests, 110 full Backend tests, 57 Flutter tests, diff review, and operator approval. C-1c is completed and accepted at implementation commit `c856374`: Flutter consumes structured lifecycle/outcome/problem data, remains compatible with legacy payloads, displays turn progress and user-facing outcome states, disables terminal sending, clears stale sessions, and provides direct restart. Acceptance passed with 7 focused Flutter tests, 110 Backend tests, 64 full Flutter tests, diff review, and operator approval. Parent C-1, T-1, and V-1 are completed and accepted; R-1a is completed/accepted; R-1b is completed/accepted at implementation commit `72dd42c`; R-1c and R-1d are completed/accepted; R-1e and parent R-1 are completed/accepted; v2.1.0 is released.
T-1a is completed and accepted at implementation commit `0b06378`: it inventories the accepted opaque MP3 handoff, 24-hour/100-artifact retention, generic audio-route 404, external URL launch flow, and the missing in-app playback/expired-state model. T-1b is completed and accepted at implementation commit `161e624`: its adapter-neutral engine interface and controller model idle/loading/playing/stopped/completed/failed/expired states with ten fake-engine lifecycle tests. T-1c is completed and accepted at implementation commit `4d3d5d5` plus desktop plugin registrant follow-up `9771f76`; it owns HomeScreen, concrete player wiring, audible playback, artifact HTTP expiry recovery, and regenerate recovery. Parent T-1 and V-1 are completed and accepted; R-1a is completed/accepted; R-1b is completed/accepted at implementation commit `72dd42c`; R-1c and R-1d are completed/accepted; R-1e and parent R-1 are completed/accepted; v2.1.0 is released.
T-1c and parent T-1 are completed and accepted. `audioplayers` remains isolated behind the accepted engine interface, HomeScreen owns in-app play/stop/replay/regenerate controls, and HTTP 404/410 becomes expired without exposing the opaque URL. Acceptance passed with 20 focused Flutter tests, 110 Backend tests, 84 Flutter tests, Web/Windows builds, PC and smartphone audible playback, stop/replay/completion, expiry mapping, regenerate recovery, diff review, and operator approval.
...
R-1  COMPLETED / ACCEPTED                  v2.1.0 aggregate readiness, smartphone Web evidence, and release preparation
...
W-1 through W-5 are completed and accepted. W-2 added provider-neutral token/status/reconnect states and one-time OAuth state consumption. W-3 added allow-listed Fitbit sleep API error classification, positive-duration normalization, complete `SleepSummary` real-data mapping, and deterministic fake-HTTP/API regression coverage. W-4a implementation commit `1619b0b` added the read-only provider-selection metadata contract. W-4b implementation commit `1fbea58` added Flutter configured-provider/data-source presentation, simplified Google Health normal-user UX, retained Advanced diagnostics, and mock-safe model/widget regressions. W-4b acceptance passed on 2026-07-23 after compileall, W-1/W-2/W-3/W-4a/W-4b checks, v2.0.x guards, 4 focused model tests, 35 widget tests, 92 backend tests, 57 Flutter tests, diff review, and operator approval. W-5 is COMPLETED / ACCEPTED. W-5a implementation commit `7f84980` added the public-safe dedicated env, network-free preflight, guarded launcher, explicit real-execution smoke, and operator runbook without executing Fitbit. W-5a was accepted on 2026-07-24 after compileall, default/example preflight, W-1 through W-5a checks, v2.0.x guards, 92 backend tests, 57 Flutter tests, diff review, and operator approval passed. W-5b1 was completed and accepted on 2026-07-24 at implementation commit `081cfdd` after compileall, W-1 through W-5b1 checks, v2.0.x guards, 8 focused Google Health v4 tests, 16 combined provider/migration tests, 100 backend tests, 57 Flutter tests, diff review, and operator approval passed. It corrected the platform direction, retired legacy Fitbit execution, locked the Google Health API v4 contract, and fixed the Flutter `provider_options` parsing mismatch. W-5b2 and parent W-5 are completed and accepted. The ignored operator profile and launcher gate passed, stored-token refresh succeeded, the explicit real Google Health API request returned HTTP 200, the backend produced a normalized real `SleepSummary`, and PC plus smartphone Web showed Google Health / 実データ / 取得済み. Screenshots and private sleep values remain outside Git. The operator confirmed Fitbit Versa 2 device-model provenance without publishing a device identifier. Parent W-5 is COMPLETED / ACCEPTED. C-1a, C-1b, C-1c, and parent C-1 are COMPLETED / ACCEPTED. T-1, V-1, and R-1 are COMPLETED / ACCEPTED; v2.1.0 is released from the accepted fixed artifact and verified publication tuple. V-1a is COMPLETED / ACCEPTED at implementation commit `1602b2f` as a docs/test-only inventory step. It freezes the current HomeScreen, CharacterPreset, advice source, TTS playback state, Motion Demo separation, existing widget tests, pubspec asset registration, static assets, and immutable release records. Acceptance passed after compileall, all v2.1.0 checks, v2.0.x guards, 110 Backend tests, 84 Flutter tests, diff review, and operator approval. V-1b and V-1c are COMPLETED / ACCEPTED, and no Flutter or Backend runtime change is claimed by V-1a. V-1b is completed and accepted at implementation commit `e1f8d6f` with an app-owned deterministic content/activity model, a standalone static character card, nine model tests, and four widget tests. Acceptance passed after compileall, all v2.1.0 checks, v2.0.x guards, 110 Backend tests, 9 focused model tests, 4 focused widget tests, 97 full Flutter tests, diff review, and explicit operator approval. V-1c was accepted at implementation commit `995145d` after all v2.1.0 checks, v2.0.x guards, 110 Backend tests, 9 model tests, 5 card tests, 5 HomeScreen integration tests, 103 full Flutter tests, Web/Windows builds, diff review, and operator approval passed. Backend, Motion Demo, dependencies, static assets, real motion execution, and release records remained unchanged. Parent V-1 is completed/accepted. R-1a is completed and accepted at implementation commit `dbc84db` after compileall, all 18 v2.1.0 checks, v2.0.x guards, 110 Backend tests, 103 Flutter tests, diff review, and operator approval passed. R-1b is COMPLETED / ACCEPTED at implementation commit `72dd42c`; R-1c and R-1d are COMPLETED / ACCEPTED; R-1e and parent R-1 are COMPLETED / ACCEPTED; v2.1.0 is released. The accepted R-1b owns candidate metadata and the aggregate source/test/build gate only; R-1c owns final smartphone evidence, while R-1d/R-1e retain fixed ZIP, tag, and publication work.

Current W-4 split:

```text
W-4a  COMPLETED / ACCEPTED   Read-only GET /sleep/providers metadata contract
W-4b  COMPLETED / ACCEPTED   Flutter configured-provider/data-source presentation and
                                simplified Google Health UX with diagnostics retained
```

Accepted W-4a preserves `backend/app/api/sleep.py`, `backend/app/models/sleep.py`, the existing provider factory, W-3 Fitbit runtime/normalization files, and all Flutter files. It does not instantiate providers or execute OAuth, token refresh, external network, Fitbit, or Google Health paths. Detailed contract: `docs/v210_sleep_provider_selection_source_labels.md`. Accepted W-4b consumes this metadata contract without changing W-3 semantics or claiming configured real Fitbit acceptance.
...
Current implementation commit: none
Current implementation boundary: exact 7 public documentation/static-gate files; Backend/Flutter runtime, existing tests, Stage 1 runner/focused tests, strict validator, earlier operator runners, private configuration, private manifest content, operator evidence, dependencies, version metadata, release records, RT-9 implementation, and all configured execution remain protected.
...
feedback `FW-F9` through `FW-F12`. RT-0c accepted the v5.1.0 reclassification and the remaining DRC-to-FW handoff order.

### RT-0c released Framework v5.1.0 reassessment

```text
Released Framework tag: v5.1.0
...
v5.1.0 resolves the public capability snapshot, provider-config ownership,
opaque voice artifact, and public conformance-gate feedback. It partially
resolves package import, stable factory/result, and lifecycle feedback. It does
not release public voice-input/STT, unified realtime lifecycle, hard
cancellation/TTS queue/barge-in, or motion/VTS contracts.
...
RT-1 through RT-5 remain blocked pending released voice-input, realtime, and
cancellation contracts. RT-6 through RT-7 remain blocked pending a released
motion contract. Detailed reassessment:
...
```text
RT-0   COMPLETED / ACCEPTED      Prerequisite, inventory, and public-contract readiness
...
```text
- Freeze the current DRC Backend one-shot voice-output request and opaque audio handoff.
- Freeze the current Flutter single-source local playback stop behavior.
- Separate local playback stop, app-owned queue clear, Backend synthesis cancel,
  FW queue flush, provider hard cancel, LLM stream cooperative cancel, and barge-in.
- Classify FW v5.4.0 root-public output-control readiness without using FW internals.
- Resolve RT-5b through RT-5f before runtime changes begin.
```
...
Flutter local playback stop, explicit manual flush UI, and fake-only lifecycle
tests.

Not ready or not claimable: provider synthesis hard cancel, FW-backed active
...
```text
- FIFO pending utterance queue owned by Flutter only.
- maximum 8 pending items.
- maximum 4096 Unicode code points per utterance.
- maximum 16384 retained Unicode code points across active and pending text.
- exactly one active claim.
- typed enqueue, claim, complete, fail, flush, and dispose lifecycle.
- generation increment and late-result rejection on flush.
- pending clear and active invalidation before local stop completion.
- one injected local playback-stop callback shared by concurrent flush callers.
- typed partial result when local playback stop fails.
- utterance text absent from public state, logs, persistence, and UI.
```
...
RT-5b acceptance did not itself authorize RT-5c. RT-5c was later separately
reviewed and authorized for the exact fake-only implementation below.
...
- one RT-5b FIFO claim per explicit processNext call; no automatic drain.
- completed-terminal consistency validation and 32-entry private dedup window.
- injected fake synthesis with rejected / failed / audioReady outcomes.
- maximum 2048-code-point absolute HTTP(S) opaque URI validation.
- injected fake playback future that ends only at completed / failed / expired / stopped.
- queue complete only after playback completed; fixed safe failure codes otherwise.
- operation token/epoch plus RT-5b item/generation revalidation around async boundaries.
- flush invalidates late synthesis/playback and releases the slot for new-generation work.
- no terminal text, IDs, URI, provider payload, or raw exception in public state.
```
...
speech-triggered barge-in was added. RT-5e was separately reviewed, implemented, pushed, and operator-accepted at `ef5f96337b5f601277a9bcc38b9e6fedc520b0a6` under the exact thirteen-file contract.
...
Implementation status: `COMPLETED / ACCEPTED / PUSHED` at
`ef5f96337b5f601277a9bcc38b9e6fedc520b0a6`.
...
Configured local operator acceptance passed on 2026-07-31. One explicitly
enqueued completed terminal produced real audio through the FW v5.4.0
root-public boundary and completed natural audible local playback. A second
explicit item reached active playback; explicit flush stopped the RT-5e-owned
player audibly and ended at `idle`, pending `0`, active
o`, with
`last flush=completed`, `cleared pending=0`, `stop requested=true`, and
`stop succeeded=true`.

Cleanup stopped Flutter and Backend, restored the FW real-provider gates to
disabled, removed three operator-run artifact files plus private logs/backups,
and left DRC at `ef5f96337b5f601277a9bcc38b9e6fedc520b0a6` and FW at `d313eb6acb643103fe25988720ebee5976a04f78` with clean working trees.

Implementation verification recorded the dedicated gate, FW root-public smoke,
Backend 192, Flutter analyze, focused Flutter 82, full Flutter 343, exact
...
flush, automatic TTS, speech-triggered barge-in, or real-STT-to-TTS claim is
included.

Exact implementation and public-safe operator acceptance record:
`docs/v300_rt5e_configured_local_voice_output_acceptance.md`.
The dedicated candidate gate remains a historical pre-commit gate and is not
rerun for this six-document acceptance sync.

RT-5 remains `CURRENT / NOT_COMPLETED`. RT-5f remains
...
RT-2e-c2  Operator-only harness implementation and fake/widget tests; no real execution
RT-2e-c3a Real Android operator preflight/runbook and marker-only safe evidence contract; docs/test-only
RT-2e-c3b Explicit physical-Android permission, bounded capture, automatic private-artifact cleanup evidence
```

RT-2e-c1 fixes a double opt-in operator boundary: a separate Flutter target
`lib/main_rt2ec_operator.dart`, compile-time
`DRC_RT2EC_OPERATOR=true`, and an in-app acknowledgement. The default
`lib/main.dart` and `HomeScreen` remain unchanged. Permission check, permission
...
and fake/widget tests only. RT-2e-c3a froze the physical-Android runbook,
manual permission reset/request sequence, exact operator command, one bounded
non-sensitive capture, immediate opaque-id cleanup, and marker-only safe evidence.
...
ten-file review, and explicit operator approval. RT-2e-c3b is now COMPLETED / ACCEPTED from the accepted physical-Android marker-only evidence.
...
RT-2e-c3a status: COMPLETED / ACCEPTED; docs/test-only preflight
RT-2e-c3b status: COMPLETED / ACCEPTED; authorization completed-accepted-explicit-real-android-bounded-capture-evidence
```
...
R-1c through R-1e and parent R-1 are COMPLETED / ACCEPTED. The ignored private manifest validated against accepted candidate source `1e922e68685dadfc1008f1119d0ce492584e8f19`, and the exact unchanged fixed ZIP is `DailyRhythmCompanion_v2.1.0_20260725_160036.zip` from source `6e7af31f85eb6ee7887df3e184ac6a58142d6fec` with size `1747337` bytes and SHA-256 `55bf584592b1824948ec847205132582a436f2c521feb593bac914a4904074e5`. Raw screenshots and private values remain outside Git. The annotated tag, GitHub Release, and post-publication asset verification are complete.


## Historical R-1d implementation checkpoint

`build_v210_fixed_release_zip_from_head.ps1` and `scripts/check_v210_fixed_release_zip.py` define the clean-main, one-build, same-artifact boundary. At this historical implementation checkpoint, source presence did not yet prove that a ZIP was built or verified: R-1d was `CURRENT / NOT_COMPLETED` and `IMPLEMENTED / NOT_ACCEPTED`, while R-1e remained `PLANNED`. Both are now `COMPLETED / ACCEPTED`.

## R-1d accepted artifact checkpoint
...
A superseded pre-fix candidate was rejected before publication and is not part of this tuple. R-1d is `COMPLETED / ACCEPTED`; R-1e and parent R-1 are `COMPLETED / ACCEPTED`; v2.1.0 is released.

### RT-1a released Framework v5.2.0 adoption

```text
Implementation: COMPLETED / ACCEPTED
Decision: RT1_MOCK_CONTRACT_INTEGRATION_AUTHORIZED
Released Framework: v5.2.0@c2e247064987c94bf735a359700f0462439b8286
```

v5.2.0 publishes mock-safe public Voice Input, Realtime, interrupt/output
control, and Motion session contracts. This authorizes RT-1 Backend model and
normalization work without authorizing real STT, real orchestration, hard
...
RT-6 may follow RT-1 using the public mock motion contract.
RT-7 remains blocked because real Live2D/VTS adapter execution is not implemented.
```

The released global `get_capabilities()` snapshot remains on the v5.1 schema and
still reports the new boundaries as missing. DRC RT-1 must normalize the
v5.2.0 session-specific capability/info contracts rather than trusting that
global snapshot alone.

Historical accepted RT-0b planning marker retained for its immutable check:
...
RT-2c is COMPLETED / ACCEPTED on 2026-07-27 at implementation commit `fe26c3c` after dependency resolution, exact Windows generated-plugin review, analyzer cleanup, focused Flutter 13, full Flutter 125, Backend 116 with one existing warning, the RT-2c gate, Android debug APK build, `git diff --check`, 16-file review, and operator acceptance evidence passed. The Android build emitted a Kotlin daemon incremental-cache warning in `audioplayers_android`, then completed through Gradle fallback and produced `app-debug.apk`. RT-2d is COMPLETED / ACCEPTED after fake-only lifecycle verification; RT-2e is CURRENT / NOT_COMPLETED and NOT_STARTED.


### RT-2d implementation checkpoint: capture lifecycle and fake engine

```text
status: COMPLETED / ACCEPTED
changed surface: 9 files
app-owned lifecycle/request/result/controller: added
deterministic fake capture engine: added
single active capture: enforced
hard maximum duration and deterministic deadline: enforced
permission check only; permission request: not executed
real capture dependency: not added
UI/platform/Backend: unchanged
...
RT-2d focused tests cover granted and blocked permission states, busy rejection,
duration bounds, opaque fake completion, cancel, timeout, partial-start cleanup,
stop-error cleanup, cleanup failure, exception redaction, close/dispose, and
immutable metadata. Acceptance completed on 2026-07-27 after compileall, the
RT-2d gate, `flutter analyze` with no issues, focused Flutter 17, full Flutter
...
RT-2e-c historical checkpoint: current-pending-rt2ec-implementation
RT-2e-c current status: completed-accepted
```
...
```text
private backend/local_data/voice_input/staging root
300-second TTL
maximum 8 managed artifacts
1048576-byte maximum
chunked WAV write with RIFF/WAVE structural guard
32-character opaque staging ID
path-free metadata
scoped single-use consume
explicit discard
expiry/capacity/partial cleanup
```

RT-3c2 does not add a route, multipart dependency, Flutter transfer, Framework
import/session, provider execution, transcription, or STT evidence. Acceptance passed
with compileall, four RT-3 gates, focused Backend 14, full Backend 127 with one existing
warning, clean Flutter analysis, full Flutter 192, exact 18-file surface review, and
`git diff --check`. RT-3c3 is also COMPLETED / ACCEPTED after compileall, five RT-3 gates, focused Backend 21, full Backend 137 with one existing warning, clean Flutter analysis, focused Flutter 29, full Flutter 200, exact 22-file surface review, and `git diff --check`. RT-3c4 and parent RT-3c are COMPLETED / ACCEPTED with authorization
`authorized-fake-fw-public-session-handoff-and-single-use-staged-artifact-cleanup-only`.


### RT-3c3 guarded binary upload and Flutter scoped staging consumer

Status: **COMPLETED / ACCEPTED**. Implementation: **COMPLETED / ACCEPTED**. Authorization at implementation: `authorized-guarded-binary-upload-route-and-flutter-scoped-staging-consumer-only`.

The Backend now exposes a guarded streamed `audio/wav` staging route that passes request chunks directly into the accepted private store. It enforces explicit feature/engine/adapter guards, 1 MiB body bounds, WAV/16 kHz/mono/15000 ms safe metadata, path-free errors, and path-free opaque staging metadata. Flutter now provides a scoped staging consumer using `http.StreamedRequest`; it resolves the private path only inside the accepted lease and returns only safe handoff metadata while retaining a path-free staging handle for RT-3c4.

Synthetic tests cover the route and consumer contracts. Real microphone audio transfer, Framework import, fake or real VoiceInputSession execution, provider execution, transcription, and STT remained absent. Acceptance passed with compileall, five RT-3 gates, focused Backend 21, full Backend 137 with one existing warning, clean Flutter analysis, focused Flutter 29, full Flutter 200, exact 22-file surface review, and `git diff --check`.


### RT-3c4 fake FW public-session handoff and single-use cleanup

Status: **CURRENT / NOT_COMPLETED — IMPLEMENTED / NOT_ACCEPTED**. Authorization: `authorized-fake-fw-public-session-handoff-and-single-use-staged-artifact-cleanup-only`.

RT-3c4 may consume one path-free Backend staging handle through the accepted private store, create only FW v5.3.0 public host-audio/session objects with `FakeVoiceInputProviderAdapter`, normalize the typed fake result without paths or raw bytes, and guarantee single-use consume/discard cleanup. Real provider execution, real transcription, and real STT remain forbidden.
...
`OpenAIVoiceInputRealClientFactory`, `OpenAIVoiceInputProviderAdapter`, and
`OpenAIVoiceInputRealProviderExecutor`.

All explicit operator/provider gates are checked before Framework import and
private credential-object construction. The client factory is not invoked and
the executor is not executed. DRC reads no credential value and performs no
...
is COMPLETED / ACCEPTED / PUSHED at `9b19e379634a718df2ab3ed5eb49bb20bfe7e240`. RT-5 is NOT_STARTED / NOT_AUTHORIZED and TTS queue/flush/barge-in remains excluded.
...
streaming and cooperative cancel visible UI acceptance passed with manual
bounded input. Real incremental streaming and cooperative cancel are accepted;
...
### RT-4f2 HomeScreen fake streaming UI

Status: `COMPLETED / ACCEPTED / PUSHED`

RT-4f2 adds an optional HomeScreen realtime controller factory, owned
controller/listener/dispose lifecycle, bounded manual input validation, visible
stream phase/output/error/cancel mode/hard-cancel support fields, and
cooperative cancel controls. `const HomeScreen()` remains unconfigured because
`main.dart` is unchanged and no real stream client is automatically created.
All focused coverage uses fake BackendApiClient and fake/in-memory HTTP for
the accepted RT-4e client/controller.
...
soft-barge-in coordinator. Operation epochs invalidate old work. Queue
ownership is exclusive before capture and terminal enqueue, including after
synchronous phase listeners. `processNext()` must return the current
`itemId`/`generation`.

Acceptance passed with Backend 204, Flutter analyze, focused Flutter 26,
...
compile-time gates, session-local opt-in, foreground state, and explicit
...
Acceptance passed with compileall, the dedicated RT-5f3 gate, Backend 204,
Flutter analyze, 53 focused Flutter tests, 408 full Flutter tests, exact
surface/privacy review, `git diff --check`, explicit commit approval, push, and
a clean DRC working tree.

The HomeScreen section remains metadata-only. Real end-to-end execution,
audible barge-in, threshold quality, acoustic echo cancellation, provider hard
cancel, FW queue flush, operator acceptance, and release readiness remain
unclaimed.
...
RT-6c: default-off injectable FW root-public mock MotionSession adapter
RT-6d: Flutter motion presentation model/client/controller without HomeScreen
RT-6e: default-off HomeScreen wiring through app-owned boundaries
...
RT-6c is **IMPLEMENTED / AWAITING_REVIEW** under the exact ten-file default-off root-public mock-only adapter contract.
...
## RT-6e accepted — default-off HomeScreen character-motion wiring
...
Accepted scope is limited to optional HomeScreen ownership of the RT-6d
controller, session-local default-off opt-in, explicit manual lifecycle-fact
apply/reset controls, a public-safe normalized mock-motion panel, and
fake/in-memory widget coverage. Normal `main.dart` remains unconfigured.

Acceptance passed with compileall, the dedicated gate, Backend 279, Dart
format, Flutter analyze, 16 focused tests, 468 full tests, the lifecycle
dropdown structural corrective, exact-surface/privacy review, CRLF-aware diff
checking, explicit approval, push, and a clean DRC working tree.
...
## RT-6f accepted — configured local mock-motion presentation
...
local Backend endpoint and one bounded Flutter HTTP runtime. Controls A-E
accepted the double-default-off behavior, configured idle state, speaking
2/2 mock completion, unknown 0/0 pre-import ignore, and local reset/opt-out.

The accepted implementation adds no automatic lifecycle subscription, retry,
queue, real adapter, provider execution, external Framework network work, or
visible animation claim. Completion of RT-6 does not authorize RT-7; real
Live2D/VTS adapter execution remains blocked.
...
```text
DRC_ACCEPTED_MOCK_MOTION_PATH: READY_AND_FROZEN
REAL_LIVE2D_VTS_EXECUTION: BLOCKED_FRAMEWORK_REAL_MOTION_ADAPTER_RELEASE_REQUIRED
...
support is false, configured real adapters resolve to typed
ot_implemented`,
...
## RT-7c — Guarded fixed-vendor FW v5.5.0 VTS adapter acceptance
...
```text
compileall: PASS
dedicated gate: PASS before and after regression
focused Backend: 31 passed
Backend full: 320 passed, 1 existing warning
Flutter analyze: PASS
Flutter full: 483 passed
exact implementation/corrective/privacy/diff review: PASS
...
RT-7c introduces a separate guarded real-adapter core rather than changing the
accepted RT-6 mock path. It uses the fixed released Framework v5.5.0 vendor,
root-public motion session API, preflight-before-apply, exact intent policy,
bounded public-safe result normalization, and created-session close ownership.

The strict-boolean corrective makes the double opt-in and all capability/result
boolean decisions fail closed for non-boolean values. Verification imported no
`pyvts`, opened no WebSocket, attempted no provider/network operation, and
executed no real motion.

RT-7d may now enter a separate exact contract review. This acceptance sync does
not authorize RT-7d implementation, private configuration wiring, RT-7e, or
