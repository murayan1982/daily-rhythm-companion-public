...
Current state: preparing review and approval for the Stage 4 authorization-sync that allows same-artifact verification of the fixed ZIP built in Stage 3.
...
Not yet run: same-ZIP verification, tag, GitHub Release, and publication.
Next human decision: review and approve the Stage 4 authorization-sync.
...
Control D Stage 4 authorization-sync: same-artifact verification authorization preparation / current.
...
player audibly and ended at `idle`, pending `0`, active `no`, with
