...
try {
    ...
    if ($LASTEXITCODE -ne 0 -or $branchName -ne "main") {
        throw "The official v3.0.0 fixed ZIP must be built from Public main."
    }
    ...
    foreach ($tagName in @("DRC_v2.0.0", "DRC_v2.0.1", "DRC_v2.1.0")) {
    ...
    $releaseTagOutput = @(& git tag --list "DRC_v3.0.0")
    if ($LASTEXITCODE -ne 0) {
        throw "Unable to inspect the v3.0.0 release tag."
    }
    $releaseTags = @($releaseTagOutput | Where-Object { $_ })
    if ($releaseTags.Count -gt 0) {
        throw "DRC_v3.0.0 already exists. Refusing to build another fixed v3.0.0 artifact."
    }
    ...
    if ($outputFullDirectory -ne $expectedOutputDirectory) {
        throw "The v3.0.0 fixed release ZIP must be written to the repository release directory."
    }
    ...
    $existingV300Zips = @(
        Get-ChildItem -LiteralPath $outputFullDirectory `
            -File `
            -Filter "DailyRhythmCompanion_v3.0.0_*.zip" `
            -ErrorAction Stop
    )
    if ($existingV300Zips.Count -gt 0) {
        throw "A v3.0.0 fixed ZIP already exists in release/. Refusing to build or overwrite another one."
    }

    $env:PYTHONDONTWRITEBYTECODE = "1"

    Write-Host "[Preflight] Running the strict RT-9c committed-source no-build gate..."
    ...
    if ($PreflightOnly) {
        Write-Host ""
        Write-Host "========================================"
        Write-Host "v300_fixed_release_zip_preflight_status: passed-no-build"
        Write-Host "v300_fixed_release_zip_preflight_source_branch: $branchName"
        Write-Host "v300_fixed_release_zip_preflight_source_head: $headCommit"
        Write-Host "v300_fixed_release_zip_preflight_origin_main_head: $originMain"
        Write-Host "v300_fixed_release_zip_preflight_build_invocation_count: $buildInvocationCount"
        Write-Host "v300_fixed_release_zip_preflight_fixed_zip_built: False"
        Write-Host "v300_fixed_release_zip_preflight_tag_created: False"
        Write-Host "v300_fixed_release_zip_preflight_github_release_created: False"
        Write-Host "========================================"
        return
    }

    $checklistPath = Join-Path $repoRoot "docs\DRC_v300_goal_checklist_small_commit.md"
    $checklistText = Get-Content -LiteralPath $checklistPath -Raw
    if ($checklistText -notmatch "RT-9c: COMPLETED / ACCEPTED / PUSHED") {
        throw "Actual fixed-ZIP build is blocked until RT-9c is publicly accepted."
    }
    if ($checklistText -notmatch "RT-9d: READY_FOR_EXACT_CONTRACT_REVIEW / NOT_AUTHORIZED") {
        throw "Actual fixed-ZIP build requires the accepted RT-9d entry state."
    }

    $tempRoot = Join-Path (
        [IO.Path]::GetTempPath()
    ) ("DailyRhythmCompanion_v300_fixed_" + [Guid]::NewGuid().ToString("N"))
    ...
    $match = [regex]::Match(
        $builtZips[0].Name,
        "^DailyRhythmCompanion_(\d{8}_\d{6})\.zip$"
    )
    if (-not $match.Success) {
        throw "Unexpected generic builder ZIP name: $($builtZips[0].Name)"
    }

    $fixedBasename = "DailyRhythmCompanion_v3.0.0_$($match.Groups[1].Value).zip"
    ...
    Write-Host ""
    Write-Host "========================================"
    Write-Host "v300_fixed_release_zip_build_status: built-once-from-clean-public-main"
    Write-Host "v300_fixed_release_zip_public_repository: murayan1982/daily-rhythm-companion-public"
    Write-Host "v300_fixed_release_zip_source_branch: $branchName"
    Write-Host "v300_fixed_release_zip_source_head: $headCommit"
    Write-Host "v300_fixed_release_zip_origin_main_head: $originMain"
    Write-Host "v300_fixed_release_zip_public_root_commit_count: $($rootCommits.Count)"
    Write-Host "v300_fixed_release_zip_build_invocation_count: $buildInvocationCount"
    Write-Host "v300_fixed_release_zip_path: $relativeZipPath"
    Write-Host "v300_fixed_release_zip_basename: $fixedBasename"
    Write-Host "v300_fixed_release_zip_file_size_bytes: $($destinationFile.Length)"
    Write-Host "v300_fixed_release_zip_sha256: $sha256"
    Write-Host "v300_fixed_release_zip_verification_status: not-run"
    Write-Host "v300_fixed_release_zip_tag_created: False"
    Write-Host "v300_fixed_release_zip_github_release_created: False"
    Write-Host "v300_fixed_release_zip_next_action: verify-this-same-zip-without-rebuilding"
    Write-Host "========================================"
} finally {
