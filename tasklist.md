...
current small commit: C-1 CURRENT / NOT_COMPLETED
...
```text
- accepted chat/session boundariesを棚卸しする。
- post-advice chatのturn、expired、unavailable、fallback UI状態を明確化する。
- 既存の30分idle TTL、100 session上限、LRU境界を意図なく変更しない。
- runtime変更前にsmall-commit splitとmock-safe回帰範囲を固定する。
- T-1、V-1、R-1を前倒ししない。
```

C-1の実装と受け入れはまだ行っていない。
...
W-5b2と親W-5は2026-07-24にCOMPLETED / ACCEPTEDとなった。C-1はCURRENT / NOT_COMPLETEDであり、T-1、V-1、R-1はPLANNEDのままである。
