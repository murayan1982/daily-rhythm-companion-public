...
```text
- generated voice outputをFlutter画面内で再生する。
- play / stop / replay / loading / completion / failure / expiredを区別する。
- DRC-owned opaque MP3 URLと24時間・100件のaccepted retentionを維持する。
- Framework管理path、provider payload、private audioを表示・commitしない。
- V-1とR-1を前倒ししない。
```
