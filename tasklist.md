...
現在の小コミットはない。M-8は受け入れ済みで、M-9はPLANNEDのまま。

## M-8 — v2.0.x aggregate maintenance readiness
...
```text
- M-1〜M-7の受け入れ済み保守checkを1つの通常コマンドへ集約する。
- compileall、backend pytest、任意のFlutter testを明示的にまとめる。
- v2.0.0の歴史release validatorを通常保守checkから分離する。
- M-9へ進むための固定ZIP・scope freeze・同一artifact検証条件を定義する。
```
...
Flutterを含むローカル受け入れコマンド:

```powershell
python scripts\check_v20x_maintenance_readiness.py --with-flutter
```

M-8は2026-07-23に、compileall、Flutterを含む集約check、backend pytest 38件、Flutter test 43件、差分確認、オペレーター承認の通過後に受け入れ済みとなった。M-9はPLANNEDのまま維持する。
...
完了要件を前倒ししない。M-8は受け入れ済みで、M-9は`PLANNED`を維持する。
