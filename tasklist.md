...
```text
- mock / Google Health / Fitbitの選択・設定状態を一貫して表示する。
- 使用中のsleep providerとapp-facing data sourceを明確にする。
- Google Healthの通常ユーザー向けUXを簡略化し、operator diagnosticsは明示的な境界で維持する。
- W-3で受け入れたFitbit backend normalization/API contractを変更しない。
- configured real Fitbit実行やsmartphone Web受け入れを完了扱いしない。
```

W-4の変更対象は、実コード・既存Flutter tests・W-3受け入れ状態を再確認してから固定する。現時点ではprovider選択UI、source-label UI、Google Health UX変更、configured real Fitbit実行を開始していない。
...
v2.1.0はW-1、W-2、W-3を受け入れ済みで、W-4がCURRENT / NOT_COMPLETEDである。configured real Fitbit実利用、chat、TTS、character、release readinessは未実装・未受け入れ。provider選択・source-label UI・Google Health UX整理はW-4でこれから着手する。v3.0.0は計画段階である。
