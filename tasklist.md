...
## M-9 — v2.0.1 patch release preparation
...
```text
- M-1〜M-8のaccepted patch scopeを固定する。
- official Public mainのfinal committed source gateを追加する。
- committed HEADからfixed ZIPを一度だけ作るbuilderを追加する。
- 同じZIPを再構築せず、SHA-256・CRC・内容・testsを検証する。
- v2.0.1 release notesとpatch release record契約を準備する。
- 最終承認前にtag / GitHub Releaseへ進まない。
```
...
既存scripts/check_v20x_*.pyのcurrent-state gate
```
...
準備コミット後のfinal source gate:

```powershell
python scripts\check_v20x_patch_release.py --source-tree --with-flutter
```

その通過後だけ、一度だけbuilderを実行する:

```powershell
.\build_v201_fixed_release_zip_from_head.ps1
```

builderが表示した同じZIPを`--release-zip`で検証し、source HEAD、ZIP名、size、SHA-256、検証結果を提示して停止する。明示的な最終承認を受けるまでtag / GitHub Releaseへ進まない。

M-8は2026-07-23に受け入れ済み。M-9は現在準備中であり、v2.0.1は未リリース。
...
M-9  CURRENT    release: prepare final source, one-time fixed ZIP, same-ZIP verification, and patch release record gates
```

完了要件を前倒ししない。M-8は受け入れ済みで、M-9は`CURRENT / NOT_COMPLETED`。fixed ZIP、tag、GitHub Release、v2.0.1 RELEASEDはまだ実施しない。
