...
M-1〜M-9は受け入れ済みで、v2.0.1は正式リリース済み。W-1とW-2も受け入れ済みで、v2.1.0のauthoritative checklistとtoken/status/reconnect境界が確立された。次の実装変更はW-3の別小コミットとして進める。
...
v2.1.0はW-1とW-2を受け入れ済みで、W-3がCURRENT / NOT_COMPLETEDである。configured real Fitbit実利用、provider選択、chat、TTS、character、release readinessは未実装・未受け入れ。v3.0.0は計画段階である。
