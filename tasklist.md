...
RT-6f: BLOCKED_PENDING_RT6E_ACCEPTANCE / NOT_AUTHORIZED
```

- [x] optional HomeScreen controller factoryを追加
- [x] factory未注入時をunconfiguredのまま維持
- [x] session-local opt-inをdefault off・非永続で追加
- [x] opt-in単独ではtransportを呼ばない
- [x] explicit apply 1回につきtransport最大1回
- [x] source event typeを`home_screen_manual_motion`へ固定
- [x] source session/turn IDを使用しない
- [x] reset/opt-outをlocal-onlyにしてlate completionを無効化
- [x] aggregate mock-only panelを追加
- [x] ID、raw command/event/response/exceptionを非表示
- [x] main.dart、Backend、RT-6d runtime、依存関係を非変更
- [ ] focused Flutter widget testを実環境で通す
- [ ] Flutter全回帰を実環境で通す
- [ ] exact surface/privacy/diff reviewを通す
- [ ] commit/pushは明示承認後のみ実行
