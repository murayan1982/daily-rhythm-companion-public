...
```text
- W-1で固定した現状契約を前提に、token/status/reconnect状態をhardeningする。
- fake HTTP responseと一時token/state storeを使うmock-safe実装・テストを優先する。
- configured real Fitbit検証、実sleep取得、smartphone Web受け入れはW-5まで完了扱いしない。
- W-2の具体的な変更対象は、実コード差分計画を別途提示してから確定する。
```

W-2実装はW-1受け入れ同期には含まれていない。
...
M-1〜M-9は受け入れ済みで、v2.0.1は正式リリース済み。W-1も受け入れ済みで、v2.1.0のauthoritative checklistが確立された。次の実装変更はW-2の別小コミットとして進める。

---

# 6. Later version boundaries
...
v2.1.0はW-1を受け入れ済みで、W-2がCURRENT / NOT_COMPLETEDである。Fitbit実利用、provider選択、chat、TTS、character、release readinessは未実装・未受け入れ。v3.0.0は計画段階である。
