...
## RT-7c — guarded vendored FW v5.5.0 VTS session adapter candidate

```text
status: IMPLEMENTED / AWAITING_REVIEW
baseline: 35582f06ca037401b2cef8d97cfc5fc26cd40654
Framework local source: vendor/ai-character-framework-5.5.0
change surface: exact 11 files
...
- [x] RT-6 mock-specific model/adapter/routeをprotected non-changeへ固定
- [x] fixed vendor root-public package origin検証を追加
- [x] Framework development checkout/CWD/sys.path fallbackを不使用
- [x] explicit-only private VTS config value objectを追加
- [x] disabledとprovider-disallowedの二重closed guardを追加
- [x] expression / emotion / gesture / reset_expressionをrequired化
- [x] stop_motion unsupportedをoptional safe skip化
- [x] speaking_state / idle_motion / look_atをmodel validationで拒否
- [x] preflight-before-applyとintent毎capability確認を追加
- [x] bounded allowlist result/event/boolean normalizationを追加
- [x] session closeをcreated-session全経路で実行
- [x] raw exception/private endpoint/token/hotkey/ID/payload非露出
- [x] pyvts==0.3.3 / websockets==16.0 exact pinを追加
- [x] fake/injected focused Backend testsを追加
- [x] exact 11-file candidate gateを追加
- [ ] 実checkout compileall / gate / focused / full regressions
- [ ] exact surface/privacy/diff review
- [ ] commit/pushは明示承認後のみ
- [ ] RT-7d/RT-7eは別exact contractまで開始しない
