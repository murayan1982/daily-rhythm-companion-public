...
R-1  CURRENT / NOT_COMPLETED
  ...
  R-1e  CURRENT / NOT_COMPLETED  Explicit approval, publication, and post-publication verification (NOT_STARTED)
```
...
R-1d is `COMPLETED / ACCEPTED`. The fixed ZIP remains ignored under `release/` and must not be renamed, rebuilt, overwritten, or modified. R-1e is `CURRENT / NOT_COMPLETED` and `NOT_STARTED`; it requires explicit approval for this exact tuple before publication.
