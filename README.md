...
Current realtime action: review the exact ten-file RT-6b candidate; commit/push and RT-6c remain NOT_AUTHORIZED
...
RT-6b is an **IMPLEMENTED / AWAITING_REVIEW** exact ten-file Backend pure-mapping candidate against baseline `6ed5f2252c6c6f47fc8c50f577c4f20b7fa0cb68`. It adds app-owned bounded models, a deterministic stateless mapper, focused Backend tests, documentation, and a static/runtime contract gate.

```text
RT-6a: COMPLETED / ACCEPTED / PUSHED
RT-6b: IMPLEMENTED / AWAITING_REVIEW
exact surface: 10 files
Backend runtime: 2 new pure-mapping files
Backend focused test: 1 new file
FW import/execution: none
network/provider/VTS/Live2D execution: none
RT-6c implementation: NOT_AUTHORIZED
```

The mapper converts accepted app-owned lifecycle facts into maximum-three-command plans using `expression`, `speaking_state`, `idle_motion`, `stop_motion`, and `reset_expression`. `motion_active` prevents recursive motion planning and `unknown` fails closed as ignored. Arbitrary metadata is not accepted. Existing motion-demo routes remain metadata-only and unchanged. Flutter is unchanged.

Generation-side verification passed 37 focused Backend tests and 241 full Backend tests. Real-checkout gate, Flutter regression, exact-surface review, explicit approval, commit, and push remain required.

Detailed candidate contract: `docs/v300_rt6b_provider_neutral_motion_mapping.md`.
Dedicated candidate gate: `scripts/check_v300_rt6b_provider_neutral_motion_mapping.py`.
