...
Current realtime action: review the exact ten-file RT-6c candidate; commit/push and RT-6d remain NOT_AUTHORIZED
...
`d313eb6acb643103fe25988720ebee5976a04f78`, with local source mode
`external-vendored-snapshot`. The vendor directory remains outside DRC Git
history; normal verification checks the required root-public contract rather
than requiring vendor Git identity. RT-6c adds a default-off Backend adapter
from accepted RT-6b motion plans to a new FW root-public
`create_motion_session(adapter="mock")` session for each execution.

The adapter forces `real_adapter_enabled=false` and
`allow_provider_execution=false`, runs at most three commands synchronously,
fails fast, closes its owned session, retains only bounded normalized DRC
results and event type strings, and exposes no raw FW object or identifier.
No route, config flag, normal runtime caller, Flutter file, Framework source,
dependency, provider, network, VTS, or Live2D execution is added.
