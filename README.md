...
Current implementation: RT-5f3 exact twenty-file default-off configured integrated voice-turn candidate.
...
Current realtime action: review the exact twenty-file RT-5f3 candidate; do not commit or push without explicit approval
...
RT-5f3 is **IMPLEMENTED / AWAITING_REVIEW** under the separately approved
exact twenty-file contract.
...
The candidate adds a default-off configured integrated voice-turn factory,
session-local HomeScreen controls, a dedicated non-shared stream/TTS ownership
graph, and a bounded `record: 6.2.1` speech-activity detector. PCM stream bytes
are drained and dropped; only dBFS samples enter a three-consecutive-sample,
one-event-per-generation state machine. The detector is disarmed during initial
capture and may arm only during staging, transcript acquisition, streaming, or
voice output while the app is foreground and opted in.

HomeScreen displays metadata only. No real microphone, network, provider, STT,
synthesis, playback, audible interruption, or operator acceptance is executed
or claimed by synthetic verification.
