...
Current implementation: DRC-V4-6 Control A configured provider-free FW v6 Flutter session runtime/factory.
...
DRC-V4-6 Control A baseline: `26a4222eec724a7517f2e812dfe4bd039c5b511d`
DRC-V4-6 Control A commit: none
DRC-V4-6 Control A commit / push: NOT_AUTHORIZED
...
Current v4 action: DRC-V4-6 Control A configured provider-free FW v6 Flutter session runtime/factory
...
RT-4  COMPLETED / ACCEPTED  Streaming LLM, DRC event consumption, and cooperative cancellation
...
RT-5  COMPLETED / ACCEPTED  TTS queue/output control and interruption
  ...
  RT-5e  COMPLETED / ACCEPTED / PUSHED  Configured local synthesis and playback-stop acceptance
  RT-5f  COMPLETED / ACCEPTED  Speech-triggered DRC-local soft barge-in and real-STT-to-TTS integration
    RT-5f0  COMPLETED / ACCEPTED / PUSHED  Readiness decision and exact real-input/soft-barge-in split
...
RT-8  COMPLETED / ACCEPTED  Platform-appropriate PC Windows and Android realtime acceptance evidence
  ...
  RT-8c  COMPLETED / ACCEPTED / PUSHED  Configured PC Windows realtime acceptance
    ...
    RT-8c Stage 3  COMPLETED / ACCEPTED / PUSHED  PC acceptance synchronization
  ...
  RT-8e  COMPLETED / ACCEPTED / PUSHED  Aggregate cleanup and RT-8 acceptance sync
...
RT-9  COMPLETED / ACCEPTED  Security, cleanup, aggregate readiness, and release
  ...
  RT-9d  COMPLETED / ACCEPTED  Fixed ZIP built, same-artifact verified, and tuple recorded
  RT-9e  COMPLETED / ACCEPTED  Publication and post-publication verification
T-1  COMPLETED / ACCEPTED
V-1  COMPLETED / ACCEPTED
  V-1a  COMPLETED / ACCEPTED
  V-1b  COMPLETED / ACCEPTED
  V-1c  COMPLETED / ACCEPTED
R-1  COMPLETED / ACCEPTED
...
## v4.0.0 readiness baseline

DRC-V4-1 is **CLOSED / ACCEPTED** as the public docs/static-gate acceptance
sync for AI Character Framework v6.0.0 readiness. The required aggregate
decision remains **PARTIAL_READY**.
...
Current v4 checklist:
`docs/DRC_v400_goal_checklist_small_commit.md`.

Dedicated gate:
`scripts/check_v400_framework_v600_readiness_acceptance.py`.

DRC-V4-1 changes no Backend runtime/tests, Flutter runtime/tests,
dependencies/lockfiles, version metadata, Framework vendor, `.gitignore`, v3
release records, or private config/evidence. It performs no provider/network,
microphone, real STT/LLM/TTS, playback, VTube Studio, or real-motion execution.
...
DRC-V4-3 exposes the accepted DRC-V4-2 provider-free adapter through a bounded
FastAPI API at `/realtime/framework-v6/provider-free`. It does not claim or
enable a production real unified RealtimeSession pipeline.
...
DRC-V4-6 Control A configured provider-free FW v6 Flutter session
runtime/factory is **IMPLEMENTED / AWAITING_REVIEW** against baseline
...
Control A adds only a default-off configured runtime/factory around the
accepted V4-4 Flutter client/controller. It does not wire HomeScreen or
main.dart, open a session automatically, execute providers, or replace the
existing accepted v3 realtime paths.
...
RT-4a is **COMPLETED / ACCEPTED / PUSHED** and remains docs/test-only.
It froze the accepted RT-3 baseline, the current synchronous DRC text-chat
...
boundary before any RT-4 runtime code was added.
...
RT-4a changed no Backend or Flutter runtime, existing tests, dependency, API
route, version, provider execution, private audio/transcript handling, or
release record. Acceptance passed with the dedicated gate, Backend 163,
Flutter analyze, Flutter 200, diff review, explicit approval, commit, and push.

## v3.0.0 RT-4b Backend provider-neutral text stream contract

RT-4b is **COMPLETED / ACCEPTED / PUSHED**. It adds a Backend-only,
Framework-free state machine for bounded incremental response events. The
contract owns session/turn snapshots, monotonic sequence, text chunks,
completed/cancelled/failed/closed terminals, cooperative cancel requests, and
late/stale callback rejection.

Detailed contract: `docs/v300_rt4_backend_stream_contract.md`.
Dedicated gate: `scripts/check_v300_rt4_backend_stream_contract.py`.

RT-4b adds no FastAPI route, SSE/WebSocket transport, Framework import,
provider call, dependency, Flutter change, hard-cancel claim, TTS output
control, version, or release record. RT-4b acceptance passed with the dedicated gate, 13 focused Backend tests, 176 full Backend tests, Flutter analyze, Flutter 200, exact ten-file review, explicit approval, commit, and push.
...
Dedicated gate: `scripts/check_v300_rt4_backend_sse_transport.py`.
...
## v3.0.0 RT-4e Flutter stream client/controller

RT-4e is **COMPLETED / ACCEPTED / PUSHED** at `1cfe6134b0d19a4d14ebcf3ec76812ce07dac261`. It adds Flutter-only
stream models, an injectable HTTP/SSE client, and a ChangeNotifier controller
for the accepted RT-4c/RT-4d Backend text-stream contract.

Accepted behavior includes incremental UTF-8 SSE parsing, CRLF/LF HTTP
chunk-boundary handling, same-origin `events_path` and `cancel_path`
enforcement, monotonic sequence/session/turn validation, event
type/state/payload/terminal validation, Unicode code-point chunk/output/safe
message bounds, cooperative cancel only, `hard_cancel_supported=false`,
failed/terminal/dispose subscription cleanup, active-stream replacement and
simultaneous start rejection, and preserving `cancelRequested` when a delayed
`streamStarted` arrives after local cancel.

Normal tests use fake/in-memory transport only. HomeScreen integration, STT
transcript handoff, real Backend/Framework/provider execution, and TTS
queue/flush/barge-in remain absent from RT-4e.

Detailed contract:
`docs/v300_rt4_flutter_stream_client_controller.md`.

RT-4e acceptance passed with compileall, the dedicated gate, Backend 192 with
...
RT-4f is **COMPLETED / ACCEPTED**. RT-4f1 is **COMPLETED / ACCEPTED /
PUSHED** at `f54e8638f0255b28e015702bc64b624a6d4a36af` and is docs/test-only. It records the current HomeScreen,
...
Resolved RT-4f split: RT-4f1 inventory only, RT-4f2 HomeScreen fake streaming
UI/controller lifecycle wiring, and RT-4f3 app-owned provider-neutral
transcript-to-stream handoff are completed, accepted, and pushed. RT-4f4
...
pushed. Real-STT-to-stream was not executed or accepted, and RT-5 TTS
queue/flush/barge-in remains excluded.

RT-4f1 acceptance verification: implementation commit
`f54e8638f0255b28e015702bc64b624a6d4a36af`, implementation pushed true,
compileall passed, dedicated RT-4f1 gate passed, Backend full tests 192 passed
with one existing warning, Flutter analyze passed, Flutter full tests 233
passed, exact seven-file review passed, `git diff --check` passed, factual
transcript inventory correction accepted, explicit operator approval accepted.

## v3.0.0 RT-4f2 HomeScreen fake streaming UI

RT-4f2 is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`1e1a4b27a0fe7c105eec344bfde39afe6a077f8a`. It adds HomeScreen ownership
of an optional `RealtimeTextStreamController` factory, a bounded manual input
stream section, visible phase/output/cancel/problem state, cooperative cancel
UI, and fake/in-memory widget coverage. Normal `const HomeScreen()` remains
unconfigured because `main.dart` is unchanged and no real client is constructed
automatically.
...
RT-4f2 adds no STT transcript handoff, real Backend/FW/provider execution,
configured runtime wiring, automatic TTS start, TTS queue/flush/barge-in,
provider-level hard cancel, reconnect/resume, or WebSocket.

Acceptance passed with compileall, the dedicated RT-4f2 source-tree gate,
Backend full tests 192 passed with one existing warning, Flutter analyze,
focused RT-4f2 widget tests 9 passed, Flutter full tests 242 passed, exact
ten-file implementation review, `git diff --check`, HomeScreen unrelated
formatting removal, a 239 additions / 0 deletions HomeScreen implementation
numstat, a 240 Unicode code-point public-safe problem display bound, corrected
input echo assertion, fake/in-memory HTTP only, and explicit operator approval.
RT-4f3 is COMPLETED / ACCEPTED / PUSHED at implementation commit
`d651a00be8713a70be3a46524f33c787299bbe9c`. RT-4f4 is COMPLETED /
ACCEPTED / PUSHED at `9b19e379634a718df2ab3ed5eb49bb20bfe7e240`.
...
## v3.0.0 RT-4f3 transcript-to-stream handoff

RT-4f3 is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`d651a00be8713a70be3a46524f33c787299bbe9c`. It adds a provider-neutral
`ProviderNeutralTranscriptResult` model and an app-owned
`RealtimeTextStreamTranscriptHandoff` service that accepts only injected/fake
final transcript results, validates opaque result IDs and transcript text,
marks consumed result IDs before exactly one `controller.start(inputText:)`,
and retains no transcript text in public state. The model fields are limited to
opaque `resultId`, transcript `text`, and `isFinal`; it carries no provider
name, model name, confidence, audio path, raw provider payload, or credential.
Transcript text is bounded to 4096 Unicode code points, result IDs to 128
Unicode code points, and remembered consumed result IDs to 32.

HomeScreen accepts an optional transcript handoff factory and remains
unconfigured in normal `const HomeScreen()` because `main.dart` is unchanged.
The UI exposes a provider-neutral transcript handoff subsection, does not show
transcript text or result IDs, does not copy transcript text into manual input,
does not read `VoiceInputDemoRequestResponse.transcript`, and starts no TTS.

RT-4f3 uses transcript text only as the local argument to one
`controller.start()` invocation. Transcript text is not copied into handoff
public state, retained in a handoff field, copied into HomeScreen manual input,
rendered by the UI, logged, or persisted. Opaque result IDs are not rendered;
only a bounded in-memory consumed-ID collection may retain them.

Accepted verification passed on 2026-07-30: compileall, dedicated RT-4f3 gate,
Backend 192 with one existing warning, Flutter analyze, focused RT-4f3 unit
tests 15, focused RT-4f3 HomeScreen widget tests 9, Flutter full tests 266,
exact thirteen-file implementation review, `git diff --check`, HomeScreen
implementation numstat 115 additions / 0 deletions, independent in-flight
guard, three-or-more concurrent invocation coverage, concurrent create-failure
exactly-once coverage, explicit operator approval, commit, and push.

RT-4f3 adds no configured real transcript source, real microphone/STT
execution, Backend transcript route, configured runtime wiring,
Framework/provider execution, hard cancel, reconnect/resume, WebSocket,
automatic TTS playback, or TTS queue/flush/barge-in. The private real-STT
operator result still does not automatically reach Flutter. RT-4f4 accepted
configured manual-input local Backend/FW text streaming and cooperative cancel
visible UI acceptance, but real-STT transcript handoff acceptance remains false
without a separately reviewed safe app-visible transcript source.

## v3.0.0 RT-4f4 configured local stream acceptance

RT-4f4 is **COMPLETED / ACCEPTED / PUSHED** at `9b19e379634a718df2ab3ed5eb49bb20bfe7e240`. It adds default-off Flutter
startup wiring that can construct the accepted RT-4e
`RealtimeTextStreamController` from `BackendApiClient.baseUrl` when
`DRC_RT4_ENABLE_CONFIGURED_TEXT_STREAM=true`.

The configured runtime validates absolute `http`/`https` Backend URLs, rejects
invalid URLs safely as unconfigured, lazily creates a fresh `http.Client`,
`RealtimeTextStreamClient`, and `RealtimeTextStreamController` per factory
call, and starts no network request during runtime construction, factory
lookup, or widget pump. Controller dispose closes the HTTP client through the
accepted RT-4e ownership path.

Configured local operator acceptance passed with manual bounded input:
configured local Backend/FW execution passed, one `stream_started` event,
23 `stream_chunk` events, one `stream_completed` event, chunks arriving at
multiple distinct times, incremental UI output before terminal, duplicate
start blocking while active, cooperative cancel POST HTTP 200,
`cancel_requested` UI phase, cancelled terminal, retained partial output after
cancel, stopped output after cancellation, and start/cancel button recovery.

RT-4f4 does not change HomeScreen, RT-4e client/controller, RT-4f3 handoff,
Backend, FW, dependencies, versions, platform permissions, or release
artifacts. It does not configure a real transcript source, does not connect
private real-STT output to Flutter, does not claim real-STT-to-stream
acceptance, keeps cooperative cancel only with `hard_cancel_supported=false`,
and does not start automatic TTS or RT-5 TTS queue/flush/barge-in work.
...
## v3.0.0 RT-5a TTS output-control inventory

RT-5a is **COMPLETED / ACCEPTED / PUSHED** at `1cf77774dca75b9875099c2b6c6c03992456d80f` and remains docs/test-only. It
records the current DRC one-shot voice-output request boundary, the Flutter
single-source local playback controller, the absence of an app-owned TTS queue,
the absence of automatic realtime stream-to-TTS wiring, and the released FW
v5.4.0 root-public output-control readiness.
...
RT-5a changes no Backend or Flutter runtime, existing tests, dependency,
permission, version, release record, FW file, real TTS execution, audio
playback execution, provider client, network request, microphone use, or
automatic stream-to-TTS behavior.

Acceptance passed on 2026-07-30 with compileall, the dedicated RT-5a candidate gate, Backend 192 passed with one existing warning, Flutter analyze, Flutter 278 passed, exact seven-file review, changed-content privacy scan, `git diff --check`, explicit operator approval, commit, and push. Implementation commit: `1cf77774dca75b9875099c2b6c6c03992456d80f`.

```text
RT-5 runtime behavior changed: false
real TTS executed: false
audio playback executed: false
automatic TTS added: false
provider hard cancel claimed: false
RT-5b authorized: false
```

## v3.0.0 RT-5b app-owned voice-output queue

RT-5b is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`c48238256cb0b17c925f8063c3b636d3b4ccf533` as an exact Flutter-only fake/in-memory boundary. It
adds a bounded FIFO utterance queue, one active claim, typed complete/fail
results, generation-based late-result rejection, pending/active clear, and one
injected local playback-stop request per flush.

Fixed bounds:

```text
pending items: 8
utterance: 4096 Unicode code points
retained active + pending text: 16384 Unicode code points
active claims: 1
```

Public queue state exposes counts, phase, generation, active metadata, and
bounded technical outcomes, but not utterance text. Utterance text and opaque
item IDs are not logged or persisted.
...
Acceptance passed on 2026-07-30 at implementation commit
...
commit, and push. The dedicated gate remains a historical pre-commit candidate
gate and is not rerun for the later docs-only acceptance sync.

RT-5b changes no Backend, HomeScreen, `main.dart`, existing local player,
Backend API client, dependency, permission, version, release record, or
Framework file. It performs no HTTP request, synthesis, provider execution,
real audio playback, automatic TTS, Framework real queue flush, provider hard
cancel, or speech-triggered barge-in. RT-5b acceptance did not itself
authorize RT-5c. RT-5c was later separately reviewed and authorized for the
exact fake-only nine-file implementation completed and accepted at `f00214cd7e75b28c041728bca6ffc3b180face80`.

## v3.0.0 RT-5c realtime-terminal voice-output orchestration

RT-5c is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`f00214cd7e75b28c041728bca6ffc3b180face80`. It adds an explicit Flutter-only
orchestrator that validates a completed realtime terminal, enqueues it into the
accepted RT-5b queue, claims one FIFO item per explicit `processNext()` call,
invokes injected fake synthesis, validates a bounded absolute opaque HTTP(S)
URI, waits for an injected fake terminal playback lifecycle, and then completes
or fails the queue item.

The orchestrator uses both an operation epoch/token and RT-5b item/generation
checks. Flush during synthesis cannot start late playback; flush during playback
makes the late terminal inert; a new generation can process without waiting for
an invalidated old delegate future. Completed-terminal duplicate keys are
private, in-memory, and bounded to 32 entries. Public state contains no terminal
text, session/turn ID, audio URI, provider payload, or raw exception.
...
Acceptance passed on 2026-07-31 at implementation commit
...
changed-content privacy review, `git diff --check`, explicit operator approval,
commit, and push. The dedicated gate remains a historical pre-commit candidate
gate and is not rerun for the later six-document acceptance sync.

No HomeScreen integration, Backend HTTP execution, real synthesis, real audio
playback, automatic TTS, Framework real output flush, provider hard cancel, or
speech-triggered barge-in was added. RT-5d was later separately reviewed and
authorized for the exact fake-only HomeScreen implementation below.

## v3.0.0 RT-5d HomeScreen manual voice-output controls

RT-5d is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`eff46a3b4de771aa37a48ea9ef5959918e407200` under the separately authorized exact ten-file
fake-only UI contract.

It adds an optional HomeScreen binding factory, session-local default-off
opt-in, and three independent manual controls:

```text
Enqueue completed terminal
Process next queued item
Flush RT-5 queue
```

Stream completion and opt-in alone start nothing. Enqueue does not process.
Each process press handles at most one RT-5b FIFO item. Flush invalidates the
app-owned queue and requests only the injected local fake stop.

The binding owns orchestrator teardown and binding-owned cleanup exactly once.
HomeScreen removes its listener before disposal. A UI operation sequence keeps
late invalidated synthesis/playback futures from overwriting a newer generation
after flush.

Visible state is limited to configuration, opt-in, phase, pending/active
metadata, typed action outcomes, flush counts/booleans, and fixed technical
codes. The RT-5d section does not display terminal text, session/turn IDs, queue
item IDs, audio URIs, provider payloads, or raw exceptions.
...
reviewed as an insertion-only `+396/-0` diff after formatter-noise cleanup.

The dedicated candidate gate remains a historical pre-commit gate bound to
baseline `04b52a2e12d5f4dafd4e9a1172d628c6c58f9a70` and the exact ten-file
candidate. It is not rerun for the later six-document acceptance sync.

Normal `main.dart` remains unchanged and does not configure this binding.
...
speech-triggered barge-in. RT-5e was later separately reviewed, implemented,
pushed, and operator-accepted at `ef5f96337b5f601277a9bcc38b9e6fedc520b0a6` under the exact thirteen-file
...
`ef5f96337b5f601277a9bcc38b9e6fedc520b0a6` under the separately authorized exact thirteen-file contract.
The default-off configured Flutter runtime reuses the existing Backend
voice-output route and FW v5.4.0 root-public
...
```text
configured runtime visible: true
opt-in default off: true
explicit enqueue accepted: true
one explicit process action: true
real FW root-public synthesis: accepted
natural audible local playback: accepted
explicit flush during active playback: accepted
binding-owned local playback stopped audibly: true
last flush: completed
cleared pending during active playback: 0
local playback stop requested: true
local playback stop succeeded: true
final RT-5 phase: idle
final pending: 0
final active: no
operator artifact files removed: 3
operator artifacts remaining: false
private logs/evidence committed: false
...
An explicit flush clears/invalidates only app-owned queue work and requests stop
...
Implementation verification passed with the dedicated source gate, FW
root-public boundary smoke, Backend 192 tests, Flutter analyze, 82 focused
Flutter tests, 343 full Flutter tests, exact thirteen-file review,
HomeScreen semantic-only `+6/-6`, privacy review, and `git diff --check`.

Detailed contract and public-safe acceptance record:
`docs/v300_rt5e_configured_local_voice_output_acceptance.md`.
Historical pre-commit candidate gate:
`scripts/check_v300_rt5e_configured_local_voice_output_acceptance.py`.

Private credentials, input/output text, provider payloads, audio URLs, raw
audio, artifact IDs, private paths, LAN addresses, screenshots, and raw logs
are not committed. RT-5e acceptance does not authorize RT-5f.

## v3.0.0 RT-5f0 real-input and soft-barge-in readiness

RT-5f0 is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`348669884e872475aaa4242a5960a6de6fb7e10b` as the exact seven-file
docs/test-only readiness and split checkpoint.

The accepted review confirms that real microphone capture, private real STT,
configured text streaming, app-owned TTS queueing, real FW synthesis, local
playback, and explicit local playback stop exist as separately accepted
boundaries. It also confirms three remaining integration gaps:

```text
app-visible real-STT transcript source: absent
normal main.dart microphone/STT wiring: absent
speech-activity/onset source: absent
```

FW v5.4.0 remains suitable for root-public one-shot STT, text streaming,
one-shot TTS, and cooperative text interruption, but its unified realtime
runtime, real output flush, and provider hard cancel remain unsupported. RT-5f
therefore limits the product claim to **speech-triggered DRC-local soft
barge-in**. This may stop the RT-5e-owned local player, invalidate app-owned
queue and stale results, and request cooperative text-stream cancellation. It
must not claim Backend HTTP cancellation, provider synthesis cancellation, FW
real queue flush, or provider-level hard cancellation.

Acceptance verification passed with compileall, the dedicated pre-commit gate,
Backend 192 tests with one existing warning, Flutter analyze, 343 Flutter
tests, exact seven-file review, private-content review, `git diff --check`,
explicit approval, push, and post-push clean-tree verification.

The accepted follow-up split is RT-5f1 app-visible real-STT source, RT-5f2
fake-only integrated voice-turn/soft-barge-in coordinator, RT-5f3 default-off
HomeScreen and production speech-activity wiring, and RT-5f4 configured local
real-STT-to-stream-to-TTS plus audible speech-triggered soft-barge-in
acceptance. Each runtime step requires a separate exact contract review and
...
## v3.0.0 RT-5f1 app-visible real-STT transcript source

RT-5f1 is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`daca3a68672eb3106e861278ebb65612380140ed` under the exact seventeen-file default-off contract.

The accepted implementation adds the body-only provider-neutral Backend
transcript endpoint, private host-owned credential boundary, process-wide
single-flight execution gate, FW v5.4.0 root-public real-STT executor reuse,
bounded final transcript response, and injectable Flutter transcript provider.
It preserves single-use staged WAV cleanup and the existing
`ProviderNeutralTranscriptResult` / transcript-to-stream handoff boundary.

Acceptance verification passed with:
...
Candidate validation remained credential-free, provider-free, network-free,
microphone-free, playback-free, and real-transcript-free. Normal `main.dart`,
HomeScreen, automatic TTS, speech activity, and barge-in remain unchanged.
...
## v3.0.0 RT-2a microphone permission/capture inventory

RT-2a records the current Flutter and platform permission surface and fixes the
RT-2 small-commit split before any microphone dependency, platform permission,
or capture code is added. The current app has no microphone package, Android
`RECORD_AUDIO`, iOS `NSMicrophoneUsageDescription`, browser capture adapter, or
app-owned permission/capture abstraction. The existing voice-input demo remains
metadata-only and explicitly states that it does not record audio.
...
RT-2c pins `permission_handler` 12.0.3, adds an Android/iOS-only implementation
of the app-owned permission gateway, declares Android `RECORD_AUDIO`, and adds the
iOS `NSMicrophoneUsageDescription` string. `flutter pub get` also resolves the
transitive Windows implementation and updates Flutter's generated Windows plugin
registrant/CMake list; those generated entries are reviewed but the DRC gateway
still fails closed before calling the plugin on desktop. The gateway maps plugin
states and errors into the existing DRC-owned result contract.
...
RT-2e-a rereads the exact accepted RT-2d Flutter, platform, permission, capture,
dependency, and test surfaces before any recorder dependency is added. The current
app uses Dart `^3.11.5`; therefore the `record` 7.x line, which requires Dart
3.12 or newer, is not selected. `record` 6.2.1 is selected for the later
RT-2e-b implementation because it is the final pre-7 line and supports the
current SDK baseline.

The later adapter must use file-mode recording behind a private temporary
artifact boundary. `record.startStream` exposes raw audio bytes and is not
allowed for RT-2e. The file path returned by `stop` must remain private; DRC
public results continue to expose only an opaque capture identifier and safe
metadata. Permission checks and requests remain owned by the existing
`permission_handler` gateway rather than the recorder package.

This checkpoint is docs/test-only. It adds no `record` or `path_provider` direct
dependency, generated plugin registration, platform file change, Flutter runtime
code, UI wiring, permission request, microphone access, audio capture, file
creation, upload, Framework/provider call, or STT execution. Acceptance
...
RT-2e-c1 rereads the accepted `5a7f814` Flutter capture, permission, adapter,
startup, and voice-input UI surfaces and fixes the exact real-device operator
flow before adding executable harness code. The later harness must use a
separate `main_rt2ec_operator.dart` entrypoint, require
`--dart-define=DRC_RT2EC_OPERATOR=true`, require an in-app acknowledgement,
and keep the normal `main.dart` and `HomeScreen` unchanged.

The planned operator flow separates permission check, permission request, start,
stop, and cancel into explicit user actions. Capture is capped at 15 seconds,
uses the accepted WAV 16 kHz mono file-mode adapter, automatically discards a
completed private artifact through its opaque id, and exposes only allowlisted
safe evidence. Private paths, raw bytes, audio content, transcript content,
Backend upload, Framework/provider calls, and STT are forbidden.

This checkpoint changed documentation and a source/surface gate only. It added no
Flutter runtime, dependency, platform declaration, generated plugin, UI, or
Backend change and performed no permission request, microphone access, or audio
...
Daily Rhythm Companion v2.1.0 is the current Public release. Daily Rhythm Companion v2.0.0 remains the immutable capability baseline, and v2.0.1 remains the immutable maintenance release baseline.
...
Fixed release ZIP: DailyRhythmCompanion_v2.1.0_20260725_160036.zip
Fixed release ZIP size: 1747337 bytes
Fixed release ZIP SHA-256: 55bf584592b1824948ec847205132582a436f2c521feb593bac914a4904074e5
Post-publication SHA-256 re-verification: completed
```

Historical v2.0.1 patch release:
...
- [`docs/v210_release_readiness.md`](docs/v210_release_readiness.md)
...
W-4b and W-4 are completed and accepted. The detailed Flutter contract is [`docs/v210_flutter_sleep_provider_source_ui.md`](docs/v210_flutter_sleep_provider_source_ui.md). W-5, C-1, T-1, and V-1 are completed and accepted; R-1a is completed/accepted; R-1b is completed/accepted at implementation commit `72dd42c`; R-1c and R-1d are completed/accepted; R-1e and parent R-1 are completed/accepted; v2.1.0 is released.
...
V-1b is completed and accepted at implementation commit `e1f8d6f`. It adds the app-owned `CharacterDisplayPresentation` content/activity model, the standalone static `CharacterDisplayCard`, nine model tests, and four widget tests. Content precedence is character unavailable → asset unavailable → framework fallback → advice → mood; activity precedence is speaking → loading → idle. Acceptance passed after compileall, all v2.1.0 checks, v2.0.x guards, 110 Backend tests, 9 focused model tests, 4 focused widget tests, 97 full Flutter tests, diff review, and explicit operator approval. HomeScreen, Backend, Motion Demo, dependencies, and static assets remained unchanged. V-1c and parent V-1 are completed/accepted; R-1a is completed/accepted; R-1b is completed/accepted at implementation commit `72dd42c`; R-1c and R-1d are completed/accepted; R-1e and parent R-1 are completed/accepted; v2.1.0 is released. Detailed contract: [`docs/v210_character_display_state_contract.md`](docs/v210_character_display_state_contract.md).

V-1c and parent V-1 are completed and accepted at implementation commit `995145d`. The accepted scope connects the presentation model/card to HomeScreen mood, advice, initial/advice/TTS loading, and in-app playback speaking state; adds repository fallback-image retry and unknown-ID fallback; updates the existing character-choice regression; and adds five focused HomeScreen integration tests plus one fallback-image widget test. Acceptance passed with all v2.1.0 checks, v2.0.x guards, 110 Backend tests, 9 model tests, 5 card tests, 5 HomeScreen integration tests, 103 full Flutter tests, Web/Windows builds, diff review, and operator approval. Backend, Motion Demo, dependencies, static assets, real motion execution, and release records remained unchanged. R-1a is completed/accepted; R-1b is completed/accepted at implementation commit `72dd42c`; R-1c and R-1d are completed/accepted; R-1e and parent R-1 are completed/accepted; v2.1.0 is released. Detailed contract: [`docs/v210_character_display_home_integration.md`](docs/v210_character_display_home_integration.md).

R-1a is completed and accepted at implementation commit `dbc84db`. It freezes the reusable generic package builder and ZIP hygiene checker, historical v2.0.1 one-time release boundary, current 2.0.1 application metadata, accepted 110 Backend / 103 Flutter baseline, prior smartphone Web evidence, and the missing v2.1.0 release files without changing runtime or publication state. Acceptance passed on 2026-07-25 after compileall, all 18 v2.1.0 source-tree checks, v2.0.x compatibility and maintenance guards, 110 Backend tests, 103 Flutter tests, diff review, and explicit operator approval. No version metadata, aggregate gate, fixed ZIP, tag, GitHub Release, or historical release record changed. R-1b is completed/accepted at implementation commit `72dd42c`; R-1c and R-1d are completed/accepted; R-1e and parent R-1 are completed/accepted; v2.1.0 is released.

R-1b is completed and accepted at implementation commit `72dd42c`. It updates the active source metadata to Backend `2.1.0` and Flutter `2.1.0+3`, adds the credential-free aggregate readiness gate, and prepares candidate release notes plus an intentionally unfilled release record. Acceptance passed on 2026-07-25 with all 18 accepted v2.1.0 child checks, the v2.0.x compatibility gate, 110 Backend tests, 103 Flutter tests, Web build, Windows build, unchanged `backend/local_data`/`release`, diff review, and explicit operator approval. It did not build a ZIP, create `DRC_v2.1.0`, publish a GitHub Release, execute providers, or complete final smartphone Web evidence. R-1c and R-1d are completed/accepted; R-1e and parent R-1 are completed/accepted; v2.1.0 is released. Detailed gate: [`docs/v210_release_readiness.md`](docs/v210_release_readiness.md).
...
The accepted C-1c mock-safe gate is:
...
This gate is mock-safe. It does not configure or call a real AI Character Framework checkout, LLM provider, wearable provider, TTS/STT runtime, or release workflow. C-1c and parent C-1 are completed and accepted; T-1 remains current and not completed.
...
Run the accepted W-5a public-safe gate with:
...
## v2.1.0 R-1d accepted fixed-ZIP boundary
...
DRC_v2.1.0 tag at R-1d acceptance: not created
GitHub Release at R-1d acceptance: not created
```

R-1d and R-1e are `COMPLETED / ACCEPTED`. The fixed ZIP remains ignored under `release/` and must not be renamed, rebuilt, overwritten, or modified. The annotated tag and published asset match the accepted tuple, and post-publication verification passed.

## Framework v5.2.0 adoption checkpoint

AI Character Framework v5.2.0 releases provider-neutral public Voice Input,
Realtime, interrupt/output-control, and Motion session contracts. RT-1a records:

```text
RT1_MOCK_CONTRACT_INTEGRATION_AUTHORIZED
real Framework STT/realtime/cancel/TTS-queue/Live2D execution: not implemented
...
## v3.0.0 RT-3b app-owned host-audio handoff lifecycle

RT-3b adds a Flutter-owned lifecycle between a completed microphone capture and a future private Backend staging consumer. It retains one opaque private artifact, validates WAV/16 kHz/mono metadata and the 15-second bound, allows only scoped app-internal private-path access during consumer execution, and discards the artifact on completion, failure, cancel, explicit discard, or close. Public results never contain the opaque capture ID or private path.

This checkpoint is fake-only. It does not change the accepted RT-2 operator path, Backend runtime, routes, dependencies, platform files, or vendor FW. It performs no network upload, audio read, Framework import, provider execution, or STT. Acceptance state: **COMPLETED / ACCEPTED** after the source gate, Backend 116 with one existing warning, clean Flutter analysis, focused Flutter 21, full Flutter 192, exact ten-file review, cleanup-retry test correction, and `git diff --check` passed. RT-3c is now CURRENT / NOT_COMPLETED and authorized only for private Backend staging plus a fake FW public-session handoff.


## v3.0.0 RT-3c1 private staging and fake FW handoff readiness

RT-3c1 inspects the exact accepted `cf734aa04990aa55ccfcd56b65052fbe206f74fb`
tree and the vendored FW v5.3.0 public voice-input surface before any audio is
uploaded or staged. The current Flutter handoff contract already grants a
consumer scoped private-path access and cleans the mobile artifact after the
consumer completes. The current Backend voice-input route remains metadata-only,
has no staging store, no upload route, no bounded voice-input staging
configuration, and no FW voice-input adapter.

The selected later transport is a bounded streamed `audio/wav` request body, not
multipart. This avoids adding `python-multipart`; server-side staging will use a
...
`git diff --check` passed. RT-3c3 is also COMPLETED / ACCEPTED after compileall, five RT-3 gates, focused Backend 21, full Backend 137 with one existing warning, clean Flutter analysis, focused Flutter 29, full Flutter 200, exact 22-file surface review, and `git diff --check` passed. RT-3c4 and parent RT-3c are COMPLETED / ACCEPTED after compileall, six RT-3 gates, focused Backend 8, full Backend 145 with one existing warning, clean Flutter analysis, full Flutter 200, exact 22-file surface review, `git diff --check`, and explicit operator approval.


## v3.0.0 RT-3c3 guarded upload and Flutter scoped staging consumer

RT-3c3 is **COMPLETED / ACCEPTED**. Implementation: **COMPLETED / ACCEPTED**. Authorization at implementation: `authorized-guarded-binary-upload-route-and-flutter-scoped-staging-consumer-only`.

Implemented boundary:

- guarded `POST /demo/voice-input/staging` with explicit feature/engine/adapter guards;
- streamed `audio/wav` body into the accepted bounded private staging store;
- required safe WAV metadata headers for 16 kHz, mono, and at most 15000 ms;
- path-free success/error responses and opaque 32-character staging IDs;
- Flutter `BackendVoiceInputStagingConsumer` that opens the local artifact only inside `withPrivateArtifactPath(...)`, streams with `http.StreamedRequest`, and leaves a path-free staged-artifact handle for RT-3c4;
- local mobile artifact cleanup remains owned by the accepted RT-3b controller.

Synthetic tests cover route guards, size/header validation, partial cleanup, streamed client transfer, safe error normalization, local cleanup, and single-handle ownership. No real microphone artifact was uploaded. Framework was not imported, no `VoiceInputSession` was created, and no provider or STT execution occurred. Acceptance passed with compileall, five RT-3 gates, focused Backend 21, full Backend 137 with one existing warning, clean Flutter analysis, focused Flutter 29, full Flutter 200, exact 22-file surface review, and `git diff --check`.
...
RT-3d2b is **COMPLETED / ACCEPTED**. It keeps the RT-3c4 session
fake-handoff intact and adds a separate guarded
`/openai-fake-executor` path. The new path consumes one private staged WAV,
uses FW v5.4.0 public execution configuration, OpenAI adapter, nominal fake
client, bounded fake executor, typed path-free result, and single-use cleanup.
Real credentials, SDK/client creation, network execution, and real STT remain
absent.

RT-3d2b operator approval: ACCEPTED. The accepted implementation commit is
`044f978240b1abda3d28206093e25c4ce285906d`. RT-3d2c is authorized but not
started. Credential values, OpenAI SDK/client creation, network execution,
microphone access, and real STT remain absent.
...
-> OpenAIVoiceInputRealClientFactory (constructed, not invoked)
-> OpenAIVoiceInputProviderAdapter
-> OpenAIVoiceInputRealProviderExecutor (constructed, not executed)
```

Every operator and provider execution gate must be true before Framework import
or private credential-object construction. DRC does not read the credential
value. Normal tests use a synthetic public contract and verify that the client
factory and executor are never called.

RT-3d2c adds no API route, AppConfig/environment credential flag, staging
consume, audio source/read, microphone access, OpenAI dependency, provider
...
## v3.0.0 RT-5f2 fake-only integrated voice turn

RT-5f2 is **COMPLETED / ACCEPTED / PUSHED**.

```text
original implementation baseline: 1cba847b7c443c4d41a2ff6bd2c18d20689e5029
...
The accepted fake/in-memory coordinator composes capture, staging,
provider-neutral transcript handoff, text streaming, RT-5c queue processing,
fake synthesis, fake terminal playback, and DRC-local soft interruption.

Exact review found and corrected queue ownership. Voice output must be empty and
idle before capture, before terminal enqueue, and after synchronous
voice-output phase listeners return. The processed item must match the current
enqueue item's `itemId` and `generation`.
...
exact surfaces and privacy/fake-only review: passed
git diff --check: passed
implementation and corrective pushes: completed
post-push DRC/FW working trees: clean
```

No real microphone, real STT, provider/network execution, production speech
activity, real synthesis/playback, HomeScreen/main wiring, provider hard
cancel, or FW real queue flush is claimed.
...
## v3.0.0 RT-5f3 default-off HomeScreen and production speech activity

RT-5f3 is **COMPLETED / ACCEPTED / PUSHED**.
...
The accepted implementation adds a default-off configured integrated
voice-turn factory, session-local HomeScreen controls, a dedicated non-shared
stream/TTS ownership graph, and a bounded `record: 6.2.1` speech-activity
detector. PCM stream bytes are drained and dropped; only dBFS samples enter a
three-consecutive-sample, one-event-per-generation state machine. The detector
is disarmed during initial capture and may arm only during staging, transcript
acquisition, streaming, or voice output while the app is foreground and opted
in.
...
HomeScreen remains metadata-only. RT-5f3 performed no real microphone,
network, provider, STT, synthesis, playback, audible interruption, threshold
quality, echo-cancellation, or operator acceptance run. Provider hard cancel,
FW real queue flush, and release readiness remain unclaimed.
...
## v3.0.0 RT-5f4 configured local end-to-end and audible soft-barge-in acceptance
...
RT-6a is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`cbcb218aa54d286da7515a01e899121b22d8f3fc`. The exact seven-file docs/static-gate checkpoint was accepted after
its dedicated gate, Backend full regression, Flutter analyze/full regression,
exact-surface review, changed-content privacy review, explicit approval, push,
and clean-tree verification passed.
...
The accepted inventory remains conservative: the Backend motion-demo is
metadata-only and does not send motion; Flutter has static
`idle` / `loading` / `speaking` presentation only; FW v5.4.0 exposes a
root-public mock-safe motion contract; and real Live2D/VTS adapter execution
remains unavailable. RT-6a authorizes no runtime implementation.

RT-6b may now enter a separate exact contract review for a pure app-owned,
provider-neutral, fake-only mapping layer. RT-6b is not implemented or
authorized by this acceptance sync. RT-6c through RT-6f also remain not
authorized, and RT-7 remains blocked.

This acceptance sync changes only the same seven documentation/static-gate
files. It performs no network/provider/VTS/Live2D/audio/STT/LLM/TTS execution
and records no private credential, path, transcript, audio, screenshot, raw
log, provider payload, or operator evidence.
...
## v3.0.0 RT-6b provider-neutral character-motion mapping

RT-6b is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`17f0c46eb0b4e26e2fdf5ffd4090c15c69f4e594`. It adds the exact ten-file app-owned bounded model, deterministic
stateless mapper, focused Backend regression, documentation, and contract gate.
...
Flutter analyze: No issues found
Flutter full: 411 passed
FW import/execution: none
network/provider/VTS/Live2D execution: none
...
The accepted mapper converts bounded app-owned lifecycle facts into zero-to-three
provider-neutral commands. `motion_active` prevents recursive planning,
`unknown` fails closed as ignored, and only `speaking` may set
`speaking=true`. Existing motion-demo routes, Realtime models, API routes,
Flutter runtime, Framework source, dependencies, and release metadata remain
unchanged.

The three Backend warnings are installed dependency deprecations and did not
fail the suite. This acceptance sync changes only seven documentation/static-gate
files and performs no Framework, network, provider, VTS, Live2D, microphone,
audio, STT, LLM, or TTS execution.
...
## v3.0.0 RT-6c FW root-public mock motion-session adapter

RT-6c is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`f929e8faa65a817f1ba4fed82b729438b73dbfab`. The accepted implementation is the exact ten-file default-off
Backend adapter from RT-6b motion plans to the FW root-public mock
motion-session contract.
...
The accepted adapter imports only the FW root module dynamically, forces
`adapter=mock`, `real_adapter_enabled=false`, and
`allow_provider_execution=false`, applies at most three ordered commands,
fails fast, and closes each owned session. Disabled and ignored plans stop
before FW import. Results retain only bounded DRC-owned normalized fields and
at most twelve event type strings; raw FW objects, identifiers, metadata,
exceptions, paths, credentials, payloads, and evidence are not retained.

The local Framework source remains an external vendored snapshot outside DRC
Git history. This small-commit record identifies FW v5.4.0 and canonical
reference commit `d313eb6acb643103fe25988720ebee5976a04f78`, while acceptance verifies the required root-public
contract and local mock execution rather than vendor Git HEAD or clean status.
No API route, configuration enablement, normal runtime caller, Flutter source,
Framework source, dependency, network connection, provider call, VTS
connection, or Live2D runtime is added.

The three Backend warnings are installed dependency deprecations and did not
fail the suite. This acceptance-state sync changes only the same seven
documentation/static-gate files; accepted Backend runtime and focused tests
remain unchanged.
...
## v3.0.0 RT-6d Flutter motion presentation

RT-6d is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`0f220b792feb7ebb82c5871a794731aa1327439a` against baseline `cd423fa2236ce16a7635f0c67460f2fa2fd210e9`.
It adds Flutter-only immutable motion-presentation models, an injected
fake/in-memory client, a single-active-request `ChangeNotifier` controller,
and focused tests.
...
Backend/HTTP/FW/provider/network/VTS/Live2D execution: false
Framework baseline version: 5.4.0
Framework reference commit: d313eb6acb643103fe25988720ebee5976a04f78
Framework source mode: external-vendored-snapshot
...
The accepted client consumes only an injected transport and strictly decodes
DRC-owned RT-6c normalized mock results. The controller rejects simultaneous
requests and ignores stale completions after reset, close, or dispose. It
retains no raw transport exception, response map, arbitrary metadata, path,
credential, or provider payload.

RT-6d changed no HomeScreen, `main.dart`, static character-display model,
motion-demo model/client, Backend route/runtime, dependency, platform file,
asset, Framework source, vendor source, version, or release record. The
acceptance-state sync changes only the seven documentation/static-gate files;
the accepted Flutter runtime and focused tests remain unchanged.
...
focused Flutter: 41 passed
Flutter full: 452 passed
exact twelve-file implementation review: PASS
...
## v3.0.0 RT-6e HomeScreen character-motion presentation

RT-6e is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`13343017738d0bb5fe23583467856233d62196fb` against baseline `8d69b539e974ba71fde5d9b15dd951d0c670b7ff` under the exact ten-file contract.

```text
implementation surface: exact 10 files
acceptance-sync surface: exact 7 documentation/static-gate files
Flutter runtime: 2 files
Flutter focused tests: 1 file
focused Flutter: 16 passed
Flutter full: 468 passed
Backend full: 279 passed, 3 dependency warnings
Dart format: passed
Flutter analyze: passed
normal main.dart configuration: unchanged / unconfigured
opt-in default: off / session-local / not persisted
explicit apply transport limit: 1
HTTP/FW/provider/network/VTS/Live2D execution: false
Framework baseline version: 5.4.0
Framework reference commit: d313eb6acb643103fe25988720ebee5976a04f78
Framework source mode: external-vendored-snapshot
...
The accepted HomeScreen integration owns an optional injected controller,
starts default-off, requires explicit opt-in plus one explicit apply action,
and displays only bounded normalized mock-motion aggregate state. Opt-in,
initial loading, character selection, advice, streaming, voice, TTS,
interruption, and barge-in perform zero automatic motion requests. Reset and
opt-out are local-only and invalidate stale completions.

The existing static character image remains static. RT-6e does not configure
`main.dart`, add Backend/HTTP transport, execute Framework/provider/network
work, connect VTS/Live2D, display raw IDs/commands/events/responses/exceptions,
or claim visible animation acceptance.
...
exact ten-file implementation review: PASS
changed-content privacy review: PASS
CRLF-aware git diff --check: PASS
explicit commit approval: ACCEPTED
implementation commit/push: COMPLETED
...
## v3.0.0 RT-7a real-motion adapter readiness acceptance

RT-7a is **COMPLETED / ACCEPTED / PUSHED** at implementation commit
`efb139b2c0b6c7cc66912a229bd674b36df82dd7` against DRC baseline `c3c78316fd2bcd4f9939dcaadc32134a704374cf` under the exact seven-file
documentation/static-gate contract. The implementation commit is present on
`origin/main`.

RT-7a freezes the accepted RT-6 mock-motion path, records the released
Framework v5.4.0 real-motion limitation, and fixes the minimum root-public
real-adapter contract required before DRC runtime work may begin.
