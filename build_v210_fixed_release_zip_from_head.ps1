...
try {
    ...
    if ($LASTEXITCODE -ne 0 -or $branchName -ne "main") {
        throw "The official v2.1.0 fixed ZIP must be built from Public main."
    }
    ...
    $releaseTagOutput = @(& git tag --list "DRC_v2.1.0")
    if ($LASTEXITCODE -ne 0) {
        throw "Unable to inspect the v2.1.0 release tag."
    }
    $releaseTags = @($releaseTagOutput | Where-Object { $_ })
    if ($releaseTags.Count -gt 0) {
        throw "DRC_v2.1.0 already exists. Refusing to build another fixed v2.1.0 artifact."
    }
    ...
    if ($outputFullDirectory -ne $expectedOutputDirectory) {
        throw "The v2.1.0 fixed release ZIP must be written to the repository release directory."
    }
    ...
    if ($existingV210Zips.Count -gt 0) {
        throw "A v2.1.0 fixed ZIP already exists in release/. Refusing to build or overwrite another one."
    }

    $env:PYTHONDONTWRITEBYTECODE = "1"

    Write-Host "[Preflight] Running the strict R-1d committed-source gate..."
    ...
    $fixedBasename = "DailyRhythmCompanion_v2.1.0_$($match.Groups[1].Value).zip"
    ...
    Write-Host ""
    Write-Host "========================================"
    Write-Host "v210_fixed_release_zip_build_status: built-once-from-clean-public-main"
    Write-Host "v210_fixed_release_zip_public_repository: murayan1982/daily-rhythm-companion-public"
    Write-Host "v210_fixed_release_zip_source_branch: $branchName"
    Write-Host "v210_fixed_release_zip_source_head: $headCommit"
    Write-Host "v210_fixed_release_zip_origin_main_head: $originMain"
    Write-Host "v210_fixed_release_zip_public_root_commit_count: $($rootCommits.Count)"
    Write-Host "v210_fixed_release_zip_build_invocation_count: $buildInvocationCount"
    Write-Host "v210_fixed_release_zip_path: $relativeZipPath"
    Write-Host "v210_fixed_release_zip_basename: $fixedBasename"
    Write-Host "v210_fixed_release_zip_file_size_bytes: $($destinationFile.Length)"
    Write-Host "v210_fixed_release_zip_sha256: $sha256"
    Write-Host "v210_fixed_release_zip_verification_status: not-run"
    Write-Host "v210_fixed_release_zip_tag_created: False"
    Write-Host "v210_fixed_release_zip_github_release_created: False"
    Write-Host "v210_fixed_release_zip_next_action: verify-this-same-zip-without-rebuilding"
    Write-Host "========================================"
} finally {
