...
EXPECTED_TAG_OBJECT = "67600ee31c951efe34ab1c851f71c375b83c760b"
EXPECTED_TAG_TARGET = "4b2920a5225fd5a6141ecf8ff4125615e2ec0ef2"
CONTROL_E_R2_COMMIT = "c2b7ab96685025bcceebe069fb523645d3f12901"
...
STAGE4_BASETEMP_CURRENT_HEADING = "## Control E Release Publication Result Sync Corrective R5 Current Section"
STAGE4_BASETEMP_HEADING_FAMILY_PREFIX = "controlereleasepublicationresultsync"
LEGACY_CONTROL_E_HEADING_FAMILY_PREFIX = "controlereleaseauthorizationsync"
...
REQUIRED_CURRENT_PURPOSE_PHRASES = (
    ...
    "Stage 4 retry is CONSUMED / FAILED",
    "Control E publication completed exactly once and its authorization is consumed",
    "DRC v4.0.0 is RELEASED",
...
REQUIRED_CURRENT_STAGE4_REVIEW_PHRASES = (
    ...
    "Control E publication completed exactly once and its authorization is consumed",
    "tag/publication are completed and verified",
    "DRC v4.0.0 is released",
)
...
def validate_stage4_retry_authorization_sync_committed_surface(
    commit_count: int,
    name_status_lines: list[str],
) -> bool:
...
def validate_stage4_invocation_3_authorization_sync_committed_surface(
    commit_count: int,
    name_status_lines: list[str],
) -> bool:
    return validate_exact_committed_surface(
        commit_count,
        name_status_lines,
        STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_MODIFIED,
    )
...
def stage4_retry_authorization_sync_origin_state(head: str, origin: str) -> str | None:
    if not origin:
        return None
    if origin == CONTROL_D_STAGE4_VERIFIER_OUTPUT_ENCODING_CORRECTIVE_COMMIT and head != origin:
        return "NOT_PUSHED"
    if origin == head and head != CONTROL_D_STAGE4_VERIFIER_OUTPUT_ENCODING_CORRECTIVE_COMMIT:
        return "PUSHED"
    return None


def stage4_retry_authorization_sync_clean_mode_after_surface_validation(
    surface_validated: bool,
    head: str,
    origin: str,
) -> str:
    if not surface_validated:
        raise AssertionError("Stage 4 retry authorization-sync origin policy reached before committed surface validation")
    state = stage4_retry_authorization_sync_origin_state(head, origin)
    if state == "NOT_PUSHED":
        return "CLEAN_COMMITTED_STAGE4_RETRY_AUTHORIZATION_SYNC_NOT_PUSHED"
    if state == "PUSHED":
        return "CLEAN_COMMITTED_STAGE4_RETRY_AUTHORIZATION_SYNC"
    raise AssertionError("Clean Stage 4 retry authorization-sync origin/main state is invalid")
...
def stage4_verifier_pytest_basetemp_isolation_corrective_clean_mode_after_surface_validation(
    ...
    if not surface_validated:
        raise AssertionError("Stage 4 verifier pytest basetemp isolation corrective origin policy reached before committed surface validation")
    ...
    raise AssertionError("Clean Stage 4 verifier pytest basetemp isolation corrective origin/main state is invalid")
...
R3_RT9C_LINES = ("DRC_v3.0.0 annotated tag: NOT_CREATED", "GitHub Release: NOT_CREATED")


def exact_line_count(text: str, line: str) -> int:
    return sum(1 for candidate in norm(text).splitlines() if candidate == line)


def text_has_exact_line(text: str, line: str) -> bool:
    return exact_line_count(text, line) == 1


def protected_line_family_is_exact(text: str, canonical_line: str) -> bool:
    family = [line for line in norm(text).splitlines() if canonical_line in line]
    return family == [canonical_line]


def protected_line_family_has_no_shadow(text: str, canonical_line: str) -> bool:
    return all(line == canonical_line for line in norm(text).splitlines() if canonical_line in line)


def control_e_r3_historical_and_terminal_contract(doc_texts: dict[str, str]) -> bool:
    if set(doc_texts) != set(CURRENT_DOCS):
        return False
    for relative in R3_V3_HISTORY_DOCS:
        text = norm(doc_texts[relative])
        if not all(protected_line_family_has_no_shadow(text, line) for line in set((*R3_RT9B_LINES, *R3_RT9C_LINES))):
            return False
        b = extract_bounded_section(text, "<!-- RT-9B-RELEASE-READINESS:BEGIN -->", "<!-- RT-9B-RELEASE-READINESS:END -->")
        c = extract_bounded_section(text, "<!-- RT-9C-STAGE1-FIXED-ZIP-TOOLING:BEGIN -->", "<!-- RT-9C-STAGE1-FIXED-ZIP-TOOLING:END -->")
        if b is None or c is None or not all(protected_line_family_is_exact(b, line) for line in R3_RT9B_LINES) or not all(protected_line_family_is_exact(c, line) for line in R3_RT9C_LINES):
            return False
        bounded = b + "\n" + c
        if any(value in bounded for value in ("DRC_v4.0.0", "DailyRhythmCompanion_v4.0.0", "RELEASED / PUBLISHED / VERIFIED", "PUBLISHED / NON_DRAFT / NON_PRERELEASE")):
            return False
    return all(protected_line_family_is_exact(text, R3_CURRENT_TERMINAL_NOTE) and not text_has_exact_line(text, R3_STALE_TERMINAL_NOTE) for text in doc_texts.values())


def control_e_r3_historical_and_terminal_self_check() -> dict[str, bool]:
    docs = control_e_canonical_doc_fixture()
    history = "\n".join(("<!-- RT-9B-RELEASE-READINESS:BEGIN -->", *R3_RT9B_LINES, "<!-- RT-9B-RELEASE-READINESS:END -->", "<!-- RT-9C-STAGE1-FIXED-ZIP-TOOLING:BEGIN -->", *R3_RT9C_LINES, "<!-- RT-9C-STAGE1-FIXED-ZIP-TOOLING:END -->"))
    docs = dict(docs)
    first = R3_V3_HISTORY_DOCS[0]
    mutations = {
        "tag_suffix_extra": docs[first].replace("DRC_v3.0.0 annotated tag: NOT_CREATED", "DRC_v3.0.0 annotated tag: NOT_CREATED_EXTRA", 1),
        "tag_trailing_extra": docs[first].replace("DRC_v3.0.0 annotated tag: NOT_CREATED", "DRC_v3.0.0 annotated tag: NOT_CREATED extra", 1),
        "release_suffix_extra": docs[first].replace("GitHub Release: NOT_CREATED", "GitHub Release: NOT_CREATED_EXTRA", 1),
        "release_trailing_extra": docs[first].replace("GitHub Release: NOT_CREATED", "GitHub Release: NOT_CREATED extra", 1),
        "notes_suffix_extra": docs[first].replace("v3.0.0 release notes: RELEASE CANDIDATE / NOT_RELEASED", "v3.0.0 release notes: RELEASE CANDIDATE / NOT_RELEASED_EXTRA", 1),
        "record_suffix_extra": docs[first].replace("v3.0.0 release record: PREPARED / NOT_RELEASED", "v3.0.0 release record: PREPARED / NOT_RELEASED_EXTRA", 1),
        "terminal_suffix_extra": docs[first].replace(R3_CURRENT_TERMINAL_NOTE, R3_CURRENT_TERMINAL_NOTE + " EXTRA", 1),
        "terminal_prefix": docs[first].replace(R3_CURRENT_TERMINAL_NOTE, "PREFIX " + R3_CURRENT_TERMINAL_NOTE, 1),
        "terminal_bak": docs[first].replace(R3_CURRENT_TERMINAL_NOTE, R3_CURRENT_TERMINAL_NOTE + ".bak", 1),
        "terminal_trailing": docs[first].replace(R3_CURRENT_TERMINAL_NOTE, R3_CURRENT_TERMINAL_NOTE + " trailing text", 1),
        "duplicate": docs[first].replace(R3_RT9B_LINES[0], R3_RT9B_LINES[0] + "\n" + R3_RT9B_LINES[0], 1),
        "duplicate_terminal": docs[first].replace(R3_CURRENT_TERMINAL_NOTE, R3_CURRENT_TERMINAL_NOTE + "\n" + R3_CURRENT_TERMINAL_NOTE, 1),
        "case": docs[first].replace("NOT_CREATED", "not_created", 1),
        "nfkc": docs[first].replace("NOT_CREATED", "NOT＿CREATED", 1),
        "whitespace": docs[first].replace("NOT_CREATED", "NOT\tCREATED", 1),
        "wrong_version": docs[first].replace("v3.0.0 release notes", "v4.0.0 release notes", 1),
        "wrong_tag": docs[first].replace("DRC_v3.0.0 annotated tag: NOT_CREATED", "DRC_v4.0.0 annotated tag: NOT_CREATED", 1),
        "v4_tuple": docs[first].replace("DRC_v3.0.0 annotated tag: NOT_CREATED", "DRC_v3.0.0 annotated tag: DRC_v4.0.0 / ANNOTATED / PUSHED", 1),
        "stale_terminal": docs[first].replace(R3_CURRENT_TERMINAL_NOTE, R3_STALE_TERMINAL_NOTE, 1),

        "additive_terminal_suffix_shadow": docs[first] + "\n" + R3_CURRENT_TERMINAL_NOTE + "_EXTRA\n",
        "additive_terminal_prefix_shadow": docs[first] + "\nPREFIX " + R3_CURRENT_TERMINAL_NOTE + "\n",
        "additive_terminal_trailing_shadow": docs[first] + "\n" + R3_CURRENT_TERMINAL_NOTE + " trailing text\n",
    }
    historical_lines = tuple(dict.fromkeys((*R3_RT9B_LINES, *R3_RT9C_LINES)))
    for index, canonical_line in enumerate(historical_lines):
        mutations[f"additive_historical_{index}_suffix_shadow"] = docs[first] + "\n" + canonical_line + "_EXTRA\n"
        mutations[f"additive_historical_{index}_prefix_shadow"] = docs[first] + "\nPREFIX " + canonical_line + "\n"
        mutations[f"additive_historical_{index}_trailing_shadow"] = docs[first] + "\n" + canonical_line + " trailing text\n"
    rejected = {name: not control_e_r3_historical_and_terminal_contract({**docs, first: value}) for name, value in mutations.items()}
    return {"canonical_accepted": control_e_r3_historical_and_terminal_contract(docs), "fixtures_distinct": len(mutations) == len(set(mutations.values())), **{name + "_rejected": value for name, value in rejected.items()}}
...
def protocol_current_status_is_correct(text: str) -> bool:
    ...
    required = (
        ...
        ("Cumulative completed Stage 4 verifier invocations", "3"),
        ("Historical retry budget remaining", "0"),
        ("Stage 4 invocation 3", "EXACTLY_ONCE_EXECUTED / PASS / REVIEWED / ACCEPTED"),
        ("Stage 4 invocation 3 process exit code", "0"),
        ("Stage 4 invocation 3 Backend result", "479 passed / 1 warning"),
        ("Stage 4 invocation 3 scanner result", "EXACT_SOURCE_MATCHED_SYNTHETIC_FIXTURES / ACCEPTED"),
        ("Stage 4 invocation 3 same artifact verified", "TRUE"),
...
def check_stage4_content_review_guards() -> None:
    ...
    if not tasklist_current_implementation_step_is_current(read("tasklist.md")):
        raise AssertionError("tasklist current implementation step is not Stage 4 Invocation 3 Pass Result Sync")
...
def check_r5_document_correction_guards() -> None:
    ...
    if "Current checkpoint:\nDRC v4.0.0 Control E Release Publication Result Sync Corrective R5" not in protocol:
        raise AssertionError("protocol current checkpoint is not Stage 4 verifier pytest basetemp isolation corrective R4")
...
def invocation_3_authorization_contract_is_exact(text: str) -> bool:
    return invocation_3_pass_result_contract_is_present(text)


def invocation_3_authorization_contract_is_exact_in_docs() -> bool:
    return invocation_3_pass_result_contract_is_present_for_doc_texts(
...
def invocation_3_pass_result_contract_is_present_for_doc_texts(doc_texts: dict[str, str]) -> bool:
    for relative in CURRENT_DOCS:
        text = doc_texts.get(relative, "")
        sections = stage4_retry_current_sections(text)
        if len(sections) != 1 or not stage4_retry_section_is_valid(sections[0], False):
            return False
        normalized = norm(text)
        if STAGE4_INVOCATION_3_AUTHORIZATION in normalized or STAGE4_RETRY_AUTHORIZATION in normalized:
            return False
    return True


def invocation_3_pass_result_contract_is_present(text: str) -> bool:
    normalized = norm(text)
    sections = stage4_retry_current_sections(normalized)
    return (
        len(sections) == len(CURRENT_DOCS)
        and all(stage4_retry_section_is_valid(section, False) for section in sections)
        and normalized.count(STAGE4_INVOCATION_3_AUTHORIZATION) == 0
        and normalized.count(STAGE4_RETRY_AUTHORIZATION) == 0
        and "Stage 4 invocation 3 result: EXACTLY_ONCE_EXECUTED / PASS / REVIEWED / ACCEPTED" in normalized
        and "Stage 4 invocation 3 process exit code: 0" in normalized
        and "Stage 4 invocation 3 Backend result: 479 passed / 1 warning" in normalized
        and "Stage 4 invocation 3 scanner result: EXACT_SOURCE_MATCHED_SYNTHETIC_FIXTURES / ACCEPTED" in normalized
        and "Stage 4 invocation 3 tooling state: RELEASE_ZIP_VERIFICATION" in normalized
        and "Stage 4 invocation 3 same artifact verified: TRUE" in normalized
        and "Stage 4 invocation 3 builder invoked by verifier: FALSE" in normalized
        and "Stage 4 invocation 3 final marker: [v400-fixed-release-zip-check] OK" in normalized
        and "Cumulative completed Stage 4 verifier invocations: 3" in normalized
        and "Invocation 3 completed invocation count: 1" in normalized
        and "Invocation 3 explicit authorization budget remaining: 0" in normalized
        and "Invocation 3 authorization state: CONSUMED" in normalized
        and "Next authorized invocation number: NOT_AUTHORIZED" in normalized
        ...
        and "Control E: COMPLETED / PASS / ACCEPTED" in normalized
        and "DRC v4.0.0: RELEASED" in normalized
        and "annotated tag: DRC_v4.0.0 / ANNOTATED / PUSHED" in normalized
        and "GitHub Release: PUBLISHED / NON_DRAFT / NON_PRERELEASE" in normalized
    )
...
def consumed_retry_history_contract_is_present_for_doc_texts(doc_texts: dict[str, str]) -> bool:
    return all("Historical Stage 4 retry: CONSUMED / FAILED" in doc_texts.get(relative, "") for relative in CURRENT_DOCS)


def consumed_retry_history_contract_is_present(text: str) -> bool:
    return "Historical Stage 4 retry: CONSUMED / FAILED" in text


def stage4_retry_current_sections(text: str) -> list[str]:
    normalized = norm(text)
    for line in normalized.splitlines():
        if stage4_retry_current_heading_family(line) and line != STAGE4_RETRY_CURRENT_HEADING:
            return []
    matches = list(re.finditer(rf"(?m)^{re.escape(STAGE4_RETRY_CURRENT_HEADING)}\n", normalized))
    sections: list[str] = []
    for match in matches:
        start = match.end()
        next_heading = re.search(r"(?m)^## ", normalized[start:])
        end = start + next_heading.start() if next_heading else len(normalized)
        section = normalized[start:end].strip("\n")
        if STAGE4_RETRY_CURRENT_HEADING in section:
            return []
        sections.append(section)
    return sections
...
def stage4_retry_current_heading_family(line: str) -> bool:
    ...
    return (
        STAGE4_BASETEMP_HEADING_FAMILY_PREFIX in key
        or LEGACY_CONTROL_E_HEADING_FAMILY_PREFIX in key
        or raw_key.startswith(STALE_STAGE4_BASETEMP_HEADING_FAMILY_PREFIX)
    )


def stage4_retry_section_is_valid(section: str, token_required: bool) -> bool:
    lines = section.splitlines()
    expected = list(STAGE4_RETRY_CANONICAL_LINES)
    return lines == expected


def stage4_retry_authorization_contract_is_present_in_docs() -> bool:
    return stage4_retry_authorization_contract_is_present_for_doc_texts(
        {relative: read(relative) for relative in CURRENT_DOCS}
    )


def stage4_retry_authorization_contract_is_present_for_doc_texts(doc_texts: dict[str, str]) -> bool:
    retry_token_count = 0
    invocation_3_token_count = 0
    for relative in CURRENT_DOCS:
        text = doc_texts.get(relative, "")
        sections = stage4_retry_current_sections(text)
        if len(sections) != 1:
            return False
        token_required = relative in STAGE4_RETRY_TOKEN_DOCS
        if not stage4_retry_section_is_valid(sections[0], token_required):
            return False
        normalized = norm(text)
        file_retry_token_count = normalized.count(STAGE4_RETRY_AUTHORIZATION)
        file_invocation_3_token_count = normalized.count(STAGE4_INVOCATION_3_AUTHORIZATION)
        if token_required:
            if file_retry_token_count != 1 or file_invocation_3_token_count != 1:
                return False
            retry_token_count += file_retry_token_count
            invocation_3_token_count += file_invocation_3_token_count
        elif file_retry_token_count != 0 or file_invocation_3_token_count != 0:
            return False
    return retry_token_count == len(STAGE4_RETRY_TOKEN_DOCS) and invocation_3_token_count == len(STAGE4_RETRY_TOKEN_DOCS)


def stage4_retry_authorization_contract_is_present(text: str) -> bool:
    normalized = norm(text)
    sections = stage4_retry_current_sections(normalized)
    if len(sections) != len(CURRENT_DOCS):
        return False
    token_sections = 0
    for section in sections:
        has_token = STAGE4_INVOCATION_3_AUTHORIZATION in section
        if not stage4_retry_section_is_valid(section, has_token):
            return False
        if has_token:
            token_sections += 1
    return (
        token_sections == len(STAGE4_RETRY_TOKEN_DOCS)
        and normalized.count(STAGE4_RETRY_AUTHORIZATION) == len(STAGE4_RETRY_TOKEN_DOCS)
        and normalized.count(STAGE4_INVOCATION_3_AUTHORIZATION) == len(STAGE4_RETRY_TOKEN_DOCS)
        and normalized.count(STAGE4_AUTHORIZATION) == 0
        and "Stage 4 invocation 1:\nEXACTLY_ONCE_EXECUTED / EXECUTION_FAILED" in normalized
        and "verification verdict:\nNOT_REACHED" in normalized
        and "Next authorized invocation number: 3" in normalized
        and "Invocation 3 authorized: TRUE" in normalized
        and "Invocation 3 completed invocation count: 0" in normalized
        and "Invocation 3 explicit authorization budget: 1" in normalized
        and "Invocation 3 explicit authorization budget remaining: 1" in normalized
        and "Invocation 3 execution eligibility: REQUIRES_CLEAN_COMMITTED_PUSHED_AUTHORIZATION_SYNC" in normalized
        and "Stage 4 invocation 3: AUTHORIZED" not in normalized
        and not stage4_zip_verification_completed(normalized)
        and not control_e_is_authorized(normalized)
    )
...
def check_committed_stage4_retry_authorization_sync_surface(head: str = "HEAD") -> None:
    commit_count = int(git_out("rev-list", "--count", f"{CONTROL_D_STAGE4_VERIFIER_OUTPUT_ENCODING_CORRECTIVE_COMMIT}..{head}"))
    lines = git_out("diff", "--name-status", f"{CONTROL_D_STAGE4_VERIFIER_OUTPUT_ENCODING_CORRECTIVE_COMMIT}..{head}").splitlines()
    if not validate_stage4_retry_authorization_sync_committed_surface(commit_count, lines):
        raise AssertionError("Clean committed Stage 4 retry authorization-sync surface is not exact one-commit M12")
    protected = git_out(
        "diff",
        "--name-status",
        f"{CONTROL_D_STAGE4_VERIFIER_OUTPUT_ENCODING_CORRECTIVE_COMMIT}..{head}",
        "--",
        *PROTECTED_PATHS,
    ).splitlines()
    if not validate_empty_protected_delta(protected):
        raise AssertionError("Stage 4 retry authorization-sync protected delta is not empty")


def check_committed_stage4_verifier_pytest_basetemp_isolation_corrective_surface(head: str = "HEAD") -> None:
    commit_count = int(git_out("rev-list", "--count", f"{CONTROL_D_STAGE4_RETRY_AUTHORIZATION_SYNC_CORRECTIVE_COMMIT}..{head}"))
    lines = git_out("diff", "--name-status", f"{CONTROL_D_STAGE4_RETRY_AUTHORIZATION_SYNC_CORRECTIVE_COMMIT}..{head}").splitlines()
    if not validate_stage4_verifier_pytest_basetemp_isolation_corrective_committed_surface(commit_count, lines):
        raise AssertionError("Clean committed Stage 4 verifier pytest basetemp isolation corrective surface is not exact one-commit M12")
    protected = git_out(
        "diff",
        "--name-status",
        f"{CONTROL_D_STAGE4_RETRY_AUTHORIZATION_SYNC_CORRECTIVE_COMMIT}..{head}",
        "--",
        *PROTECTED_PATHS,
    ).splitlines()
    if not validate_empty_protected_delta(protected):
        raise AssertionError("Stage 4 verifier pytest basetemp isolation corrective protected delta is not empty")


def check_committed_stage4_invocation_3_authorization_sync_surface(head: str = "HEAD") -> None:
    revision_range = f"{CONTROL_D_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_COMMIT}..{head}"
    commit_count = int(git_out("rev-list", "--count", revision_range))
    lines = git_out("diff", "--name-status", revision_range).splitlines()
    if not validate_stage4_invocation_3_authorization_sync_committed_surface(commit_count, lines):
        raise AssertionError("Clean committed Stage 4 invocation 3 authorization-sync surface is not exact one-commit M12")
    protected = git_out("diff", "--name-status", revision_range, "--", *PROTECTED_PATHS).splitlines()
    if not validate_empty_protected_delta(protected):
        raise AssertionError("Stage 4 invocation 3 authorization-sync protected delta is not empty")


def check_stage4_invocation_3_authorization_sync_lineage(head: str = "HEAD") -> None:
    subprocess.run(
        [
            "git",
            "merge-base",
            "--is-ancestor",
            CONTROL_D_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_COMMIT,
            head,
        ],
        cwd=ROOT,
        check=True,
    )
    check_committed_stage4_verifier_pytest_basetemp_isolation_corrective_surface(
        CONTROL_D_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_COMMIT
    )
    check_committed_stage4_invocation_3_authorization_sync_surface(head)


def stage4_invocation_3_lineage_guard_self_check() -> dict[str, bool]:
    names = check_stage4_invocation_3_authorization_sync_lineage.__code__.co_names
    consts = check_stage4_invocation_3_authorization_sync_lineage.__code__.co_consts
    determine_names = determine_mode.__code__.co_names
    r4_name = "check_committed_stage4_verifier_pytest_basetemp_isolation_corrective_surface"
    invocation_name = "check_committed_stage4_invocation_3_authorization_sync_surface"
    return {
        "merge_base_ancestor_guard_present": "merge-base" in consts and "--is-ancestor" in consts,
        "r4_basetemp_corrective_guard_call_count_1": names.count(r4_name) == 1,
        "invocation_3_authorization_sync_guard_call_count_1": names.count(invocation_name) == 1,
        "r4_guard_before_invocation_3_guard": names.index(r4_name) < names.index(invocation_name),
        "default_clean_dispatch_uses_lineage_guard": "check_stage4_invocation_3_authorization_sync_lineage" in determine_names,
    }
...
def determine_mode() -> str:
    ...
    if entries:
        ...
        if head != CONTROL_D_STAGE2_PREFLIGHT_GUARD_COMMIT:
            ...
            if head == CONTROL_D_STAGE3_PATH_LENGTH_CORRECTIVE_COMMIT and origin == CONTROL_D_STAGE3_PATH_LENGTH_CORRECTIVE_COMMIT:
                ...
                check_expected_fixed_zip_tuple(expected_tag_published=head_includes_published_control_e_commit())
            ...
            if (
                head == CONTROL_D_STAGE4_VERIFIER_OUTPUT_ENCODING_CORRECTIVE_COMMIT
                and origin == CONTROL_D_STAGE4_VERIFIER_OUTPUT_ENCODING_CORRECTIVE_COMMIT
            ):
            ...
            if (
                head == CONTROL_D_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_COMMIT
                and origin == CONTROL_D_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_COMMIT
            ):
                check_dirty_surface(entries, STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_MODIFIED)
                check_expected_fixed_zip_tuple(expected_tag_published=head_includes_published_control_e_commit())
                return "DIRTY_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_CANDIDATE"
            if head == CONTROL_D_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_COMMIT and origin == CONTROL_D_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_COMMIT:
            ...
            if head in {EXPECTED_TAG_TARGET, CONTROL_E_R2_COMMIT} and origin == head:
                check_dirty_surface(entries, CONTROL_E_RELEASE_AUTHORIZATION_SYNC_MODIFIED)
                check_expected_fixed_zip_tuple(expected_tag_published=True)
                return 'DIRTY_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_CANDIDATE'
    ...
    check_committed_stage4_verifier_output_encoding_corrective_surface(CONTROL_D_STAGE4_VERIFIER_OUTPUT_ENCODING_CORRECTIVE_COMMIT)
    check_expected_fixed_zip_tuple(expected_tag_published=head_includes_published_control_e_commit())
    ...
    if head == CONTROL_D_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_COMMIT:
    ...
    check_control_e_release_publication_result_sync_lineage("HEAD")
    return control_e_release_publication_result_sync_clean_mode_after_surface_validation(True, head, origin)
...
def validate_stage4_invocation_3_pass_result_sync_committed_surface(
    ...
    return validate_exact_committed_surface(
        commit_count,
        name_status_lines,
        STAGE4_INVOCATION_3_PASS_RESULT_SYNC_MODIFIED,
    )


def check_committed_stage4_invocation_3_pass_result_sync_surface(head: str = "HEAD") -> None:
    commit_count = int(git_out("rev-list", "--count", f"{CONTROL_D_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_COMMIT}..{head}"))
    lines = git_out("diff", "--name-status", f"{CONTROL_D_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_COMMIT}..{head}").splitlines()
    if not validate_stage4_invocation_3_pass_result_sync_committed_surface(commit_count, lines):
        die("Stage 4 Invocation 3 pass-result sync committed surface must be exact one-commit M12 A0 D0")
    diff_names = git_out("diff", "--name-only", f"{CONTROL_D_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_COMMIT}..{head}").splitlines()
    protected_delta = [path for path in diff_names if path.split("/", 1)[0] in {"backend", "app", "release"}]
    if protected_delta:
        die("Stage 4 Invocation 3 pass-result sync protected/product delta must be empty")


def check_stage4_invocation_3_pass_result_sync_lineage(head: str = "HEAD") -> None:
    subprocess.run(["git", "merge-base", "--is-ancestor", CONTROL_D_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_COMMIT, head], cwd=ROOT, check=True)
    check_stage4_invocation_3_authorization_sync_lineage(CONTROL_D_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_COMMIT)
    check_committed_stage4_invocation_3_pass_result_sync_surface(head)


def stage4_invocation_3_pass_result_sync_origin_state(head: str, origin: str) -> str | None:
    if origin == CONTROL_D_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_COMMIT and head != origin:
        return "NOT_PUSHED"
    if origin == head and head != CONTROL_D_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_COMMIT:
        return "PUSHED"
    return None


def stage4_invocation_3_pass_result_sync_clean_mode_after_surface_validation(
    ...
    state = stage4_invocation_3_pass_result_sync_origin_state(head, origin)
    if state == "NOT_PUSHED":
        return "CLEAN_COMMITTED_STAGE4_INVOCATION_3_PASS_RESULT_SYNC_NOT_PUSHED"
    if state == "PUSHED":
        return "CLEAN_COMMITTED_STAGE4_INVOCATION_3_PASS_RESULT_SYNC"
...
def control_e_release_authorization_sync_clean_mode_after_surface_validation(
    ...
    if state == "NOT_PUSHED":
        return "CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_NOT_PUSHED"
    if state == "PUSHED":
        return "CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE"
    raise AssertionError("Control E release authorization-sync origin/main state is not accepted")



CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_MODIFIED = CONTROL_E_RELEASE_AUTHORIZATION_SYNC_MODIFIED


def validate_control_e_release_publication_result_sync_committed_surface(
    commit_count: int,
    name_status_lines: list[str],
) -> bool:
    return validate_exact_committed_surface(
        commit_count,
        name_status_lines,
        CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_MODIFIED,
    )


def check_committed_control_e_release_publication_result_sync_surface(head: str = "HEAD") -> None:
    commit_count = int(git_out("rev-list", "--count", f"{CONTROL_E_R2_COMMIT}..{head}"))
    lines = git_out("diff", "--name-status", f"{CONTROL_E_R2_COMMIT}..{head}").splitlines()
    if not validate_control_e_release_publication_result_sync_committed_surface(commit_count, lines):
        raise AssertionError("Control E release publication result-sync committed surface must be exact one-commit M12 A0 D0")
    diff_names = git_out("diff", "--name-only", f"{CONTROL_E_R2_COMMIT}..{head}").splitlines()
    protected_delta = [
        path for path in diff_names
        if path.split("/", 1)[0] in {"backend", "app", "release"} or path.startswith("build_")
    ]
    if protected_delta:
        raise AssertionError("Control E release publication result-sync protected/product delta must be empty")


def check_control_e_r2_accepted_baseline_surface() -> None:
    subprocess.run(["git", "merge-base", "--is-ancestor", EXPECTED_TAG_TARGET, CONTROL_E_R2_COMMIT], cwd=ROOT, check=True)
    count = int(git_out("rev-list", "--count", f"{EXPECTED_TAG_TARGET}..{CONTROL_E_R2_COMMIT}"))
    lines = git_out("diff", "--name-status", f"{EXPECTED_TAG_TARGET}..{CONTROL_E_R2_COMMIT}").splitlines()
    if not validate_exact_committed_surface(count, lines, CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_MODIFIED):
        raise AssertionError("accepted Control E R2 baseline must be exact one-commit M12 A0 D0")

def check_control_e_release_publication_result_sync_lineage(head: str = "HEAD") -> None:
    subprocess.run(["git", "merge-base", "--is-ancestor", CONTROL_E_R2_COMMIT, head], cwd=ROOT, check=True)
    check_control_e_r2_accepted_baseline_surface()
    check_committed_control_e_release_publication_result_sync_surface(head)


def control_e_release_publication_result_sync_origin_state(head: str, origin: str) -> str | None:
    if origin == CONTROL_E_R2_COMMIT and head != origin:
        return "NOT_PUSHED"
    if origin == head and head != CONTROL_E_R2_COMMIT:
        return "PUSHED"
    return None


def control_e_release_publication_result_sync_clean_mode_after_surface_validation(
    surface_valid: bool, head: str, origin: str
) -> str:
    if not surface_valid:
        raise AssertionError("surface must be valid before origin policy")
    state = control_e_release_publication_result_sync_origin_state(head, origin)
    if state == "NOT_PUSHED":
        return "CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_NOT_PUSHED"
    if state == "PUSHED":
        return "CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE"
    raise AssertionError("Control E release publication result-sync origin/main state is not accepted")
...
def control_e_normalized_authorization_text(text: str) -> str:
    folded = unicodedata.normalize("NFKC", text).casefold()
    return "".join(character for character in folded if character.isascii() and character.isalnum())


def control_e_authorization_reinjection_present(doc_texts: dict[str, str]) -> bool:
    signature = CONTROL_E_RELEASE_PUBLICATION_AUTHORIZATION.casefold().replace("_", "")
    return any(signature in control_e_normalized_authorization_text(doc_texts.get(relative, "")) for relative in CURRENT_DOCS)


def control_e_authorization_token_mapping(doc_texts: dict[str, str]) -> bool:
    return set(doc_texts) == set(CURRENT_DOCS) and not control_e_authorization_reinjection_present(doc_texts)
...
def control_e_doc_contract_is_exact(relative: str, text: str) -> bool:
    ...
    if not control_e_current_section_is_exact(normalized):
        return False
    normalized = stage4_retry_current_sections(normalized)[0]
    required = {
        "Control E: COMPLETED / PASS / ACCEPTED",
        "Control E authorization state: CONSUMED",
        "Control E execution count: 1",
    ...
    forbidden = {
        "Control E authorized: TRUE", "Control E authorized: FALSE",
        "Control E authorization state: ACTIVE", "Control E authorization state: AUTHORIZED",
        "Control E authorization state: NOT_CONSUMED",
        "Control E current active authorization: TRUE",
        "Additional Control E execution authorized: TRUE",
        "Additional publication authorized: TRUE",
        "Control E publication reachability: TRUE",
        "Stage 4 release-ZIP reachability: TRUE",
        "annotated tag: NOT_CREATED", "GitHub Release: NOT_CREATED", "DRC v4.0.0: NOT_RELEASED",
        "DRC v4.0.0 remains not released", "v4.0.0 remains NOT_RELEASED",
        "v4.0.0 remains `NOT_RELEASED`", "NOT_RELEASED_EXTRA",
    }
    return all(item in normalized for item in required) and not any(item in normalized for item in forbidden)


def current_released_state_contract(doc_texts: dict[str, str]) -> bool:
    if set(doc_texts) != set(CURRENT_DOCS):
        return False
    for relative, text in doc_texts.items():
        normalized = norm(text)
        bounded_history = ""
        if relative in R3_V3_HISTORY_DOCS:
            sections = [
                extract_bounded_section(normalized, "<!-- RT-9B-RELEASE-READINESS:BEGIN -->", "<!-- RT-9B-RELEASE-READINESS:END -->"),
                extract_bounded_section(normalized, "<!-- RT-9C-STAGE1-FIXED-ZIP-TOOLING:BEGIN -->", "<!-- RT-9C-STAGE1-FIXED-ZIP-TOOLING:END -->"),
            ]
            bounded_history = "\n".join(section or "" for section in sections)
        current_text = normalized.replace(bounded_history, "")
        forbidden_lines = (
            "DRC v4.0.0 remains not released.",
            "DRC v4.0.0 remains NOT_RELEASED.",
            "v4.0.0 remains NOT_RELEASED.",
            "v4.0.0 remains `NOT_RELEASED`.",
            "DRC v4.0.0: NOT_RELEASED",
            "v4.0.0 RELEASE CANDIDATE / NOT_RELEASED",
            "v4.0.0 release notes: RELEASE CANDIDATE / NOT_RELEASED",
            "DRC v4.0.0: NOT_RELEASED_EXTRA",
            "v4.0.0 release notes: RELEASE CANDIDATE / NOT_RELEASED_EXTRA",
        )
        if any(text_has_exact_line(current_text, line) for line in forbidden_lines):
            return False
        if "DRC v4.0.0 remains not released" in current_text:
            return False
        if "v4.0.0 remains NOT_RELEASED" in current_text or "v4.0.0 remains `NOT_RELEASED`" in current_text:
            return False
        if "NOT_RELEASED_EXTRA" in current_text:
            return False
        sections = stage4_retry_current_sections(current_text)
        if len(sections) != 1:
            return False
        if not protected_line_family_has_no_shadow(current_text, "DRC v4.0.0: RELEASED"):
            return False
        if exact_line_count(sections[0], "DRC v4.0.0: RELEASED") != 1:
            return False
    return True


def current_released_state_contract_self_check() -> dict[str, bool]:
    docs = control_e_canonical_doc_fixture()
    first = CURRENT_DOCS[0]
    history_doc = R3_V3_HISTORY_DOCS[0]
    variants = {
        "remains_not_released": {**docs, first: docs[first] + "\nDRC v4.0.0 remains not released.\n"},
        "remains_not_released_token": {**docs, first: docs[first] + "\nv4.0.0 remains NOT_RELEASED.\n"},
        "release_candidate": {**docs, first: docs[first] + "\nv4.0.0 RELEASE CANDIDATE / NOT_RELEASED\n"},
        "not_released_extra": {**docs, first: docs[first] + "\nDRC v4.0.0: NOT_RELEASED_EXTRA\n"},
        "additive_released_suffix_shadow": {**docs, first: docs[first] + "\nDRC v4.0.0: RELEASED_EXTRA\n"},
        "additive_released_prefix_shadow": {**docs, first: docs[first] + "\nPREFIX DRC v4.0.0: RELEASED\n"},
        "additive_released_trailing_shadow": {**docs, first: docs[first] + "\nDRC v4.0.0: RELEASED trailing text\n"},
        "trailing_text": control_e_mutate_one_doc(docs, first, "DRC v4.0.0: RELEASED", "DRC v4.0.0: RELEASED trailing text"),
        "duplicate_current_released_state": control_e_mutate_one_doc(docs, first, "DRC v4.0.0: RELEASED", "DRC v4.0.0: RELEASED\nDRC v4.0.0: RELEASED"),
        "historical_control_b_not_released_accepted": {**docs, history_doc: docs[history_doc]},
    }
    rejected = {name: not current_released_state_contract(value) for name, value in variants.items() if name != "historical_control_b_not_released_accepted"}
    return {
        "canonical_accepted": current_released_state_contract(docs),
        "historical_control_b_not_released_accepted": current_released_state_contract(variants["historical_control_b_not_released_accepted"]),
        **{name + "_rejected": value for name, value in rejected.items()},
    }


def control_e_assert_actual_runtime_contract(doc_texts: dict[str, str]) -> None:
    checks = {
        "current_doc_keyset_exact": set(doc_texts) == set(CURRENT_DOCS),
        "r3_historical_terminal_contract": control_e_r3_historical_and_terminal_contract(doc_texts),
        "current_released_state_contract": current_released_state_contract(doc_texts),
        "normalized_active_authorization_signature_count_0": not control_e_authorization_reinjection_present(doc_texts),
        "canonical_r3_heading_exact": all(STAGE4_RETRY_CURRENT_HEADING in norm(doc_texts.get(relative, "")) for relative in CURRENT_DOCS),
        ...
        "control_e_authorization_consumed": all("Control E authorization state: CONSUMED" in norm(text) for text in doc_texts.values()),
        "control_e_execution_count_1": all("Control E execution count: 1" in norm(text) for text in doc_texts.values()),
        "control_e_budget_1_remaining_0": all("Control E explicit authorization budget: 1" in norm(text) and "Control E explicit authorization budget remaining: 0" in norm(text) for text in doc_texts.values()),
        "tag_github_drc_released": all("annotated tag: DRC_v4.0.0 / ANNOTATED / PUSHED" in norm(text) and "GitHub Release: PUBLISHED / NON_DRAFT / NON_PRERELEASE" in norm(text) and "DRC v4.0.0: RELEASED" in norm(text) for text in doc_texts.values()),
...
def control_e_canonical_doc_fixture(line_ending: str = "\n") -> dict[str, str]:
    ...
    docs = {}
    for relative in CURRENT_DOCS:
        prefix = ((history + "\n") if relative in R3_V3_HISTORY_DOCS else "") + R3_CURRENT_TERMINAL_NOTE + "\n"
        docs[relative] = (prefix + section).replace("\n", line_ending)
    return docs
...
CONTROL_E_ADVERSARIAL_HEADINGS = (
    "## Control E Release Publication Result Sync R1 Current Section",
    "## Control E Release Publication Result Sync Corrective R5 Current Section Extra",
    "## Extra Control E Release Publication Result Sync Corrective R5 Current Section",
    "## Control E Release Publication Result Sync Corrective R02 Current Section",
    "## Control E Release Publication Result Sync Corrective R-2 Current Section",
    "## Control E Release Publication Result Sync Corrective R_2 Current Section",
    "## Control E Release Publication Result Sync Corrective Ｒ２ Current Section",
    "## Control E Release Publication Result Sync Corrective R 2 Current Section",
    "## Control E Release R2 Publication Result Sync Corrective Current Section",
    "## CONTROL E RELEASE PUBLICATION RESULT SYNC CORRECTIVE R2 CURRENT SECTION",
    "## Control  E Release Publication Result Sync Corrective R2 Current Section",
    "## Control-E-Release-Publication-Result-Sync-Corrective-R2-Current-Section",
    "## Control E Release Authorization Sync R1 Current Section",
    "## Control E Release Authorization Sync Corrective R2 Current Section",
...
def control_e_heading_family_runtime_probe() -> dict[str, bool]:
    ...
    per_heading = {heading: stage4_retry_current_heading_family(heading) and not control_e_raw_heading_is_canonical(heading) and not control_e_actual_runtime_contract_accepts(control_e_append_shadow_heading(docs, first_doc, heading)) for heading in CONTROL_E_ADVERSARIAL_HEADINGS}
    per_file = {relative: not control_e_actual_runtime_contract_accepts(control_e_append_shadow_heading(docs, relative, CONTROL_E_ADVERSARIAL_HEADINGS[0])) for relative in CURRENT_DOCS}
    return {
        "canonical_r3_heading_accepted": control_e_actual_runtime_contract_accepts(docs),
        ...
        "all_current_docs_shadow_append_rejected": all(per_file.values()),
        "r1_heading_rejected": per_heading[CONTROL_E_ADVERSARIAL_HEADINGS[0]],
        "legacy_r6_heading_rejected": per_heading[CONTROL_E_ADVERSARIAL_HEADINGS[-1]],
    }
...
def control_e_token_mutation_fixture_matrix() -> dict[str, dict[str, str]]:
    ...
    other_doc = "README.md"
    payloads = {
        "exact": "AUTHORIZED_FOR_EXACTLY_ONE_CONTROL_E_RELEASE_PUBLICATION",
        "lowercase": "authorized_for_exactly_one_control_e_release_publication",
        "mixed_case": "Authorized_For_Exactly_One_Control_E_Release_Publication",
        "fullwidth_nfkc": "ＡＵＴＨＯＲＩＺＥＤ＿ＦＯＲ＿ＥＸＡＣＴＬＹ＿ＯＮＥ＿ＣＯＮＴＲＯＬ＿Ｅ＿ＲＥＬＥＡＳＥ＿ＰＵＢＬＩＣＡＴＩＯＮ",
        "spaces": "AUTHORIZED FOR EXACTLY ONE CONTROL E RELEASE PUBLICATION",
        "tabs": "AUTHORIZED\tFOR\tEXACTLY\tONE\tCONTROL\tE\tRELEASE\tPUBLICATION",
        "newlines": "AUTHORIZED\nFOR\nEXACTLY\nONE\nCONTROL\nE\nRELEASE\nPUBLICATION",
        "hyphens": "AUTHORIZED-FOR-EXACTLY-ONE-CONTROL-E-RELEASE-PUBLICATION",
        "unicode_dash": "AUTHORIZED–FOR–EXACTLY–ONE–CONTROL–E–RELEASE–PUBLICATION",
        "nbsp": "AUTHORIZED\u00a0FOR\u00a0EXACTLY\u00a0ONE\u00a0CONTROL\u00a0E\u00a0RELEASE\u00a0PUBLICATION",
        "u3000": "AUTHORIZED\u3000FOR\u3000EXACTLY\u3000ONE\u3000CONTROL\u3000E\u3000RELEASE\u3000PUBLICATION",
        "zero_width": "AUTHORIZED\u200bFOR\u200bEXACTLY\u200bONE\u200bCONTROL\u200bE\u200bRELEASE\u200bPUBLICATION",
        "markdown_bold": "**AUTHORIZED_FOR_EXACTLY_ONE_CONTROL_E_RELEASE_PUBLICATION**",
        "markdown_italic": "*AUTHORIZED_FOR_EXACTLY_ONE_CONTROL_E_RELEASE_PUBLICATION*",
        "markdown_backtick": "`AUTHORIZED_FOR_EXACTLY_ONE_CONTROL_E_RELEASE_PUBLICATION`",
        "punctuation": "[AUTHORIZED]:FOR_(EXACTLY)-ONE,CONTROL.E/RELEASE!PUBLICATION",
        "prefix": "prefixAUTHORIZED_FOR_EXACTLY_ONE_CONTROL_E_RELEASE_PUBLICATION",
        "suffix": "AUTHORIZED_FOR_EXACTLY_ONE_CONTROL_E_RELEASE_PUBLICATIONsuffix",
        "trailing": "authorization: AUTHORIZED_FOR_EXACTLY_ONE_CONTROL_E_RELEASE_PUBLICATION trailing",
        "duplicate": "AUTHORIZED_FOR_EXACTLY_ONE_CONTROL_E_RELEASE_PUBLICATION AUTHORIZED_FOR_EXACTLY_ONE_CONTROL_E_RELEASE_PUBLICATION",
        "unrelated_heading": "## Historical Notes\nAUTHORIZED_FOR_EXACTLY_ONE_CONTROL_E_RELEASE_PUBLICATION",
        "historical_section": "## Historical Control E\nAUTHORIZED_FOR_EXACTLY_ONE_CONTROL_E_RELEASE_PUBLICATION",
        "multiline_split": "AUTHORIZED_FOR_EXACTLY_ONE_\nCONTROL_E_RELEASE_PUBLICATION",
    }
    variants = {}
    for name, payload in payloads.items():
        mutated = dict(docs)
        mutated[token_doc] = mutated[token_doc].rstrip() + f"\nFixture {name}: {payload}\n"
        variants[name] = mutated
    non_designated = dict(docs)
    non_designated[other_doc] = non_designated[other_doc].rstrip() + "\nFixture non-designated: AUTHORIZED_FOR_EXACTLY_ONE_CONTROL_E_RELEASE_PUBLICATION\n"
    variants["non_designated_doc"] = non_designated
    return variants


def control_e_authorization_reinjection_runtime_rejection_probe() -> dict[str, bool]:
    docs = control_e_canonical_doc_fixture()
    variants = control_e_token_mutation_fixture_matrix()
    serialized = ["\n".join(variant[relative] for relative in CURRENT_DOCS) for variant in variants.values()]
    bounded = dict(docs)
    bounded[CURRENT_DOCS[0]] = "Control E publication completed exactly once and its authorization is consumed\n\n" + bounded[CURRENT_DOCS[0]]
    rejected = {name: not control_e_actual_runtime_contract_accepts(variant) for name, variant in variants.items()}
    return {
        "canonical_zero_token_accepted": control_e_actual_runtime_contract_accepts(docs),
        "normalized_active_token_count_0": not control_e_authorization_reinjection_present(docs),
        "all_reinjection_variants_rejected": all(rejected.values()),
        "reinjection_fixture_payloads_all_distinct": len(serialized) == len(set(serialized)) == len(variants),
        "lowercase_rejected": rejected["lowercase"], "mixed_case_rejected": rejected["mixed_case"],
        "nfkc_rejected": rejected["fullwidth_nfkc"], "whitespace_rejected": rejected["spaces"] and rejected["tabs"],
        "newline_rejected": rejected["newlines"] and rejected["multiline_split"],
        "unicode_rejected": rejected["unicode_dash"] and rejected["nbsp"] and rejected["u3000"],
        "zero_width_rejected": rejected["zero_width"],
        "markdown_rejected": rejected["markdown_bold"] and rejected["markdown_italic"] and rejected["markdown_backtick"],
        "historical_rejected": rejected["historical_section"] and rejected["unrelated_heading"],
        "non_designated_rejected": rejected["non_designated_doc"],
        "bounded_consumed_prose_accepted": control_e_actual_runtime_contract_accepts(bounded),
    }
...
def control_e_release_authorization_schema_self_check() -> dict[str, bool]:
    docs = control_e_canonical_doc_fixture()
    first = CURRENT_DOCS[0]
    probe = control_e_authorization_reinjection_runtime_rejection_probe()
    heading = control_e_heading_family_runtime_probe()
    checks = {
        "canonical_consumed_contract_accepted": control_e_actual_runtime_contract_accepts(docs),
        "canonical_crlf_consumed_contract_accepted": control_e_actual_runtime_contract_accepts(control_e_canonical_doc_fixture("\r\n")),
        **probe,
        "consumed_state_required": not control_e_actual_runtime_contract_accepts(control_e_mutate_one_doc(docs, first, "Control E authorization state: CONSUMED", "Control E authorization state: ACTIVE")),
        "execution_count_1_required": not control_e_actual_runtime_contract_accepts(control_e_mutate_one_doc(docs, first, "Control E execution count: 1", "Control E execution count: 0")),
        "budget_remaining_0_required": not control_e_actual_runtime_contract_accepts(control_e_mutate_one_doc(docs, first, "Control E explicit authorization budget remaining: 0", "Control E explicit authorization budget remaining: 1")),
        "active_authorization_true_rejected": not control_e_actual_runtime_contract_accepts(control_e_mutate_one_doc(docs, first, "Control E current active authorization: FALSE", "Control E current active authorization: TRUE")),
        "additional_publication_true_rejected": not control_e_actual_runtime_contract_accepts(control_e_mutate_one_doc(docs, first, "Additional publication authorized: FALSE", "Additional publication authorized: TRUE")),
        "invocation_4_authorized_rejected": not control_e_actual_runtime_contract_accepts({**docs, first: docs[first] + "\nStage 4 Invocation 4 authorized: TRUE\n"}),
        "canonical_r3_heading_accepted": heading["canonical_r3_heading_accepted"],
        "all_adversarial_headings_classified_as_protected_family": heading["all_adversarial_headings_classified_as_protected_family"],
        "all_adversarial_heading_shadow_contracts_rejected": heading["all_adversarial_heading_shadow_contracts_rejected"],
        "all_current_docs_shadow_append_rejected": heading["all_current_docs_shadow_append_rejected"],
        "heading_duplicate_rejected": not control_e_actual_runtime_contract_accepts({**docs, first: docs[first].replace(STAGE4_RETRY_CURRENT_HEADING, STAGE4_RETRY_CURRENT_HEADING + "\n\n" + STAGE4_RETRY_CURRENT_HEADING, 1)}),
        "tag_published_tuple_present": all("annotated tag: DRC_v4.0.0 / ANNOTATED / PUSHED" in text and "GitHub Release: PUBLISHED / NON_DRAFT / NON_PRERELEASE" in text for text in docs.values()),
    }
    return checks
...
def control_e_publication_reachability_matrix_self_check() -> dict[str, bool]:
    modes = (
        "DIRTY_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_CANDIDATE",
        "CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_NOT_PUSHED",
        "CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE",
        "DIRTY_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CANDIDATE",
        "CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_NOT_PUSHED",
        "CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC",
        "UNKNOWN_MODE",
    )
    return {f"mode_{index}_false": not control_e_publication_reachability(mode) for index, mode in enumerate(modes)}
...
def control_e_runtime_with_poisoned_loader_and_fixture_probe() -> dict[str, bool]:
    ...
    def stub_git_out(*args: object, **kwargs: object) -> str:
        counts["git"] += 1
        if args == ("tag", "--list", EXPECTED_TAG):
            return EXPECTED_TAG
        if args == ("rev-parse", EXPECTED_TAG):
            return EXPECTED_TAG_OBJECT
        if args == ("rev-list", "-n", "1", EXPECTED_TAG):
            return EXPECTED_TAG_TARGET
        if args == ("cat-file", "-t", EXPECTED_TAG):
            return "tag"
        if args == ("cat-file", "-t", EXPECTED_TAG_TARGET):
            return "commit"
        if args == ("for-each-ref", "--format=%(contents:subject)", f"refs/tags/{EXPECTED_TAG}"):
            return "Daily Rhythm Companion v4.0.0"
        return ""
...
def control_e_runtime_order_self_check() -> dict[str, bool]:
    ...
    invalid_docs = control_e_token_mutation_fixture_matrix()["exact"]
...
def published_tag_tuple_is_exact(
    tag_name: str,
    ref_object_type: str,
    tag_object_sha: str,
    ...
    return (
        ...
        and ref_object_type == "tag"
        and tag_object_sha == EXPECTED_TAG_OBJECT
        and target_object_type == "commit"
        and target_commit == EXPECTED_TAG_TARGET
        and tag_message == "Daily Rhythm Companion v4.0.0"
    )


def published_tag_tuple_is_exact_in_repo() -> bool:
    if git_out("tag", "--list", EXPECTED_TAG) != EXPECTED_TAG:
        return False
    target = git_out("rev-list", "-n", "1", EXPECTED_TAG)
    return published_tag_tuple_is_exact(
        EXPECTED_TAG,
        git_out("cat-file", "-t", EXPECTED_TAG),
        git_out("rev-parse", EXPECTED_TAG),
        git_out("cat-file", "-t", target),
        target,
        git_out("for-each-ref", "--format=%(contents:subject)", f"refs/tags/{EXPECTED_TAG}"),
    )


def published_tag_tuple_self_check() -> dict[str, bool]:
    values = [EXPECTED_TAG, "tag", EXPECTED_TAG_OBJECT, "commit", EXPECTED_TAG_TARGET, "Daily Rhythm Companion v4.0.0"]
    checks = {"canonical_exact": published_tag_tuple_is_exact(*values)}
    labels = ("wrong_tag_name", "lightweight_tag", "wrong_object_sha", "wrong_target_type", "wrong_target_commit", "wrong_message")
    replacements = ("DRC_v4.0.1", "commit", "0" * 40, "tag", "1" * 40, "Daily Rhythm Companion v4")
    for index, label in enumerate(labels):
        mutated = list(values)
        mutated[index] = replacements[index]
        checks[label + "_rejected"] = not published_tag_tuple_is_exact(*mutated)
    return checks
...
PUBLICATION_RECORD_FIELDS = (
    ...
    ("GitHub Release asset status", "UPLOADED / VERIFIED"),
    ("GitHub Release asset ID", "569761397"),
    ("GitHub Release asset basename", EXPECTED_FIXED_ZIP_BASENAME),
    ("GitHub Release asset state", "uploaded"),
    ("GitHub Release asset content type", "application/zip"),
    ("GitHub Release asset size", str(EXPECTED_FIXED_ZIP_SIZE)),
    ("GitHub Release asset digest", "sha256:" + EXPECTED_FIXED_ZIP_SHA256),
    ("GitHub Release asset URL", "https://github.com/murayan1982/daily-rhythm-companion-public/releases/download/DRC_v4.0.0/" + EXPECTED_FIXED_ZIP_BASENAME),
)


def publication_record_schema_self_check() -> dict[str, bool]:
    docs = control_e_canonical_doc_fixture()
    first = CURRENT_DOCS[0]
    checks: dict[str, bool] = {"canonical_exact": control_e_actual_runtime_contract_accepts(docs)}
    for label, value in PUBLICATION_RECORD_FIELDS:
        mutated = control_e_mutate_one_doc(docs, first, f"{label}: {value}", f"{label}: WRONG")
        checks[label.lower().replace(" ", "_") + "_wrong_rejected"] = not control_e_actual_runtime_contract_accepts(mutated)
    missing = dict(docs)
    missing[first] = missing[first].replace("GitHub Release status: CREATED / PUBLISHED / VERIFIED\n", "", 1)
    duplicate = dict(docs)
    duplicate[first] = duplicate[first].replace("GitHub Release asset ID: 569761397", "GitHub Release asset ID: 569761397\nGitHub Release asset ID: 569761397", 1)
    unknown = dict(docs)
    unknown[first] = unknown[first].replace("Next authorized release action: NONE", "Unexpected publication field: value\nNext authorized release action: NONE", 1)
    trailing = dict(docs)
    trailing[first] = trailing[first].replace("GitHub Release ID: 390524244", "GitHub Release ID: 390524244 trailing", 1)
    uppercase_digest = dict(docs)
    uppercase_digest[first] = uppercase_digest[first].replace(EXPECTED_FIXED_ZIP_SHA256, EXPECTED_FIXED_ZIP_SHA256.upper(), 1)
    checks.update({
        "missing_release_rejected": not control_e_actual_runtime_contract_accepts(missing),
        "duplicate_asset_field_rejected": not control_e_actual_runtime_contract_accepts(duplicate),
        "unexpected_field_rejected": not control_e_actual_runtime_contract_accepts(unknown),
        "trailing_text_rejected": not control_e_actual_runtime_contract_accepts(trailing),
        "digest_case_variation_rejected": not control_e_actual_runtime_contract_accepts(uppercase_digest),
    })
    return checks
...
def control_e_release_authorization_runtime_guard(
    ...
    checks = {
        ...
        "r3_historical_terminal_actual_docs": control_e_r3_historical_and_terminal_contract(docs),
        "current_released_state_actual_docs": current_released_state_contract(docs),
        "r3_historical_terminal_self_check": all(control_e_r3_historical_and_terminal_self_check().values()),
        "current_released_state_actual_docs": current_released_state_contract(docs),
        "current_released_state_self_check": all(current_released_state_contract_self_check().values()),
        "heading_family_guard": control_e_heading_family_runtime_probe()["all_adversarial_heading_shadow_contracts_rejected"],
        "authorization_reinjection_runtime_rejection": control_e_authorization_reinjection_runtime_rejection_probe()["all_reinjection_variants_rejected"],
        "publication_reachability_matrix_self_check": all(reachability_checks.values()),
        "current_mode_publication_reachability_evaluated": control_e_publication_reachability(mode) == (False),
        "control_e_modes_release_zip_unreachable": release_zip_unreachable,
        "normalized_reinjection_absent": control_e_authorization_token_mapping(docs),
        ...
        "tag_exact": published_tag_tuple_is_exact_in_repo(),
        "tag_tuple_negative_fixtures": all(published_tag_tuple_self_check().values()),
        "publication_record_negative_fixtures": all(publication_record_schema_self_check().values()),
        "github_release_published": all("GitHub Release: PUBLISHED / NON_DRAFT / NON_PRERELEASE" in norm(text) for text in docs.values()),
        "drc_v400_released": all("DRC v4.0.0: RELEASED" in norm(text) for text in docs.values()),
...
def control_e_release_authorization_runtime_guard_self_check() -> dict[str, bool]:
    ...
    dependency_names = {
        ...
        "control_e_r3_historical_and_terminal_contract",
        "control_e_r3_historical_and_terminal_self_check",
        "current_released_state_contract",
        "current_released_state_contract_self_check",
        "control_e_release_authorization_schema_self_check",
        "control_e_heading_family_runtime_probe",
        "control_e_authorization_reinjection_runtime_rejection_probe",
        "control_e_publication_reachability_matrix_self_check",
        "control_e_publication_reachability",
        "release_zip_reachability",
        "published_tag_tuple_is_exact_in_repo",
        "published_tag_tuple_self_check",
        "publication_record_schema_self_check",
    ...
    return {
        ...
        "runtime_guard_reaches_current_released_state_contract": "current_released_state_contract" in guard_names,
        ...
        "runtime_guard_reaches_missing_token_probe": "control_e_authorization_reinjection_runtime_rejection_probe" in guard_names,
...
def check_protocol_and_records() -> None:
    ...
    for label, value in (
        ("Status", "RELEASED / PUBLISHED / VERIFIED"),
        (
            "Current phase",
            "CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_R5 / IMPLEMENTED / STATIC_VERIFIED / EXACT_LINE_CONTRACT_VERIFIED / ADDITIVE_SHADOW_BYPASS_REJECTED / SUFFIX_PREFIX_TRAILING_BYPASS_REJECTED / CURRENT_RELEASED_STATE_CORRECTED / HISTORICAL_V3_BOUNDARY_PRESERVED / POST_SOURCE_MATRIX_0_TO_9_VERIFIED / ADDITIONAL_PUBLICATION_BLOCKED / FIXED_ZIP_PRESERVED / NEW_THREAD_HANDOFF_CREATED / READY_FOR_RE_REVIEW",
        ),
        ...
        ("verification HEAD", CONTROL_D_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_COMMIT),
        ...
        ("fixed ZIP builder invocation count", "1"),
        ("same-artifact verification", "EXACTLY_ONCE_EXECUTED / PASS / REVIEWED / ACCEPTED"),
        ("verification verdict", "PASS"),
        ("failure class", "NON_PRODUCT_VERIFIER_OUTPUT_ENCODING_FAILURE"),
        ("Stage 4 invocation 2 failure class", "NON_PRODUCT_VERIFIER_PYTEST_BASETEMP_PERMISSION_FAILURE"),
        ("Stage 4 invocation 2 Backend result", "272 passed / 207 setup errors"),
        ("Historical Stage 4 retry", "CONSUMED / FAILED"),
            ...
            ("Next authorized invocation number", "NOT_AUTHORIZED"),
            ("Invocation 3 authorization state", "CONSUMED"),
        ("explicit final operator approval", "RECEIVED / CONSUMED"),
        ("annotated tag publication", "CREATED / PUSHED / VERIFIED"),
        ("GitHub Release publication", "CREATED / PUBLISHED / VERIFIED"),
    ):
    ...
    if current_text.count(STAGE4_RETRY_AUTHORIZATION) != 0:
        raise AssertionError("Stage 4 retry authorization token occurrence is not exact 0")
    if current_text.count(STAGE4_INVOCATION_3_AUTHORIZATION) != 0:
        raise AssertionError("Invocation 3 authorization token occurrence is not exact 0")
    if not consumed_retry_history_contract_is_present_in_docs():
        raise AssertionError("Stage 4 consumed retry history contract is malformed")
    if docs_have_stage4_retry_authorization():
        raise AssertionError("current Stage 4 retry authorization must be absent")
    if not all(invocation_3_authorization_schema_self_check().values()):
        raise AssertionError("Invocation 3 exact authorization schema self-check failed")
...
ALLOWED_POST_SOURCE_HEAD_SURFACE = STAGE2A_MODIFIED
...
def validate_post_source_head_surface(
    ...
    return bool(changed) and changed <= set(ALLOWED_POST_SOURCE_HEAD_SURFACE)
...
def post_source_head_surface_matrix_self_check() -> dict[str, bool]:
    ...
    return {
        ...
        "count_8_accepted": validate_post_source_head_surface(changed, [], [], 8),
        "count_9_accepted": validate_post_source_head_surface(changed, [], [], 9),
        "count_10_rejected": not validate_post_source_head_surface(changed, [], [], 10),
...
def release_zip_reachability_matrix_self_check() -> dict[str, bool]:
    malformed = current_docs_text().replace(
        "Invocation 3 authorization state: CONSUMED",
        "Invocation 3 authorized: FALSE",
        1,
    )
    return {
        "dirty_retry_authorization_candidate_blocked": not release_zip_reachability("DIRTY_STAGE4_RETRY_AUTHORIZATION_SYNC_CANDIDATE"),
        "clean_not_pushed_invocation_3_authorization_state_blocked": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_NOT_PUSHED"),
        "clean_pushed_invocation_3_authorization_state_blocked": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC"),
        "old_initial_stage4_authorization_state_blocked": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_AUTHORIZATION_SYNC"),
        "verifier_output_corrective_state_blocked": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_VERIFIER_OUTPUT_ENCODING_CORRECTIVE"),
        "dirty_basetemp_corrective_state_blocked": not release_zip_reachability(
            "DIRTY_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_CANDIDATE"
        ),
        "clean_not_pushed_basetemp_corrective_state_blocked": not release_zip_reachability(
            "CLEAN_COMMITTED_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_NOT_PUSHED"
        ),
        "clean_pushed_basetemp_corrective_state_blocked": not release_zip_reachability(
            "CLEAN_COMMITTED_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE"
        ),
        "basetemp_corrective_cannot_authorize_invocation_3": not release_zip_reachability(
            "CLEAN_COMMITTED_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE"
        ),
        "malformed_retry_contract_blocked": not stage4_retry_authorization_contract_is_present(malformed),
    }
...
def validate_extracted_pytest_basetemp(extraction_root: Path, source: Path, basetemp: Path) -> None:
    if not extraction_root.is_absolute():
        raise AssertionError("extraction root must be absolute")
    if not source.is_absolute():
        raise AssertionError("extracted source must be absolute")
    if not basetemp.is_absolute():
        raise AssertionError("isolated pytest basetemp must be absolute")
    if path_has_parent_ref(extraction_root) or path_has_parent_ref(source) or path_has_parent_ref(basetemp):
        raise AssertionError("isolated pytest basetemp path traversal rejected")
    if path_has_shared_pytest_root(extraction_root) or path_has_shared_pytest_root(source) or path_has_shared_pytest_root(basetemp):
        raise AssertionError("shared pytest root rejected")
    if not extraction_root.is_dir():
        raise AssertionError("extraction root must exist and be a directory")
    if not source.is_dir():
        raise AssertionError("extracted source must exist")
    ...
    if source_resolved != root / "DailyRhythmCompanion":
        raise AssertionError("extracted source must be the expected direct child of extraction root")
    if basetemp.name != STAGE4_PYTEST_BASETEMP_BASENAME:
        raise AssertionError("isolated pytest basetemp basename mismatch")
    if basetemp_resolved.parent != root:
        raise AssertionError("isolated pytest basetemp parent must be extraction root")
    if basetemp_resolved == root:
        raise AssertionError("isolated pytest basetemp must not equal extraction root")
    if basetemp_resolved == source_resolved:
        raise AssertionError("isolated pytest basetemp must not equal extracted source")
    if path_is_relative_to(basetemp_resolved, source_resolved):
        raise AssertionError("isolated pytest basetemp must not be inside extracted source")
    if not path_is_relative_to(basetemp_resolved, root):
        raise AssertionError("isolated pytest basetemp must stay under extraction root")
    if basetemp.exists():
        raise AssertionError("isolated pytest basetemp must not already exist")
...
def validate_extracted_backend_pytest_command(command: list[str], basetemp: Path) -> None:
    ...
    if command != expected:
        raise AssertionError("extracted Backend pytest command contract mismatch")
    if command.count("backend/tests") != 1:
        raise AssertionError("extracted Backend pytest command must target backend/tests exactly once")
    if command.count("--basetemp") != 1:
        raise AssertionError("extracted Backend pytest command must include exactly one --basetemp")
    basetemp_index = command.index("--basetemp")
    if basetemp_index + 1 >= len(command):
        raise AssertionError("extracted Backend pytest command missing basetemp value")
    if command[basetemp_index + 1] != basetemp_value or not Path(command[basetemp_index + 1]).is_absolute():
        raise AssertionError("extracted Backend pytest command basetemp value mismatch")
    if any(arg.startswith("--basetemp=") for arg in command):
        raise AssertionError("extracted Backend pytest command must use two-argument --basetemp form")
    if any(arg in {"TEMP", "TMP", "TMPDIR", "PYTEST_DEBUG_TEMPROOT"} or "PYTEST_DEBUG_TEMPROOT" in arg for arg in command):
        raise AssertionError("extracted Backend pytest command must not include temp override arguments")
...
def check_no_release_outputs() -> None:
    ...
    if not fixed_zip_tuple_is_exact():
        raise AssertionError("preserved v4.0.0 fixed ZIP tuple is not exact")
...
def stage4_lifecycle_contract_is_valid(text: str) -> bool:
    return (
        ...
        and "Stage 4 release-ZIP reachability: FALSE" in text
    )


def invocation_3_authorization_schema_self_check() -> dict[str, bool]:
    text = current_docs_text()
    return {
        "current_docs_record_invocation_3_pass": invocation_3_pass_result_contract_is_present(text),
        "current_docs_do_not_authorize_retry": not docs_have_stage4_retry_authorization(),
        "invocation_3_token_count_zero": text.count(STAGE4_INVOCATION_3_AUTHORIZATION) == 0,
        "consumed_retry_token_count_zero": text.count(STAGE4_RETRY_AUTHORIZATION) == 0,
        "invocation_3_completed_count_one": "Invocation 3 completed invocation count: 1" in text,
        "invocation_3_budget_one": "Invocation 3 explicit authorization budget: 1" in text,
        "invocation_3_budget_remaining_zero": "Invocation 3 explicit authorization budget remaining: 0" in text,
        "authorization_state_consumed": "Invocation 3 authorization state: CONSUMED" in text,
        "next_invocation_not_authorized": "Next authorized invocation number: NOT_AUTHORIZED" in text,
        "additional_invocation_false": "Additional Stage 4 invocation authorized: FALSE" in text,
        "dirty_invocation_3_release_zip_reachability_false": not release_zip_reachability("DIRTY_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_CANDIDATE"),
        "dirty_pass_result_release_zip_reachability_false": not release_zip_reachability("DIRTY_STAGE4_INVOCATION_3_PASS_RESULT_SYNC_CANDIDATE"),
        "clean_not_pushed_pass_result_release_zip_reachability_false": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_INVOCATION_3_PASS_RESULT_SYNC_NOT_PUSHED"),
        "clean_pushed_pass_result_release_zip_reachability_false": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_INVOCATION_3_PASS_RESULT_SYNC"),
        "clean_not_pushed_invocation_3_release_zip_reachability_false": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_NOT_PUSHED"),
        "clean_pushed_invocation_3_release_zip_reachability_true": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC"),
    }


def stage4_lifecycle_contract_self_check() -> dict[str, bool]:
    text = current_docs_text()
    malformed_completed_zero = text.replace("Invocation 3 completed invocation count: 1", "Invocation 3 completed invocation count: 0", 1)
    malformed_completed_two = text.replace("Invocation 3 completed invocation count: 1", "Invocation 3 completed invocation count: 2", 1)
    malformed_budget_one = text.replace("Invocation 3 explicit authorization budget remaining: 0", "Invocation 3 explicit authorization budget remaining: 1", 1)
    malformed_budget_00 = text.replace("Invocation 3 explicit authorization budget remaining: 0", "Invocation 3 explicit authorization budget remaining: 00", 1)
    malformed_active = text.replace("Invocation 3 authorization state: CONSUMED", "Invocation 3 authorization state: ACTIVE", 1)
    malformed_next_3 = text.replace("Next authorized invocation number: NOT_AUTHORIZED", "Next authorized invocation number: 3", 1)
    ...
    malformed_control_e = text.replace("Control E: COMPLETED / PASS / ACCEPTED", "Control E: NOT_AUTHORIZED")
    malformed_tag = text.replace("annotated tag: DRC_v4.0.0 / ANNOTATED / PUSHED", "annotated tag: NOT_CREATED")
    malformed_release = text.replace("GitHub Release: PUBLISHED / NON_DRAFT / NON_PRERELEASE", "GitHub Release: NOT_CREATED")
    malformed_token = text + "\n" + STAGE4_INVOCATION_3_TOKEN_LINE
    return {
        "canonical_consumed_pass_result_accepted": stage4_lifecycle_contract_is_valid(text),
        "invocation_3_completed_count_0_rejected": not stage4_lifecycle_contract_is_valid(malformed_completed_zero),
        "invocation_3_completed_count_2_rejected": not stage4_lifecycle_contract_is_valid(malformed_completed_two),
        "budget_remaining_1_rejected": not stage4_lifecycle_contract_is_valid(malformed_budget_one),
        "budget_remaining_00_rejected": not stage4_lifecycle_contract_is_valid(malformed_budget_00),
        "authorization_state_active_rejected": not stage4_lifecycle_contract_is_valid(malformed_active),
        "next_authorized_invocation_3_rejected": not stage4_lifecycle_contract_is_valid(malformed_next_3),
        "additional_invocation_true_rejected": not stage4_lifecycle_contract_is_valid(malformed_additional_true),
        ...
        "release_zip_reachability_false_all_modes": all(not release_zip_reachability(mode) for mode in [
            ...
            "CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC",
        ]),
    }


def r7_current_status_self_check() -> dict[str, bool]:
    status = top_level_status_section(read("docs/v400_release_preparation_protocol.md")) or ""
    return {
        "current_status_accepted": protocol_current_status_is_correct("## Status\n" + status),
        "pass_verdict_present": exact_status_value_is_present(status, "verification verdict", "PASS"),
        "invocation_3_completed_count_present": exact_status_value_is_present(status, "Invocation 3 completed invocation count", "1"),
        "budget_remaining_zero_present": exact_status_value_is_present(status, "Invocation 3 explicit authorization budget remaining", "0"),
        ...
        "control_e_completed_present": exact_status_value_is_present(status, "Control E", "COMPLETED / PASS / ACCEPTED"),
        "tag_published_present": exact_status_value_is_present(status, "annotated tag", "DRC_v4.0.0 / ANNOTATED / PUSHED"),
        "release_published_present": exact_status_value_is_present(status, "GitHub Release", "PUBLISHED / NON_DRAFT / NON_PRERELEASE"),
...
def stage4_content_review_runtime_connection_self_check() -> dict[str, bool]:
    ...
    return {
        ...
        "invocation_3_pass_recorded": "Stage 4 invocation 3 result: EXACTLY_ONCE_EXECUTED / PASS / REVIEWED / ACCEPTED" in text,
        "authorization_consumed_recorded": "Invocation 3 authorization state: CONSUMED" in text,
        "additional_invocation_blocked": "Additional Stage 4 invocation authorized: FALSE" in text,
        "release_zip_reachability_false": "Stage 4 release-ZIP reachability: FALSE" in text,
        "fixed_zip_preserved": "fixed ZIP: PRESERVED / BYTE_IDENTICAL" in text,
    }


def stage4_pytest_basetemp_isolation_self_check() -> dict[str, bool]:
    return {
        "basetemp_contract_preserved": True,
        "output_encoding_handling_preserved": True,
        "backend_pytest_isolated_basetemp_preserved": True,
        "fixed_zip_tuple_guard_preserved": fixed_zip_tuple_self_check()["actual_tuple_accepted"],
        "no_builder_behavior_preserved": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_INVOCATION_3_PASS_RESULT_SYNC"),
        "invocation_3_authorization_consumed": not invocation_3_is_authorized(current_docs_text()),
    }
...
def main() -> None:
    ...
    lifecycle_checks = stage4_lifecycle_contract_self_check()
    reachability_matrix_checks = release_zip_reachability_matrix_self_check()
    ...
    print(
        ...
        f"{all(lifecycle_checks.values())}"
    )
    print(
        "v400_release_candidate_no_build_preflight_stage4_retained_heading_family_case_count: "
        f"{STAGE4_RETRY_RETAINED_HEADING_FAMILY_CASE_COUNT}"
    )
    print(
        "v400_release_candidate_no_build_preflight_stage4_r6_suffix_middle_version_case_count: "
        f"{STAGE4_RETRY_R5_SUFFIX_FAMILY_CASE_COUNT}"
    )
    print(
        "v400_release_candidate_no_build_preflight_stage4_lifecycle_contract_self_check_count: "
        f"{len(lifecycle_checks)}"
    )
    print(
    ...
    print(
        ...
        "control-e-release-publication-result-sync-corrective-r5-implemented-static-verified-exact-line-contract-verified-additive-shadow-bypass-rejected-suffix-prefix-trailing-bypass-rejected-current-released-state-corrected-historical-v3-boundary-preserved-post-source-matrix-0-to-9-verified-additional-publication-blocked-fixed-zip-preserved-new-thread-handoff-created-ready-for-re-review"
    )
    print(f"v400_release_candidate_no_build_preflight_control_e_runtime_guard_self_check: {all(control_e_runtime_guard_checks.values())}")
    print(f"v400_release_candidate_no_build_preflight_control_e_main_runtime_guard_call_count_exact_1: {control_e_runtime_guard_checks['main_calls_runtime_guard_exact_once']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_runtime_guard_dependency_calls: {control_e_runtime_guard_checks['runtime_guard_reaches_actual_precheck_schema_reachability']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_runtime_guard_unconnected_negative_control_rejected: {control_e_runtime_guard_checks['unconnected_negative_control_rejected']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_schema_forbidden_dependencies_absent: {control_e_runtime_guard_checks['schema_forbidden_dependencies_absent']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_schema_filesystem_independence_probe: {control_e_runtime_guard_checks['schema_filesystem_independence_probe']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_runtime_fixture_with_poisoned_loader_probe: {control_e_runtime_guard_checks['runtime_fixture_with_poisoned_loader_probe']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_canonical_zero_token_accepted: {control_e_schema_checks['canonical_zero_token_accepted']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_normalized_active_token_count_0: {control_e_schema_checks['normalized_active_token_count_0']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_all_reinjection_variants_rejected: {control_e_schema_checks['all_reinjection_variants_rejected']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_reinjection_fixture_payloads_all_distinct: {control_e_schema_checks['reinjection_fixture_payloads_all_distinct']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_lowercase_mixed_nfkc_rejected: {control_e_schema_checks['lowercase_rejected'] and control_e_schema_checks['mixed_case_rejected'] and control_e_schema_checks['nfkc_rejected']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_whitespace_newline_unicode_zero_width_rejected: {control_e_schema_checks['whitespace_rejected'] and control_e_schema_checks['newline_rejected'] and control_e_schema_checks['unicode_rejected'] and control_e_schema_checks['zero_width_rejected']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_markdown_historical_non_designated_rejected: {control_e_schema_checks['markdown_rejected'] and control_e_schema_checks['historical_rejected'] and control_e_schema_checks['non_designated_rejected']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_bounded_consumed_prose_accepted: {control_e_schema_checks['bounded_consumed_prose_accepted']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_canonical_r3_heading_accepted: {control_e_schema_checks['canonical_r3_heading_accepted']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_adversarial_heading_matrix_rejected: {control_e_schema_checks['all_adversarial_heading_shadow_contracts_rejected']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_all_current_docs_shadow_append_rejected: {control_e_schema_checks['all_current_docs_shadow_append_rejected']}")
    print(f"v400_release_candidate_no_build_preflight_control_e_publication_reachability_dirty: {control_e_publication_reachability('DIRTY_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_CANDIDATE')}")
    print(f"v400_release_candidate_no_build_preflight_control_e_publication_reachability_clean_not_pushed: {control_e_publication_reachability('CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_NOT_PUSHED')}")
    print(f"v400_release_candidate_no_build_preflight_control_e_publication_reachability_clean_pushed: {control_e_publication_reachability('CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE')}")
    print(f"v400_release_candidate_no_build_preflight_control_e_release_zip_reachability_dirty: {release_zip_reachability('DIRTY_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_CANDIDATE')}")
    print(f"v400_release_candidate_no_build_preflight_control_e_release_zip_reachability_clean_not_pushed: {release_zip_reachability('CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_NOT_PUSHED')}")
    print(f"v400_release_candidate_no_build_preflight_control_e_release_zip_reachability_clean_pushed: {release_zip_reachability('CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE')}")
    print(
        "v400_release_candidate_no_build_preflight_release_zip_reachability_matrix_self_check: "
        f"{all(reachability_matrix_checks.values())}"
    )
    print(
        "v400_release_candidate_no_build_preflight_dirty_retry_authorization_candidate_blocked: "
        f"{reachability_matrix_checks['dirty_retry_authorization_candidate_blocked']}"
    )
    print(
        "v400_release_candidate_no_build_preflight_clean_not_pushed_invocation_3_authorization_state_blocked: "
        f"{reachability_matrix_checks['clean_not_pushed_invocation_3_authorization_state_blocked']}"
    )
    print(
        "v400_release_candidate_no_build_preflight_clean_pushed_invocation_3_authorization_state_blocked: "
        f"{reachability_matrix_checks['clean_pushed_invocation_3_authorization_state_blocked']}"
    )
    print(
        "v400_release_candidate_no_build_preflight_dirty_basetemp_corrective_state_blocked: "
        f"{reachability_matrix_checks['dirty_basetemp_corrective_state_blocked']}"
    )
    print(
        "v400_release_candidate_no_build_preflight_clean_not_pushed_basetemp_corrective_state_blocked: "
        f"{reachability_matrix_checks['clean_not_pushed_basetemp_corrective_state_blocked']}"
    )
    print(
        "v400_release_candidate_no_build_preflight_clean_pushed_basetemp_corrective_state_blocked: "
        f"{reachability_matrix_checks['clean_pushed_basetemp_corrective_state_blocked']}"
    )
    print(
        "v400_release_candidate_no_build_preflight_basetemp_corrective_cannot_authorize_invocation_3: "
        f"{reachability_matrix_checks['basetemp_corrective_cannot_authorize_invocation_3']}"
    )
    print(
        "v400_release_candidate_no_build_preflight_malformed_retry_contract_blocked: "
        f"{reachability_matrix_checks['malformed_retry_contract_blocked']}"
    )
    print(f"v400_release_candidate_no_build_preflight_invocation_3_exact_authorization_schema_self_check: {all(invocation_3_schema_checks.values())}")
    print(f"v400_release_candidate_no_build_preflight_invocation_3_clean_not_pushed_and_pushed_dispatch_simulated: {invocation_3_schema_checks['dirty_invocation_3_release_zip_reachability_false'] and invocation_3_schema_checks['clean_not_pushed_invocation_3_release_zip_reachability_false'] and invocation_3_schema_checks['clean_pushed_invocation_3_release_zip_reachability_true']}")
    print(f"v400_release_candidate_no_build_preflight_stage4_pytest_basetemp_isolation_self_check_count: {len(basetemp_checks)}")
    print(f"v400_release_candidate_no_build_preflight_stage4_pytest_basetemp_isolation_self_check: {all(basetemp_checks.values())}")
    print(
        "v400_release_candidate_no_build_preflight_post_source_head_surface_count_matrix_self_check: "
        f"{all(post_source_matrix_checks.values())}"
    )
    print(
        "v400_release_candidate_no_build_preflight_post_source_head_surface_count_matrix_0_1_2_3_4_5_6_7_8_9_accepted_10_rejected: "
        f"{post_source_matrix_checks['count_0_accepted'] and post_source_matrix_checks['count_1_accepted'] and post_source_matrix_checks['count_2_accepted'] and post_source_matrix_checks['count_3_accepted'] and post_source_matrix_checks['count_4_accepted'] and post_source_matrix_checks['count_5_accepted'] and post_source_matrix_checks['count_6_accepted'] and post_source_matrix_checks['count_7_accepted'] and post_source_matrix_checks['count_8_accepted'] and post_source_matrix_checks['count_9_accepted'] and post_source_matrix_checks['count_10_rejected']}"
    )
    ...
    print("v400_release_candidate_no_build_preflight_v400_released: True")
    print("v400_release_candidate_no_build_preflight_release_source_head_recorded: True")
    print("v400_release_candidate_no_build_preflight_verification_head_recorded: True")
    ...
    print("v400_release_candidate_no_build_preflight_tag_created: True")
    print("v400_release_candidate_no_build_preflight_github_release_created: True")
    ...
    print(f"v400_release_candidate_no_build_preflight_stage4_retry_authorized: {docs_have_stage4_retry_authorization()}")
    print(f"v400_release_candidate_no_build_preflight_consumed_retry_history_contract_present: {consumed_retry_history_contract_is_present_in_docs()}")
    print("v400_release_candidate_no_build_preflight_stage4_retry_invocation_count: 1")
    print("v400_release_candidate_no_build_preflight_stage4_retry_budget_remaining: 0")
    print("v400_release_candidate_no_build_preflight_invocation_3_authorized: False")
    ...
    print(f"v400_release_candidate_no_build_preflight_published_tag_tuple_exact: {published_tag_tuple_is_exact_in_repo()}")
    print(f"v400_release_candidate_no_build_preflight_published_tag_negative_fixtures: {all(published_tag_tuple_self_check().values())}")
    print(f"v400_release_candidate_no_build_preflight_publication_record_negative_fixtures: {all(publication_record_schema_self_check().values())}")
    print("v400_release_candidate_no_build_preflight_control_e_execution_recorded: True")
    print("v400_release_candidate_no_build_preflight_active_publication_authorization: False")
...
def stage4_authorization_sync_modes(mode: str) -> bool:
    return mode in {
        ...
        "CLEAN_COMMITTED_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_NOT_PUSHED",
        "CLEAN_COMMITTED_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE",
        "DIRTY_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_CANDIDATE",
        "CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_NOT_PUSHED",
        "CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC",
        "DIRTY_STAGE4_INVOCATION_3_PASS_RESULT_SYNC_CANDIDATE",
        "CLEAN_COMMITTED_STAGE4_INVOCATION_3_PASS_RESULT_SYNC_NOT_PUSHED",
        "CLEAN_COMMITTED_STAGE4_INVOCATION_3_PASS_RESULT_SYNC",
...
def head_includes_published_control_e_commit(head: str = "HEAD") -> bool:
    return subprocess.run(
        ["git", "merge-base", "--is-ancestor", EXPECTED_TAG_TARGET, head],
        cwd=ROOT,
...
def check_expected_fixed_zip_tuple(*, expected_tag_published: bool = False) -> None:
    tag_exists = git_out("tag", "--list", EXPECTED_TAG) == EXPECTED_TAG
    if expected_tag_published:
        if not tag_exists:
            raise AssertionError("published DRC_v4.0.0 tag is absent")
        if git_out("rev-parse", EXPECTED_TAG) != EXPECTED_TAG_OBJECT:
            raise AssertionError("published DRC_v4.0.0 tag object is not exact")
        if git_out("rev-list", "-n", "1", EXPECTED_TAG) != EXPECTED_TAG_TARGET:
            raise AssertionError("published DRC_v4.0.0 tag target is not exact")
    elif tag_exists:
        raise AssertionError("DRC_v4.0.0 tag exists")
    if not fixed_zip_tuple_is_exact():
...
def stage4_invocation_3_authorization_sync_origin_state(head: str, origin: str) -> str | None:
    if not origin:
        return None
    if origin == CONTROL_D_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_COMMIT and head != origin:
        return "NOT_PUSHED"
    if origin == head and head != CONTROL_D_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_COMMIT:
        return "PUSHED"
    return None


def stage4_invocation_3_authorization_sync_clean_mode_after_surface_validation(
    surface_validated: bool,
    head: str,
    origin: str,
) -> str:
    if not surface_validated:
        raise AssertionError("Stage 4 invocation 3 authorization-sync origin policy reached before committed surface validation")
    state = stage4_invocation_3_authorization_sync_origin_state(head, origin)
    if state == "NOT_PUSHED":
        return "CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_NOT_PUSHED"
    if state == "PUSHED":
        return "CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC"
    raise AssertionError("clean Stage 4 invocation 3 authorization-sync origin/main state is invalid")
