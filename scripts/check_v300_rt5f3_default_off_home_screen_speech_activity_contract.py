...
EXPECTED_HEAD = "888814d09fad75039733a4a94719454e0a69db63"
...
def main() -> None:
    if git("rev-parse", "HEAD").strip() != EXPECTED_HEAD:
        raise RuntimeError("unexpected RT-5f3 implementation baseline HEAD")
    ...
    if changed != EXPECTED_FILES:
        ...
        raise RuntimeError(
            f"RT-5f3 exact 20-file surface mismatch; missing={missing}; "
            f"unexpected={unexpected}"
        )
    for path in changed:
        if path.startswith(FORBIDDEN_CHANGED_PREFIXES):
            raise RuntimeError(f"forbidden RT-5f3 changed path: {path}")
    ...
    for marker in (
        ...
        require(docs, marker, "RT-5f3 documents")

    for forbidden in (
        "RT-5f3: COMPLETED / ACCEPTED",
        "RT-5f3: PUSHED",
        "real audible soft-barge-in accepted",
        "provider hard cancel accepted",
        "release readiness: true",
    ):
        forbid(docs, forbidden, "RT-5f3 documents")
    ...
    for path in (
        ...
        if path in changed:
            raise RuntimeError(f"dependency surface changed unexpectedly: {path}")

    print("v300_rt5f3_status: implemented-awaiting-review")
    print("v300_rt5f3_exact_change_surface: True")
    print("v300_rt5f3_exact_file_count: 20")
    ...
    print("v300_rt5f3_real_execution_in_synthetic_tests: False")
    print("v300_rt5f3_commit_push_authorized: False")
