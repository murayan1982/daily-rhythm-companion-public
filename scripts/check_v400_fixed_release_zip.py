...
def stage4_content_review_runtime_connection_self_check() -> dict[str, bool]:
    ...
    return {
        ...
        "release_zip_guard_references_stage4_invocation_3_lineage_guard": release_zip_guard_names.count("check_stage4_invocation_3_authorization_sync_lineage") == 1,
...
def check_current_docs() -> None:
    ...
    if current_docs_text().count(STAGE4_RETRY_AUTHORIZATION) != len(STAGE4_RETRY_TOKEN_DOCS):
        die("Stage 4 retry authorization token occurrence is not exact 2")
...
def check_clean_committed_pushed_stage4_authorization_sync_for_release_zip(
    ...
    check_stage4_invocation_3_authorization_sync_lineage("HEAD")
    mode = stage4_invocation_3_authorization_sync_clean_mode_after_surface_validation(True, head, origin)
    if not release_zip_reachability(mode):
        die("--release-zip requires committed and pushed Stage 4 Invocation 3 authorization-sync")
...
def release_zip_runtime_guard_plan_self_check() -> dict[str, bool]:
    ...
    return {
        ...
        "stage4_guard_uses_exact_docs_contract_guard": "invocation_3_authorization_contract_is_exact_in_docs" in guard_names,
        "stage4_guard_uses_lifecycle_contract_guard": "stage4_lifecycle_contract_is_valid" in guard_names,
        "stage4_guard_uses_exact_artifact_tuple_guard": "check_expected_fixed_zip_tuple" in guard_names,
        "stage4_guard_uses_lineage_guard": "check_stage4_invocation_3_authorization_sync_lineage" in guard_names,
        ...
        "clean_not_pushed_reachability_false": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_NOT_PUSHED"),
        "clean_pushed_release_verifier_reachability": release_zip_reachability("CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC"),
        ...
        "malformed_retry_contract_blocks_docs_guard": not stage4_retry_authorization_contract_is_present(
            current_docs_text().replace("Invocation 3 authorized: TRUE", "Invocation 3 authorized: FALSE", 1)
        ),
        "actual_release_runtime_guard_probe": all(release_zip_runtime_guard_actual_probe_self_check().values()),
    }
...
def release_zip_runtime_guard_actual_probe_self_check() -> dict[str, bool]:
    ...
    canonical_docs = dict(docs)
    fake_head = "f" * 40
    calls: list[str] = []

    def run_probe(mutated_docs: dict[str, str]) -> tuple[bool, list[str]]:
        probe_calls: list[str] = []
        original_read = globals()["read"]
        original_current_docs_text = globals()["current_docs_text"]
        original_git_out = globals()["git_out"]
        original_lineage = globals()["check_stage4_invocation_3_authorization_sync_lineage"]
        original_mode = globals()["stage4_invocation_3_authorization_sync_clean_mode_after_surface_validation"]
        original_tuple = globals()["check_expected_fixed_zip_tuple"]
        original_post_source = globals()["check_post_source_head_surface"]
        original_docs_guard = globals()["invocation_3_authorization_contract_is_exact_in_docs"]
        original_lifecycle = globals()["stage4_lifecycle_contract_is_valid"]

        def fake_read(relative: str) -> str:
            if relative in mutated_docs:
                return mutated_docs[relative]
            return original_read(relative)

        def fake_current_docs_text() -> str:
            return "\n".join(mutated_docs[relative] for relative in CURRENT_DOCS)
        ...
        def fake_lineage(head: str = "HEAD") -> None:
            probe_calls.append("lineage")

        def fake_mode(surface_valid: bool, head: str, origin: str) -> str:
            probe_calls.append("mode")
            return "CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC"

        def fake_tuple() -> None:
            probe_calls.append("tuple")

        def fake_post_source(expected_head: str | None, verification_head: str) -> None:
            probe_calls.append("post_source")

        def wrapped_docs_guard() -> bool:
            probe_calls.append("exact_docs")
            return original_docs_guard()

        def wrapped_lifecycle(value: str) -> bool:
            probe_calls.append("lifecycle")
            return original_lifecycle(value)

        try:
            globals()["read"] = fake_read
            globals()["current_docs_text"] = fake_current_docs_text
            globals()["git_out"] = fake_git_out
            globals()["check_stage4_invocation_3_authorization_sync_lineage"] = fake_lineage
            globals()["stage4_invocation_3_authorization_sync_clean_mode_after_surface_validation"] = fake_mode
            globals()["check_expected_fixed_zip_tuple"] = fake_tuple
            globals()["check_post_source_head_surface"] = fake_post_source
            globals()["invocation_3_authorization_contract_is_exact_in_docs"] = wrapped_docs_guard
            globals()["stage4_lifecycle_contract_is_valid"] = wrapped_lifecycle
            ...
            return True, probe_calls
        except AssertionError:
            return False, probe_calls
        finally:
            globals()["read"] = original_read
            globals()["current_docs_text"] = original_current_docs_text
            globals()["git_out"] = original_git_out
            globals()["check_stage4_invocation_3_authorization_sync_lineage"] = original_lineage
            globals()["stage4_invocation_3_authorization_sync_clean_mode_after_surface_validation"] = original_mode
            globals()["check_expected_fixed_zip_tuple"] = original_tuple
            globals()["check_post_source_head_surface"] = original_post_source
            globals()["invocation_3_authorization_contract_is_exact_in_docs"] = original_docs_guard
            globals()["stage4_lifecycle_contract_is_valid"] = original_lifecycle

    def mutate_doc(mutator) -> dict[str, str]:
        mutated = dict(canonical_docs)
        mutated["README.md"] = mutator(mutated["README.md"])
        return mutated

    conflict = "\nInvocation 3 authorized: FALSE\nInvocation 3 completed invocation count: 1\n"
    rejected_heading_cases = {
        "r1_conflicting_heading_rejected": "## Stage 4 Invocation 3 Authorization Sync Corrective R1 Current Section",
        "r2_conflicting_heading_rejected": "## Stage 4 Invocation 3 Authorization Sync Corrective R2 Current Section",
        "r4_conflicting_heading_rejected": "## Stage 4 Invocation 3 Authorization Sync Corrective R4 Current Section",
        "versionless_conflicting_heading_rejected": "## Stage 4 Invocation 3 Authorization Sync Corrective Current Section",
        "invocation_03_conflicting_heading_rejected": "## Stage 4 Invocation 03 Authorization Sync Corrective R3 Current Section",
    }
    accepted, accepted_calls = run_probe(canonical_docs)
    result = {
        "canonical_r3_docs_accepted": accepted,
        "actual_release_guard_calls_exact_docs_validator": "exact_docs" in accepted_calls,
        "actual_release_guard_calls_lifecycle_validator": "lifecycle" in accepted_calls,
    }
    for name, heading in rejected_heading_cases.items():
        rejected, _ = run_probe(mutate_doc(lambda value, heading=heading: value + "\n" + heading + conflict))
        result[name] = not rejected
    rejected, _ = run_probe(mutate_doc(lambda value: value.replace("Invocation 3 authorized: TRUE", "Invocation 3 authorized: TRUE_EXTRA", 1)))
    result["true_extra_rejected"] = not rejected
    rejected, _ = run_probe(mutate_doc(lambda value: value.replace("Next authorized invocation number: 3", "Next authorized invocation number: 30", 1)))
    result["invocation_number_30_rejected"] = not rejected
    rejected, _ = run_probe(mutate_doc(lambda value: value.replace("Invocation 3 authorized: TRUE", "Invocation 3 authorized: TRUE\nInvocation 3 authorized: TRUE", 1)))
    result["duplicate_label_rejected"] = not rejected
    rejected, _ = run_probe(mutate_doc(lambda value: value + "\nStage 4 invocation 3: AUTHORIZED_BUT_NOT_REVIEWED\n"))
    result["authorized_but_not_reviewed_rejected"] = not rejected
    calls.extend(accepted_calls)
    return result
...
def check_static_corrective_assertions() -> None:
    ...
    if current_stage4_retry_authorization_is_present(current_text) or not invocation_3_is_authorized(current_text):
        die("current docs must authorize Invocation 3 and must not authorize Stage 4 retry")
...
def main() -> None:
    ...
    if policy.name == "default":
        ...
        print("v400_control_d_stage4_invocation_3_authorized: True")
        ...
        print(
            "v400_post_source_head_surface_count_matrix_0_1_2_3_4_5_accepted_6_rejected: "
