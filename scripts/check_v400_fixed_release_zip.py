...
def stage4_retry_authorization_sync_clean_mode_after_surface_validation(
    ...
    if not surface_validated:
        die("Stage 4 retry authorization-sync origin policy reached before committed surface validation")
...
def check_stage4_content_review_guards() -> None:
    ...
    if not tasklist_current_implementation_step_is_current(read("tasklist.md")):
        die("tasklist current implementation step is not Stage 4 Invocation 3 Pass Result Sync")
...
def check_committed_stage4_retry_authorization_sync_surface(head: str = "HEAD") -> None:
    revision_range = f"{CONTROL_D_STAGE4_VERIFIER_OUTPUT_ENCODING_CORRECTIVE_COMMIT}..{head}"
    commit_count = int(git_out("rev-list", "--count", revision_range))
    lines = git_out("diff", "--name-status", revision_range).splitlines()
    if not validate_stage4_retry_authorization_sync_committed_surface(commit_count, lines):
        die("Clean committed Stage 4 retry authorization-sync surface is not exact one-commit M12")
    protected = git_out("diff", "--name-status", revision_range, "--", *PROTECTED_PATHS).splitlines()
    if not validate_empty_protected_delta(protected):
...
def check_committed_stage4_invocation_3_authorization_sync_surface(head: str = "HEAD") -> None:
    ...
    if not validate_stage4_invocation_3_authorization_sync_committed_surface(commit_count, lines):
        die("Clean committed Stage 4 invocation 3 authorization-sync surface is not exact one-commit M12")
    protected = git_out("diff", "--name-status", revision_range, "--", *PROTECTED_PATHS).splitlines()
    if not validate_empty_protected_delta(protected):
        die("Stage 4 invocation 3 authorization-sync protected delta is not empty")
...
def check_current_docs() -> None:
    ...
    if current_docs_text().count(STAGE4_RETRY_AUTHORIZATION) != 0:
        die("Stage 4 retry authorization token occurrence is not exact 0")
    if current_docs_text().count(STAGE4_INVOCATION_3_AUTHORIZATION) != 0:
        die("Invocation 3 authorization token occurrence is not exact 0")
...
def check_clean_committed_pushed_stage4_authorization_sync_for_release_zip(
    ...
    check_stage4_invocation_3_pass_result_sync_lineage("HEAD")
    mode = stage4_invocation_3_pass_result_sync_clean_mode_after_surface_validation(True, head, origin)
    if not release_zip_reachability(mode):
        die("--release-zip is blocked because Stage 4 Invocation 3 authorization is consumed")
    current_text = current_docs_text()
    if docs_have_stage4_retry_authorization():
        die("Stage 4 invocation 3 authorization-sync release guard found an active retry authorization")
    if not invocation_3_authorization_contract_is_exact_in_docs():
        die("Stage 4 invocation 3 authorization-sync release guard document contract is not exact")
    if not stage4_lifecycle_contract_is_valid(current_text):
        die("Stage 4 invocation 3 authorization-sync release guard lifecycle contract is not exact")
    check_expected_fixed_zip_tuple(expected_tag_published=head_includes_published_control_e_commit())
...
def check_static_corrective_assertions() -> None:
    ...
    if current_stage4_retry_authorization_is_present(current_text) or not invocation_3_pass_result_contract_is_present(current_text):
        die("current docs must record consumed Invocation 3 PASS and must not authorize Stage 4 retry")
    ...
    if not all(invocation_3_authorization_schema_self_check().values()):
        die("Invocation 3 exact authorization schema self-check failed")
...
def _function_calls_name(function: object, needle: str) -> bool:
    import dis

    return any(instruction.opname in {"LOAD_GLOBAL", "LOAD_NAME"} and instruction.argval == needle for instruction in dis.get_instructions(function))


def _function_contains_constant(function: object, needle: str) -> bool:
    return any(constant == needle for constant in getattr(function, "__code__", object()).co_consts)


def release_zip_runtime_guard_actual_probe_self_check() -> dict[str, bool]:
    import dis

    verify_instructions = list(dis.get_instructions(verify_release_zip))
    first_global = next((instruction.argval for instruction in verify_instructions if instruction.opname in {"LOAD_GLOBAL", "LOAD_NAME"}), None)
    structural_guard_present = _function_calls_name(
        verify_release_zip,
        "check_clean_committed_pushed_stage4_authorization_sync_for_release_zip",
    )
    structural_guard_first = first_global == "check_clean_committed_pushed_stage4_authorization_sync_for_release_zip"
    synthetic_head = "a" * 40
    calls: list[str] = []
    observed_modes: list[str] = []
    canonical_rejections: list[str] = []

    original_git_out = globals()["git_out"]
    original_lineage = globals()["check_stage4_invocation_3_pass_result_sync_lineage"]
    original_mode = globals()["stage4_invocation_3_pass_result_sync_clean_mode_after_surface_validation"]
    original_path_is_file = Path.is_file
    original_check_release_outputs = globals()["check_release_outputs_for_zip"]
    original_check_release_mode = globals()["check_release_mode_worktree_state"]
    original_check_post_source = globals()["check_post_source_head_surface"]
    original_verify_scan = globals()["verify_release_package_scan"]
    original_run_extracted = globals()["run_extracted"]
    original_subprocess_run = subprocess.run
    original_zipfile = zipfile.ZipFile

    def fake_git_out(*args: str) -> str:
        if args == ("branch", "--show-current"):
            return "main"
        if args == ("diff", "--cached", "--name-status"):
            return ""
        if args == ("remote", "get-url", "origin"):
            return "https://github.com/katsutoishii/DailyRhythmCompanion.git"
        if args == ("rev-parse", "HEAD") or args == ("rev-parse", "origin/main"):
            return synthetic_head
        return original_git_out(*args)

    def observed_lineage(head: str = "HEAD") -> None:
        calls.append("pass_result_lineage")

    def observed_mode(surface_valid: bool, head: str, origin: str) -> str:
        mode = original_mode(surface_valid, head, origin)
        calls.append("mode")
        observed_modes.append(mode)
        return mode

    def body_boundary(name: str):
        def inner(*args, **kwargs):
            calls.append(name)
            raise AssertionError(f"{name} must not be reached")
        return inner

    def fake_path_is_file(self: Path) -> bool:
        calls.append("path_is_file")
        raise AssertionError("path validation body must not be reached")

    class ForbiddenZipFile:
        def __init__(self, *args, **kwargs):
            calls.append("zipfile")
            raise AssertionError("zip body must not be reached")

    try:
        globals()["git_out"] = fake_git_out
        globals()["check_stage4_invocation_3_pass_result_sync_lineage"] = observed_lineage
        globals()["stage4_invocation_3_pass_result_sync_clean_mode_after_surface_validation"] = observed_mode
        Path.is_file = fake_path_is_file
        globals()["check_release_outputs_for_zip"] = body_boundary("release_outputs")
        globals()["check_release_mode_worktree_state"] = body_boundary("release_mode_worktree")
        globals()["check_post_source_head_surface"] = body_boundary("post_source")
        globals()["verify_release_package_scan"] = body_boundary("scanner")
        globals()["run_extracted"] = body_boundary("backend_pytest")
        subprocess.run = body_boundary("subprocess")
        zipfile.ZipFile = ForbiddenZipFile
        try:
            verify_release_zip(
                Path(EXPECTED_FIXED_ZIP_BASENAME),
                EXPECTED_FIXED_ZIP_SHA256,
                EXPECTED_FIXED_ZIP_SOURCE_HEAD,
                False,
                False,
                None,
            ...
            canonical_rejections.append(str(exc))
    finally:
        globals()["git_out"] = original_git_out
        globals()["check_stage4_invocation_3_pass_result_sync_lineage"] = original_lineage
        globals()["stage4_invocation_3_pass_result_sync_clean_mode_after_surface_validation"] = original_mode
        Path.is_file = original_path_is_file
        globals()["check_release_outputs_for_zip"] = original_check_release_outputs
        globals()["check_release_mode_worktree_state"] = original_check_release_mode
        globals()["check_post_source_head_surface"] = original_check_post_source
        globals()["verify_release_package_scan"] = original_verify_scan
        globals()["run_extracted"] = original_run_extracted
        subprocess.run = original_subprocess_run
        zipfile.ZipFile = original_zipfile

    negative_calls: list[str] = []
    original_guard = globals()["check_clean_committed_pushed_stage4_authorization_sync_for_release_zip"]

    def ineffective_guard(path: Path, expected_sha: str | None, expected_head: str | None) -> None:
        negative_calls.append("ineffective_guard")
        return None

    def negative_path_is_file(self: Path) -> bool:
        negative_calls.append("path_is_file")
        raise AssertionError("ineffective guard reached protected body boundary")

    try:
        globals()["git_out"] = fake_git_out
        globals()["check_clean_committed_pushed_stage4_authorization_sync_for_release_zip"] = ineffective_guard
        Path.is_file = negative_path_is_file
        try:
            verify_release_zip(
                Path(EXPECTED_FIXED_ZIP_BASENAME),
                EXPECTED_FIXED_ZIP_SHA256,
        ...
        globals()["git_out"] = original_git_out
        globals()["check_clean_committed_pushed_stage4_authorization_sync_for_release_zip"] = original_guard
        Path.is_file = original_path_is_file

    expected_rejection = "--release-zip is blocked because Stage 4 Invocation 3 authorization is consumed"
    canonical_rejected = canonical_rejections == [expected_rejection]
    observed_clean_pushed = observed_modes == ["CLEAN_COMMITTED_STAGE4_INVOCATION_3_PASS_RESULT_SYNC"]
    return {
        "verify_release_zip_calls_stage4_guard": structural_guard_present,
        "stage4_guard_is_first_verifier_body_boundary": structural_guard_first,
        "actual_guard_called_once": calls.count("pass_result_lineage") == 1,
        "actual_pass_result_lineage_helper_called_once": calls.count("pass_result_lineage") == 1,
        "observed_mode_equals_clean_pushed_pass_result_sync": observed_clean_pushed,
        "clean_pushed_pass_result_sync_simulated": observed_clean_pushed,
        "canonical_result_sync_docs_rejected_for_release_zip": canonical_rejected,
        "exact_consumed_rejection_observed": canonical_rejected,
        "zip_path_validation_body_not_reached": "path_is_file" not in calls,
        "release_outputs_not_reached": "release_outputs" not in calls,
        "release_mode_worktree_not_reached": "release_mode_worktree" not in calls,
        "post_source_not_reached": "post_source" not in calls,
        "release_package_scan_not_reached": "scanner" not in calls,
        "backend_pytest_not_reached": "backend_pytest" not in calls,
        "builder_flutter_build_not_reached": "subprocess" not in calls,
        "zip_body_not_reached": "zipfile" not in calls,
        "ineffective_guard_called_once": negative_calls.count("ineffective_guard") == 1,
        "ineffective_guard_detected": "path_is_file" in negative_calls,
        "ineffective_guard_reaches_protected_body_boundary": "path_is_file" in negative_calls,
    }


def release_zip_runtime_guard_plan_self_check() -> dict[str, bool]:
    verify_calls_guard = _function_calls_name(
        verify_release_zip,
        "check_clean_committed_pushed_stage4_authorization_sync_for_release_zip",
    )
    verify_first_global = next(
        (
            instruction.argval
            for instruction in __import__("dis").get_instructions(verify_release_zip)
            if instruction.opname in {"LOAD_GLOBAL", "LOAD_NAME"}
        ),
        None,
    )
    guard_uses_result_sync_lineage = _function_calls_name(
        check_clean_committed_pushed_stage4_authorization_sync_for_release_zip,
        "check_stage4_invocation_3_pass_result_sync_lineage",
    )
    guard_uses_authorization_sync_lineage = _function_calls_name(
        check_clean_committed_pushed_stage4_authorization_sync_for_release_zip,
        "check_stage4_invocation_3_authorization_sync_lineage",
    )
    guard_uses_mode_helper = _function_calls_name(
        check_clean_committed_pushed_stage4_authorization_sync_for_release_zip,
        "stage4_invocation_3_pass_result_sync_clean_mode_after_surface_validation",
    )
    guard_uses_reachability = _function_calls_name(
        check_clean_committed_pushed_stage4_authorization_sync_for_release_zip,
        "release_zip_reachability",
    )
    guard_uses_tuple_guard = _function_calls_name(
        check_clean_committed_pushed_stage4_authorization_sync_for_release_zip,
        "check_expected_fixed_zip_tuple",
    )
    guard_contains_consumed_rejection = _function_contains_constant(
        check_clean_committed_pushed_stage4_authorization_sync_for_release_zip,
        "--release-zip is blocked because Stage 4 Invocation 3 authorization is consumed",
    )
    actual_probe = release_zip_runtime_guard_actual_probe_self_check()
    return {
        "release_zip_dispatch_uses_stage4_guard": verify_calls_guard,
        "stage4_guard_is_first_verifier_boundary": verify_first_global == "check_clean_committed_pushed_stage4_authorization_sync_for_release_zip",
        "stage4_guard_uses_consumed_result_sync_lineage_guard": guard_uses_result_sync_lineage and not guard_uses_authorization_sync_lineage,
        "stage4_guard_calls_pass_result_sync_clean_mode_helper": guard_uses_mode_helper,
        "stage4_guard_calls_release_zip_reachability": guard_uses_reachability,
        "stage4_guard_contains_consumed_authorization_rejection": guard_contains_consumed_rejection,
        "stage4_guard_uses_exact_artifact_tuple_guard": guard_uses_tuple_guard,
        "actual_release_runtime_guard_probe": all(actual_probe.values()),
        "ineffective_guard_negative_control_detected": actual_probe["ineffective_guard_detected"],
        "dirty_candidate_reachability_false": not release_zip_reachability("DIRTY_STAGE4_AUTHORIZATION_SYNC_CANDIDATE"),
        "dirty_invocation_3_reachability_false": not release_zip_reachability("DIRTY_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC_CANDIDATE"),
        "dirty_pass_result_reachability_false": not release_zip_reachability("DIRTY_STAGE4_INVOCATION_3_PASS_RESULT_SYNC_CANDIDATE"),
        "clean_not_pushed_reachability_false": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_INVOCATION_3_PASS_RESULT_SYNC_NOT_PUSHED"),
        "clean_pushed_release_verifier_blocked": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_INVOCATION_3_PASS_RESULT_SYNC"),
        "old_initial_authorization_reachability_false": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_AUTHORIZATION_SYNC"),
        "old_invocation_3_authorization_reachability_false": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_INVOCATION_3_AUTHORIZATION_SYNC"),
        "release_zip_verification_mode_reachability_false": not release_zip_reachability("RELEASE_ZIP_VERIFICATION"),
        "unknown_mode_reachability_false": not release_zip_reachability("UNKNOWN_MODE"),
        "corrective_reachability_false": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_VERIFIER_OUTPUT_ENCODING_CORRECTIVE"),
        "dirty_basetemp_corrective_reachability_false": not release_zip_reachability("DIRTY_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_CANDIDATE"),
        "clean_not_pushed_basetemp_corrective_reachability_false": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE_NOT_PUSHED"),
        "clean_pushed_basetemp_corrective_reachability_false": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE"),
        "malformed_retry_contract_blocks_docs_guard": not stage4_retry_authorization_contract_is_present(
            current_docs_text().replace("Invocation 3 authorization state: CONSUMED", "Invocation 3 authorized: TRUE", 1)
        ),
        "basetemp_corrective_cannot_authorize_invocation_3": not release_zip_reachability("CLEAN_COMMITTED_STAGE4_VERIFIER_PYTEST_BASETEMP_ISOLATION_CORRECTIVE"),
    }
...
def main() -> None:
    ...
    print("v400_fixed_release_zip_tag_created: True")
    print("v400_fixed_release_zip_github_release_created: True")
    ...
    if policy.name == "default":
        ...
        print(f"v400_control_e_canonical_zero_token_accepted: {control_e_schema_checks['canonical_zero_token_accepted']}")
        print(f"v400_control_e_normalized_active_token_count_0: {control_e_schema_checks['normalized_active_token_count_0']}")
        print(f"v400_control_e_all_reinjection_variants_rejected: {control_e_schema_checks['all_reinjection_variants_rejected']}")
        print(f"v400_control_e_reinjection_fixture_payloads_all_distinct: {control_e_schema_checks['reinjection_fixture_payloads_all_distinct']}")
        print(f"v400_control_e_lowercase_mixed_nfkc_rejected: {control_e_schema_checks['lowercase_rejected'] and control_e_schema_checks['mixed_case_rejected'] and control_e_schema_checks['nfkc_rejected']}")
        print(f"v400_control_e_whitespace_newline_unicode_zero_width_rejected: {control_e_schema_checks['whitespace_rejected'] and control_e_schema_checks['newline_rejected'] and control_e_schema_checks['unicode_rejected'] and control_e_schema_checks['zero_width_rejected']}")
        print(f"v400_control_e_markdown_historical_non_designated_rejected: {control_e_schema_checks['markdown_rejected'] and control_e_schema_checks['historical_rejected'] and control_e_schema_checks['non_designated_rejected']}")
        print(f"v400_control_e_bounded_consumed_prose_accepted: {control_e_schema_checks['bounded_consumed_prose_accepted']}")
        print(f"v400_control_e_canonical_r3_heading_accepted: {control_e_schema_checks['canonical_r3_heading_accepted']}")
        print(f"v400_control_e_adversarial_heading_matrix_rejected: {control_e_schema_checks['all_adversarial_heading_shadow_contracts_rejected']}")
        ...
        print(f"v400_control_e_publication_reachability_dirty: {control_e_publication_reachability('DIRTY_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_CANDIDATE')}")
        print(f"v400_control_e_publication_reachability_clean_not_pushed: {control_e_publication_reachability('CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_NOT_PUSHED')}")
        print(f"v400_control_e_publication_reachability_clean_pushed: {control_e_publication_reachability('CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE')}")
        print(f"v400_control_e_release_zip_reachability_dirty: {release_zip_reachability('DIRTY_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_CANDIDATE')}")
        print(f"v400_control_e_release_zip_reachability_clean_not_pushed: {release_zip_reachability('CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE_NOT_PUSHED')}")
        print(f"v400_control_e_release_zip_reachability_clean_pushed: {release_zip_reachability('CLEAN_COMMITTED_CONTROL_E_RELEASE_PUBLICATION_RESULT_SYNC_CORRECTIVE')}")
        print(
        ...
        print(
            "v400_stage4_retained_heading_family_case_count: "
        ...
        print(
            "v400_stage4_lifecycle_contract_self_check_count: "
        ...
        print(f"v400_dirty_invocation_3_release_verifier_blocked: {release_zip_guard_checks['dirty_invocation_3_reachability_false']}")
        print(f"v400_clean_not_pushed_release_verifier_blocked: {release_zip_guard_checks['clean_not_pushed_reachability_false']}")
        print(f"v400_clean_pushed_release_verifier_blocked: {release_zip_guard_checks['clean_pushed_release_verifier_blocked']}")
        print(
        ...
        print(
            "v400_basetemp_corrective_cannot_authorize_invocation_3: "
            f"{release_zip_guard_checks['basetemp_corrective_cannot_authorize_invocation_3']}"
        )
        ...
        print(f"v400_invocation_3_exact_authorization_schema_self_check: {all(invocation_3_schema_checks.values())}")
        print(f"v400_invocation_3_clean_not_pushed_and_pushed_dispatch_simulated: {invocation_3_schema_checks['dirty_invocation_3_release_zip_reachability_false'] and invocation_3_schema_checks['clean_not_pushed_invocation_3_release_zip_reachability_false'] and invocation_3_schema_checks['clean_pushed_invocation_3_release_zip_reachability_true']}")
        ...
        print(
            "v400_post_source_head_surface_count_matrix_0_1_2_3_4_5_6_7_8_9_accepted_10_rejected: "
    ...
    print("v400_control_e_active_authorization: False")
    print(f"v400_published_tag_tuple_exact: {published_tag_tuple_is_exact_in_repo()}")
    print(f"v400_published_tag_negative_fixtures: {all(published_tag_tuple_self_check().values())}")
    print(f"v400_publication_record_negative_fixtures: {all(publication_record_schema_self_check().values())}")
    print("v400_control_e_execution_recorded: True")
    print("v400_additional_publication_authorized: False")
...
def stage4_invocation_3_authorization_sync_clean_mode_after_surface_validation(
    ...
    if not surface_validated:
        die("Stage 4 invocation 3 authorization-sync origin policy reached before committed surface validation")
    ...
    die("clean Stage 4 invocation 3 authorization-sync origin/main state is invalid")
