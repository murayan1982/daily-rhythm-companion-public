# Validate DRC v3.0.0 RT-6b provider-neutral motion mapping candidate.
...
DRC_BASELINE_HEAD = "6ed5f2252c6c6f47fc8c50f577c4f20b7fa0cb68"
DRC_BASELINE_PARENT = "cbcb218aa54d286da7515a01e899121b22d8f3fc"
...
BACKEND_TEST_PATHS = {"backend/tests/test_character_motion_mapper.py"}
RT6A_ACCEPTANCE_PATHS = {
    "README.md",
    "roadmap.md",
    "tasklist.md",
    "scripts/README.md",
...
def assert_repository_state(*, snapshot: bool) -> None:
    ...
    if actual != EXPECTED_PATHS:
        raise AssertionError(f"RT-6b exact surface mismatch: {sorted(actual)}")
    if snapshot:
        return

    if run("git", "rev-parse", "HEAD", capture=True) != DRC_BASELINE_HEAD:
        raise AssertionError("Unexpected DRC RT-6b baseline HEAD.")
    if run("git", "rev-parse", "origin/main", capture=True) != DRC_BASELINE_HEAD:
        raise AssertionError("Unexpected DRC origin/main for RT-6b.")
    if run("git", "rev-parse", "HEAD^", capture=True) != DRC_BASELINE_PARENT:
        raise AssertionError("Unexpected RT-6a acceptance-sync parent.")
    if commit_paths(DRC_BASELINE_HEAD) != RT6A_ACCEPTANCE_PATHS:
        raise AssertionError("RT-6a acceptance-sync commit was not exact seven-file.")
...
def assert_changed_content_safe() -> None:
    ...
    untracked_text = "\n".join(
        ...
        if relative in changed_paths() and relative not in run(
            "git", "ls-files", capture=True
        ).splitlines()
    )
    added = "\n".join(
        line[1:]
    ...
    candidate_text = added + "\n" + untracked_text
    for pattern in SENSITIVE_PATTERNS:
        if re.search(pattern, candidate_text):
            raise AssertionError(f"Sensitive-looking value in RT-6b candidate: {pattern}")
...
def assert_docs() -> None:
    ...
    for marker in (
        ...
        "RT-6b baseline: " + DRC_BASELINE_HEAD,
        ...
        require(combined, marker, f"RT-6b marker {marker}")
    ...
    active_status = "\n".join(
        (
            "\n".join(sources["README"].splitlines()[:120]),
            "\n".join(sources["roadmap"].splitlines()[:820]),
            "\n".join(sources["tasklist"].splitlines()[:80]),
            "\n".join(sources["checklist"].splitlines()[:115]),
        )
    )
    for marker in (
        ...
        forbid(active_status, marker, f"stale marker {marker}")
...
def main() -> None:
    ...
    markers = {
        ...
        "v300_rt6b_change_file_count": len(EXPECTED_PATHS),
        "v300_rt6b_backend_runtime_file_count": len(BACKEND_RUNTIME_PATHS),
        "v300_rt6b_backend_test_file_count": len(BACKEND_TEST_PATHS),
