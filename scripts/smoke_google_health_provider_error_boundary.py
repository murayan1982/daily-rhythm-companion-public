...
SCRIPT_NAME = "google-health-provider-error-boundary-smoke-v0.22.0"
...
class SmokeFailure(AssertionError):
    """Raised when the provider error boundary smoke fails."""
...
def _require_allow_real_request(allow_real_request: bool) -> None:
    if not allow_real_request:
        raise SmokeRefused(
            "Refusing to inspect a real Google Health provider error boundary. "
            "Re-run with --allow-real-request only after enabling the local .env "
            "real request gates for a narrow troubleshooting run."
        )
...
def _assert_preflight_allows_real_request(preflight: dict[str, Any]) -> None:
    ...
    for field in (
        ...
        _require(
            ...
            f"Expected preflight.api.{field}=True, got {api.get(field)!r}.",
        )


def _assert_provider_error_boundary(self_check: dict[str, Any]) -> tuple[int, str]:
    session = self_check.get("session") or {}
    api = session.get("api") or {}

    _require(
        self_check.get("real_http_attempted") is True,
        "Expected real_http_attempted=True for provider error boundary inspection.",
    )

    status_code = api.get("status_code")
    _require(
        isinstance(status_code, int),
        "Expected session.api.status_code to be captured as an integer.",
    )

    provider_error_category = api.get("provider_error_category")
    if status_code >= 400:
        _require(
            provider_error_category in SAFE_PROVIDER_ERROR_CATEGORIES,
            "Expected a safe provider_error_category for HTTP error responses, "
    ...
    return status_code, str(provider_error_category or "")
...
def run_smoke(*, base_url: str, target_date: str, allow_real_request: bool) -> None:
    ...
    status_code, category = _assert_provider_error_boundary(self_check)

    print(
        "[google-health-provider-error-boundary] "
        f"target_date={target_date} status_code={status_code} "
        f"provider_error_category={category} "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Verify that Google Health provider HTTP errors are reduced to safe "
            "status/category fields and raw provider payloads are not exposed by "
